package org.xendra.modeleditor.search;
...
import org.xendra.modeleditor.folder.virtual.VirtualFolder;
import org.xendra.modeleditor.gui.frame.ModelEditorFrameMediator;
import org.xendra.modeleditor.gui.search.BasicResultPanel;
import org.xendra.modeleditor.gui.search.ComplexResultPanel;
import org.xendra.modeleditor.search.RuleSearchProvider;
...
public class RuleSearchProvider implements ISearchProvider {
	...
	public RuleSearchProvider() {
		bundle = ResourceBundle.getBundle("org.xendra.modeleditor.i18n.search");
	}
	...
	public ImageIcon getIcon() {
    	ImageIcon icon = null;	
		try {
    		icon  = new ImageIcon(ImageIO.read(new File(Env.getResource("org.xendra.modeleditor", "drools.gif"))));
    	}
    	catch (Exception e) {}
    	return icon;
	}
	...
	public List<ISearchResult> query(String searchTerm,
		...
		List<X_AD_Table> tables = new Query(Env.getCtx(), X_AD_Table.Table_Name, where, null).list();
		for (X_AD_Table table:tables)
		{			
				SearchResult sr = new SearchResult(table.getName(), table.getName(), null);
				result.add(sr);
				// remember search result
				virtualFolder.add(table);			
		}
	...
	public void showAllResults(IFrameMediator mediator, String searchTerm, String searchCriteriaTechnicalName) {
		...
		((ModelEditorFrameMediator) newMediator).getTree().setSelectedFolder(virtualFolder);
	}
