 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: WorkflowProcessor.java,v 1.1 2007/06/15 00:03:25 sergioaguayo Exp $
 */
public class WorkflowProcessor extends XendraServer
{
	/**
	 * 	WorkflowProcessor
	 *	@param model model
	...
	private void wakeup()
	{
		String sql = "SELECT * "
			...
			+ " AND EndWaitTime > CURRENT_TIMESTAMP"
			+ " AND AD_Client_ID=?"
		...
		String subject, String message, File pdf)
	{
		...
		if (responsible.isInvoker())
			;
		//	Human
		else if (REF_WF_ParticipantType.Human.equals(responsible.getResponsibleType())
		...
		//	Org of the Document
		else if (REF_WF_ParticipantType.Organization.equals(responsible.getResponsibleType()))
		{
		...
		//	Role
		else if (REF_WF_ParticipantType.Role.equals(responsible.getResponsibleType())
