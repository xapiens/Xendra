 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: AlertProcessor.java,v 1.1 2007/06/15 00:03:25 sergioaguayo Exp $
 */
public class AlertProcessor extends XendraServer
{
	/**
	 * 	Alert Processor
	 *	@param model model
	...
	private boolean processAlert (MAlert alert)
	{
		...
		for (MAlertRule rule:rules)
		{
			if (message == null)
				message = new StringBuffer(alert.getAlertMessage()).append(Env.NL);
			else
		...
		for (MAlertRecipient recipient:recipients)
		{
