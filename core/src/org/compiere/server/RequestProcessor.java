 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: RequestProcessor.java,v 1.2 2007/06/19 20:46:08 sergioaguayo Exp $
 */
public class RequestProcessor extends XendraServer
{
	/**
	 * 	RequestProcessor
	 *	@param model model
	...
	private void processRequests ()
	{
		...
		String sql = "SELECT * FROM R_Request "
			+ "WHERE DueType='" + REF_R_RequestDueType.Scheduled + "' AND Processed='N'"
			+ " AND DateNextAction < CURRENT_TIMESTAMP"
			+ " AND AD_Client_ID=?"; 
		...
		sql = "SELECT * FROM R_Request r "
			+ "WHERE r.DueType='" + REF_R_RequestDueType.Due + "' AND r.Processed='N'"
			+ " AND AD_Client_ID=?"
			+ " AND EXISTS (SELECT * FROM R_RequestType rt "
				+ "WHERE r.R_RequestType_ID=rt.R_RequestType_ID"
				+ " AND (r.DateNextAction+rt.DueDateTolerance) < CURRENT_TIMESTAMP)";
		if (m_model.getR_RequestType_ID() != 0)
		...
		if (m_model.getOverdueAlertDays() > 0)
		{
			sql = "SELECT * FROM R_Request "
				+ "WHERE Processed='N'"
				+ " AND AD_Client_ID=?"
				+ " AND (DateNextAction+" + m_model.getOverdueAlertDays() + ") < CURRENT_TIMESTAMP"
		...
		if (m_model.getInactivityAlertDays() > 0)
		{
			sql = "SELECT * FROM R_Request "
				+ "WHERE Processed='N'"
				+ " AND AD_Client_ID=?"
				+ " AND (Updated+" + m_model.getInactivityAlertDays() + ") < CURRENT_TIMESTAMP"
				+ " AND (DateLastAlert IS NULL";
			if (m_model.getRemindDays() > 0)
				sql += " OR (DateLastAlert+" + m_model.getRemindDays() 
					+ ") < CURRENT_TIMESTAMP";
	...
	private void processStatus()
	{
		...
		String sql = "SELECT * FROM R_Request r WHERE EXISTS ("
			...
			+ " AND r.Updated+s.TimeoutDays < CURRENT_TIMESTAMP"
			+ ") "
