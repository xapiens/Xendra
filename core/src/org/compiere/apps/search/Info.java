 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: Info.java 5498 2014-12-22 06:24:51Z xapiens $
 */
	...
	implements ListSelectionListener
{
	...
	/**
	 * Show Cash Flow (non modal)
	 * @param frame Parent Frame
	 * @param WindowNo window no
	 */
	public static void showCashFlow (Frame frame, int WindowNo)
	{
		Info info = new InfoCashFlow (frame, false, WindowNo,  
			0, "", false, "");
		AEnv.showCenterWindow(frame, info);
	}   //  showCashFlow
	...
	public static void showBOE(Frame frame, int WindowNo) {
		Info info = new InfoBOE (frame, false, WindowNo, "",		//	value 
				false, "");
			AEnv.showCenterWindow(frame, info);		
	}	
	/** Window Width                */
	...
	{
		p_layout = layout;
		p_table.setModel(new DefaultTableModel()); // el_man
		...
		//
		if (staticWhere != null && staticWhere.length()>0)
		sql.append(" WHERE ").append(staticWhere);
		
		if (from.length()==0 && staticWhere.length()==0)
		sql.append(")");
	...
	private void testCount()
	{
		...
		org.compiere.apps.SwingWorker worker = new org.compiere.apps.SwingWorker()
		{
			...
			private void cmd_process() {
				...
				{
					setBusy(false);
					goAhead = ADialog.ask(p_WindowNo, null, "InfoHighRecordCount", String.valueOf(no));					
				}							
				if (goAhead)
				{
					setBusy(true);
