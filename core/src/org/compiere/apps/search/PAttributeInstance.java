 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: PAttributeInstance.java 4091 2012-04-10 20:33:02Z xapiens $
 */
	...
	implements ListSelectionListener
{
	...
	private int 				m_Ref_AttributeSetInstance_ID = -1;
	...
	private static ColumnInfo[] s_layout = new ColumnInfo[] 
	{
		...
		//
		new ColumnInfo(" ", "asi.ref_AttributeSetInstance_ID", Integer.class)
	};
	/**	From Clause							*/
	...
	private void dynInit(int C_BPartner_ID)
	{
		...
		if (C_BPartner_ID != 0)
		{
			...
			{
				m_sqlMinLife = " AND COALESCE(TRUNC(((TRUNC(asi.GuaranteeDate)-TRUNC(CURRENT_TIMESTAMP))/p.GuaranteeDays)*100),0)>=" + ShelfLifeMinPct;
				log.config( "PAttributeInstance.dynInit - ShelfLifeMinPct=" + ShelfLifeMinPct);
			}
			if (ShelfLifeMinDays > 0)
			{
				m_sqlMinLife += " AND COALESCE((TRUNC(asi.GuaranteeDate)-TRUNC(CURRENT_TIMESTAMP)),0)>=" + ShelfLifeMinDays;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getActionCommand().equals(ConfirmPanel.A_CANCEL))
		{
			...
			m_Ref_AttributeSetInstance_ID = -1;
	...
	private void enableButtons()
	{
		...
		if (enabled)
		{
			...
			if (ID != null)
			{
				...
				m_Ref_AttributeSetInstance_ID = (Integer) m_table.getValueAt(row, 12);
				//
		...
		log.fine("M_AttributeSetInstance_ID=" + m_M_AttributeSetInstance_ID 
			...
			+ " Ref_AttributeSetInstance_ID=" + m_Ref_AttributeSetInstance_ID
