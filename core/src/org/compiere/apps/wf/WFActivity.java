 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: WFActivity.java 5498 2014-12-22 06:24:51Z xapiens $
 */
	...
	implements FormPanel, ActionListener
{
	...
	public void display ()
	{
		...
		if (REF_WF_Action.UserChoice.equals(node.getAction()))
		{
			if (m_column == null)
		...
		//	--
		else if (REF_WF_Action.UserWindow.equals(node.getAction())
			|| REF_WF_Action.UserForm.equals(node.getAction()))
		{
			fAnswerButton.setText(node.getName());
			fAnswerButton.setToolTipText(node.getDescription());
			fAnswerButton.setVisible(true);
		}
		else if (REF_WF_Action.UserWorkbench.equals(node.getAction()))
	...
	private void cmd_button()
	{
		...
		//
		MWFNode node = m_activity.getNode();
		if (REF_WF_Action.UserWindow.equals(node.getAction()))
		{
		...
		else if (REF_WF_Action.UserForm.equals(node.getAction()))
		{
	...
	private void cmd_OK()
	{
		...
		//	User Choice - Answer
		else if (REF_WF_Action.UserChoice.equals(node.getAction()))
		{
			if (m_column == null)
				m_column = node.getColumn();
