 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: ProcessDialog.java 4577 2012-12-02 19:51:01Z xapiens $
 *  @author		Low Heng Sin
	...
	implements ActionListener, ASyncProcess
{
	...
	private void Initialize() throws Exception {
		...
		gbl_southPanel.columnWidths = new int[]{67, 0};
		gbl_southPanel.rowHeights = new int[]{39, 0, 0};
		...
		CPanel panel = new CPanel(new FlowLayout());
		...
		southPanel.add(statusBar, gbc_statusBar);
	...
	public ProcessDialog(String Identifier, Integer Record_ID, boolean isSOTrx) {
		super();
		MProcess process = new Query(Env.getCtx(), MProcess.Table_Name, "Identifier = ?", null)
			.setParameters(Identifier).first();
		if (process != null) {
			log.info("Process=" + process.getAD_Process_ID() + "; SOTrx=" + isSOTrx);
			enableEvents(AWTEvent.WINDOW_EVENT_MASK);
			m_AD_Process_ID = process.getAD_Process_ID();
	...
	public boolean init()
	{
		...
		String sql = "SELECT Name, Description, Help, IsReport, ShowHelp, classname, value, jasperreport "
				+ " FROM AD_Process "
				+ " WHERE AD_Process_ID=?";
		if (trl)
			sql = "SELECT t.Name, t.Description, t.Help, p.IsReport, p.ShowHelp, classname, value, jasperreport "
		...
		try
		{
			...
			if (rs.next())
			{
				...
				m_ClassName = rs.getString(6);
				m_value = rs.getString(7);
				m_jasper = rs.getString(8);
				//
		...
		if (m_Name == null) {
			throw new XendraException("@NotFound@ @AD_Process_ID@="+m_AD_Process_ID+". @CheckMissingTrl@");
		}
		//
		this.setTitle(m_Name);
		message.setText(m_messageText.toString());
		//this.statusBar.setInfo(m_ClassName);
		this.statusBar.setStatusLine(m_ClassName, false);
		
		if (m_jasper != null && m_jasper.length() > 0)
		{
			this.statusBar.setStatusDB(m_jasper);
		}
		...
		//m_pi.setExportExcel(chkExport.isSelected());
