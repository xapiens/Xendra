 ...
 *
 *  @author Jorg Janke
 *  @version $Id: VInvoiceGen.java 5498 2014-12-22 06:24:51Z xapiens $
 */
	...
	implements FormPanel, ActionListener, VetoableChangeListener, ChangeListener, TableModelListener, ASyncProcess
{
	...
	void jbInit() throws Exception
	{
		...
		fComplete.setText("Completar factura");
		fComplete.setSelected(false);
		...
		selNorthPanel.add(fComplete, null);
	...
	//	lBPartner.setText(Msg.translate(Env.getCtx(), "C_BPartner_ID"));
		...
		MLookup doctypeL = MLookupFactory.get(Env.getCtx(), m_WindowNo, DisplayType.TableDir, 1501, Env.getLanguage(Env.getCtx()), "C_DocType_ID", 172, false, "C_DocType.docbasetype='ARI'");
		fDocType = new VLookup ("C_DocType_ID", false, false, true, doctypeL);
	...
	private void dynInit()
	{
		...
		miniTable.addColumn("C_BPartner_Location_ID");
		//
		...
		miniTable.setColumnClass(7, String.class, true, Msg.translate(Env.getCtx(), "C_BPartner_Location_ID"));
		//
		miniTable.autoSize();
	...
	//	log.fine( "VInvoiceGen.executeQuery - AD_Client_ID=" + AD_Client_ID, sql.toString());
		...
		try
		{
			...
			while (rs.next())
			{
				...
				miniTable.setValueAt(rs.getString(8), row, 7);              //  Partner Location
				//  prepare next
				row++;
			}
			rs.close();
	...
	private void generateInvoices ()
	{
		...
		if (fComplete.isSelected() == false) {
			...
			para.setParameter("DocAction", "PR");
			if (!para.save())
			{
		...
		para.setParameter("C_DocType_ID", new BigDecimal((Integer)fDocType.getValue()));
		if (!para.save())
		{
			String msg = "No parameter added";  // not translated
			info.setText(msg);
