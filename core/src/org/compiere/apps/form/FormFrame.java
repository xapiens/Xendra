 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: FormFrame.java 5498 2014-12-22 06:24:51Z xapiens $
 * 
	...
	implements ActionListener 
{
	...
	{
		this.setIconImage(org.compiere.Xendra.getImage16());
	...
	public boolean openForm (int AD_Form_ID)
	{
		...
		String sql = "SELECT Name, Description, ClassName, Help, IsMenu FROM AD_Form WHERE AD_Form_ID=?";
		boolean trl = !Env.isBaseLanguage(ctx, "AD_Form");
		if (trl)
			sql = "SELECT t.Name, t.Description, f.ClassName, t.Help, f.IsMenu "
		...
		try
		{
			...
			if (rs.next())
			{
				name = rs.getString(X_AD_Form.COLUMNNAME_Name);
				m_Description = rs.getString(X_AD_Form.COLUMNNAME_Description);
				className = rs.getString(X_AD_Form.COLUMNNAME_Classname);
				m_Help = rs.getString(X_AD_Form.COLUMNNAME_Help);
				m_isMenu = rs.getString(X_AD_Form.COLUMNNAME_IsMenu).equals("Y");
			}
		}
	...
	public boolean openForm(String className, String name)
	{
		Properties ctx = Env.getCtx();
		Env.setContext(ctx, m_WindowNo, "WindowName", name);
		setTitle(Env.getHeader(ctx, m_WindowNo));
		...
		{
			log.log(Level.SEVERE, "Class=" + className, e);
			return false;
		}
		//
		m_panel.init(m_WindowNo, this);		
