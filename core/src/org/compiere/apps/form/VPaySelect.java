 ...
 *
 *  @author Jorg Janke
 *  @version $Id: VPaySelect.java 5498 2014-12-22 06:24:51Z xapiens $
 */
	...
	implements FormPanel, ActionListener, TableModelListener, ASyncProcess
{
	...
	private void dynInit()
	{
		...
		paymentTermDiscount(i.GrandTotal, i.C_PaymentTerm_ID, i.DateInvoiced, CURRENT_TIMESTAMP) AS Discount,
		CURRENT_TIMESTAMP-paymentTermDueDays(i.C_PaymentTerm_ID,i.DateInvoiced) AS DiscountDate,
		i.GrandTotal-paymentTermDiscount(i.GrandTotal,i.C_PaymentTerm_ID,i.DateInvoiced,CURRENT_TIMESTAMP) AS DueAmount,
		currencyConvert(i.GrandTotal-paymentTermDiscount(i.GrandTotal,i.C_PaymentTerm_ID,i.DateInvoiced,CURRENT_TIMESTAMP,null),
			0,i.C_Currency_ID,xx100,CURRENT_TIMESTAMP) AS PayAmt
		...
		m_sql = miniTable.prepareTable(new ColumnInfo[] {
			...
			new ColumnInfo(Msg.getMsg(ctx, "DiscountDate"), "CURRENT_TIMESTAMP-paymentTermDueDays(i.C_PaymentTerm_ID,i.DateInvoiced,CURRENT_TIMESTAMP)", Timestamp.class),
			new ColumnInfo(Msg.getMsg(ctx, "AmountDue"), "currencyConvert(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),0,i.C_Currency_ID, ?,?,i.C_ConversionType_ID, i.AD_Client_ID,i.AD_Org_ID)", BigDecimal.class),
			new ColumnInfo(Msg.getMsg(ctx, "AmountPay"), "currencyConvert(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID)-paymentTermDiscount(i.GrandTotal,i.C_Currency_ID,i.C_PaymentTerm_ID,i.DateInvoiced, ?),0,i.C_Currency_ID, ?,?,i.C_ConversionType_ID, i.AD_Client_ID,i.AD_Org_ID)", BigDecimal.class)
			},
			//	FROM
			"C_Invoice_v i"
	...
	private void loadBankInfo()
	{
		...
		String sql = info.getQuery().substring(0, info.getQuery().indexOf(" ORDER BY"))
			+ " AND " + info.KeyColumn
			+ " IN (SELECT PaymentRule FROM C_BankAccountDoc WHERE C_BankAccount_ID=?) "
			+ info.getQuery().substring(info.getQuery().indexOf(" ORDER BY"));
		try
		{
			PreparedStatement pstmt = DB.prepareStatement(sql, null);
	...
	private void loadTableInfo()
	{
		...
		if (vp != null && REF__PaymentRule.DirectDebit.equals(vp.getValue()))
		{
			isSOTrx = "Y";
			sql += " AND i.PaymentRule='" + REF__PaymentRule.DirectDebit + "'";
		}
		//
