...
import org.xendra.apps.form.ChangePasswordDialog;
 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: ALogin.java 5489 2014-11-13 07:10:42Z xapiens $
 */
	...
	implements ActionListener, ChangeListener
{
	...
	public ALogin (Frame parent)
	{
		...
		parent.setIconImage(Xendra.getImage16());
	}   //  ALogin
...
//	private BorderLayout defaultLayout = new BorderLayout();
	...
	private void jbInit() throws Exception
	{
		...
		titleLabel.setToolTipText(Xendra.getURL());
		titleLabel.setHorizontalTextPosition(SwingConstants.CENTER);
		/** Removing/modifying the Xendra logo is a violation of the license	*/
		titleLabel.setIcon(Xendra.getImageIconLogo());
		titleLabel.setText(Xendra.getSubtitle());
		...
		compileDate.setText(Xendra.DATE_VERSION);
		compileDate.setToolTipText(Xendra.getImplementationVendor());
		...
		versionLabel.setText(Xendra.MAIN_VERSION);
		versionLabel.setToolTipText(Xendra.getImplementationVersion());
	...
	{
		m_cc = CConnection.get(Xendra.getCodeBaseHost());
	...
	private void connectionOK ()
	{
		...
		{
			// Change Password
			Boolean ChangePassword = Env.getContext(Env.getCtx(), "#IsChangePassword").equals("Y");
			if (ChangePassword)
			{
				ChangePasswordDialog cpdialog = new ChangePasswordDialog(null);
			}
	...
	private boolean checkTC() {
		MCurrency[] currencies = MCurrency.getMandatory();
		MConversionType[] cm = MConversionType.getMandatory(Env.getAD_Client_ID(Env.getCtx()));
		if (cm.length > 0 && currencies.length > 0 )
		{			
			HashMap<Integer, Vector<Integer>> processList  = new HashMap<Integer, Vector<Integer>>();
			Vector<Integer> conversionTypes ;
			
			MAcctSchema as = MAcctSchema.get(Env.getCtx(), Env.getContextAsInt(Env.getCtx(), "$C_AcctSchema_ID"));
			Timestamp ConvDate  = new Timestamp (System.currentTimeMillis());

			for (int x=0; x < currencies.length; x++)
			{
				MCurrency currency = currencies[x];
				conversionTypes = new Vector<Integer>();
				
				for (int y=0; y < cm.length; y++)
				{
					MConversionType ct = cm[y];
					
					if (currency.getC_Currency_ID() !=  as.getC_Currency_ID())
					{
						BigDecimal from = MConversionRate.convert(Env.getCtx(), 
								Env.ONE, currency.getC_Currency_ID() , 
								as.getC_Currency_ID(), ConvDate/*getDateAcct()*/, 
								ct.getC_ConversionType_ID(), Env.getAD_Client_ID(Env.getCtx()), Env.getAD_Org_ID(Env.getCtx()));
						BigDecimal to = MConversionRate.convert(Env.getCtx(), 
								Env.ONE, as.getC_Currency_ID(), 
								currency.getC_Currency_ID() , ConvDate/*getDateAcct()*/, 
								ct.getC_ConversionType_ID(), Env.getAD_Client_ID(Env.getCtx()), Env.getAD_Org_ID(Env.getCtx()));
						
						if (from == null || to == null)
						{
							if (!conversionTypes.contains(ct.getC_ConversionType_ID())) {
								conversionTypes.add(ct.getC_ConversionType_ID());
							}
						}
					}
				}
				// Pngo los tipos de conversion asociandolos a una moneda.
				if (conversionTypes.size() > 0) {
					processList.put(currency.getC_Currency_ID(), conversionTypes);				
				}
			}			
			setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
			if ( processList.size() > 0)
			{
				CurrencyExchangeDialog cedialog = new CurrencyExchangeDialog(null, processList, ConvDate, Ini.isPropertyBool(Ini.P_SHOW_ACCT));
				return cedialog.isOK();
			}
			else
				return true;
		}
	...
	private boolean checkTC2() {
		...
		{			
			...
			Vector<Integer> conversionTypes ;
			HashMap<Integer, Vector<Integer>> processList  = new HashMap<Integer, Vector<Integer>>();
			Vector<Integer> monedas = new Vector<Integer>();
			
			MAcctSchema as = MAcctSchema.get(Env.getCtx(), Env.getContextAsInt(Env.getCtx(), "$C_AcctSchema_ID"));
			Timestamp ConvDate  = new Timestamp (System.currentTimeMillis());
			boolean acceptConversionType = false;
			for (int x=0; x < cm.length; x++)
			{
				acceptConversionType = false;
				MConversionType ct = cm[x];
				monedas = new Vector<Integer>();
				conversionTypes = new Vector<Integer>();;
				
				for (int y=0; y < currencies.length; y++)
				{
					MCurrency currency = currencies[y];
					...
					if (currency.getC_Currency_ID() !=  as.getC_Currency_ID())
					{
						...
						if (from == null || to == null)
						{
							acceptConversionType = true;
							if (!monedas.contains(currency.getC_Currency_ID())) {
								monedas.add(currency.getC_Currency_ID());			
							}
							
							// PARCHE ARANA: Solo agregarla si no existe ya.
							if (!coins.contains(currency.getC_Currency_ID())) {
								coins.add(currency.getC_Currency_ID());			
							}
						}
					}
				}
				if (acceptConversionType == true) {
					conversionTypes.add(ct.getC_ConversionType_ID());				
					processList.put(ct.getC_ConversionType_ID(), monedas);				
				}
			}			
			setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
			if ( coins.size() > 0)
	...
	private boolean tryConnection()
	{
		...
		if (!DB.isConnected(false))
		{
			...
			hostField.setBackground(XendraPLAF.getFieldBackground_Error());
			return false;
		}
		//	Reference check
		Ini.setProperty(Ini.P_XENDRASYS, "Reference".equalsIgnoreCase(CConnection.get().getDbUid()));
		...
		if (roles == null || roles.length == 0)
		{
			...
			userTextField.setBackground(XendraPLAF.getFieldBackground_Error());
			passwordField.setBackground(XendraPLAF.getFieldBackground_Error());
			return false;
		}
		...
		userTextField.setBackground(XendraPLAF.getFieldBackground_Normal());
		passwordField.setBackground(XendraPLAF.getFieldBackground_Normal());
		//
	...
	private boolean checkVersion()
	{
		...
		try
		{
			...
			if (Xendra.DATE_VERSION.equals(version))
			{
			...
			else if (version != null)
			{
				...
				msg.append(version == null ? "null" : version).append(" <> ")
					.append(Xendra.DATE_VERSION).append("\n");
				msg.append(res.getString("PleaseUpgrade")).append("\n<<");
				JOptionPane.showMessageDialog(null, msg.toString(),
					Xendra.getName() + " - " + res.getString("VersionConflict"),
