...
/**
 *  Class to Create a new Xendra Database from a reference DB.
 *  <pre>
 *  - Create User
 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: CreateXendra.java 5518 2015-03-18 00:12:50Z xapiens $
 */
public class CreateXendra
{
	/**
	 * 	Constructor
	 *	@param databaseType XendraDatabase.TYPE_
	 *	@param databaseHost database host
	 *	@param databasePort database port 0 for default
	 *	@param systemPassword system password
	 */
	public CreateXendra(String databaseType, String databaseHost, int databasePort,
	...
	/** Xendra Target Database */
	private XendraDatabase 	m_dbTarget = null;
	/** Xendra Source Database */
	private XendraDatabase 	m_dbSource = null;
	//
	private String				m_databaseHost = null;
	private int					m_databasePort = 0;
	private String 				m_systemPassword = null;
	private String 				m_xendraUser = null;
	private String 				m_xendraPassword = null;
	...
	/**	Logger	*/
	private static CLogger	log	= CLogger.getCLogger (CreateXendra.class);
	
	/**
	 * 	Create Xendra Database
	...
	private void initDatabase(String databaseType)
	{
		try
		{
			m_dbTarget = DB_PostgreSQL.class.newInstance(); 
//			for (int i = 0; i < Database.DB_NAMES.length; i++)
//			{
//				if (Database.DB_NAMES[i].equals (databaseType))
//				{
//					m_dbTarget = (XendraDatabase)Database.DB_CLASSES[i].
//						   newInstance ();					
//					break;
//				}
//			}
	...
	/**
	 *  Set Xendra User
	 *  @param xendraUser xendra id
	 *  @param xendraPassword xendra password
	 */
	public void setXendraUser (String xendraUser, String xendraPassword)
	{
		m_xendraUser = xendraUser;
		m_xendraPassword = xendraPassword;
	}   //  setXendraUser
	...
	{
		String dbUrl = m_dbTarget.getConnectionURL (m_databaseHost, m_databasePort, m_databaseName);	//	adempiere may not be defined yet
	...
	{
		log.info(m_xendraUser + "/" + m_xendraPassword);
		return executeCommands(m_dbTarget.getCommands(XendraDatabase.CMD_CREATE_USER), 
			sysConn, true, false);
	}   //  createUser
	...
	public boolean createDatabase (Connection sysConn)
	{
		...
		return executeCommands(m_dbTarget.getCommands(XendraDatabase.CMD_CREATE_DATABASE), 
			sysConn, true, false);
	}   //  createDatabase	
	...
	public boolean dropDatabase (Connection sysConn)
	{
		...
		return executeCommands(m_dbTarget.getCommands(XendraDatabase.CMD_DROP_DATABASE), 
			sysConn, true, false);
	}   //  dropDatabase	
	...
	private Connection getConnection (boolean asSystem, boolean createNew)
	{
		if (!createNew && m_conn != null)
			return m_conn;
		//
		String dbUrl = m_dbTarget.getConnectionURL(m_databaseHost, m_databasePort, (asSystem ? m_dbTarget.getSystemDatabase() : m_databaseName));
		try
		{
			if (asSystem)
				m_conn = m_dbTarget.getDriverConnection(dbUrl, m_dbTarget.getSystemUser(), m_systemPassword);
			else
				m_conn = m_dbTarget.getDriverConnection(dbUrl, m_xendraUser, m_xendraPassword);
		}
		catch (Exception e)
		{
		...
		try
		{
			...
			for (int i = 0; i < cmds.length; i++)
			{
				...
				if (cmd.indexOf('@') != -1)
				{
					...
					cmd = Util.replace(cmd, "@AdempiereUser@", m_xendraUser);
					cmd = Util.replace(cmd, "@AdempierePassword@", m_xendraPassword);
					cmd = Util.replace(cmd, "@SystemPassword@", m_systemPassword);
	...
	public static void main (String[] args)
	{
		...
		//cc.setDatabaseName("adempiere", "adempiere");
                CreateXendra cc = new CreateXendra (Database.DB_POSTGRESQL, "127.0.0.2", 5432 , "adempiere");
		cc.setXendraUser("adempiere", "adempiere");
		cc.setDatabaseName("adempiere", "adempiere");
