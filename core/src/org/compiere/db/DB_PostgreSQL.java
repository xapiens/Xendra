 ...
 *  @version    $Id: DB_PostgreSQL.java 5629 2015-08-14 15:53:50Z xapiens $
 *  ---
 ...
 */
public class DB_PostgreSQL implements XendraDatabase
{
	...
	/** Backup Data Source */
	//private ComboPooledDataSource m_backupds = null;
	/** Statement Converter     */
	...
	public String getConnectionURL (CConnection connection)
	{
		...
		//sb.append(m_dbName);
		sb.append(":").append(connection.getDbPort());
		//sb.append("/").append(connection.getDbName())
		sb.append("/").append(m_dbName)
		//.append("?charSet=UTF-8");
		.append("?encoding=UNICODE");
		//.append("&currentschema=xendra");
	...
	{
		//m_userName = userName;
		m_connectionURL = "jdbc:postgresql://" + dbHost + ":" + dbPort + "/" + dbName + "&currentschema=xendra";
	...
	public String convertStatement (String oraStatement)
	{
		...
		//System.out.println(oraStatement);	
		return oraStatement;
...
//    	Convert.logMigrationScript(oraStatement, retValue[0]);
	...
	public String TO_DATE (Timestamp time, boolean dayOnly)
	{
		...
		StringBuffer dateString = new StringBuffer("TO_TIMESTAMP('");
	...
	public String[] getCommands (int cmdType)
	{
		...
		if (CMD_CREATE_DATABASE == cmdType)
			return new String[]
			{
		    "CREATE DATABASE xendra OWNER xendra;",
			"GRANT ALL PRIVILEGES ON xendra TO xendra;"	,
			"CREATE SCHEMA xendra;",
	...
	{
//		return getDataSource(false, connection);
//	}
//	/**
...
//	public DataSource getDataSource(CConnection connection)
//	{
		//if (m_ds != null && !IsBackup)
		if (m_ds != null)
			return m_ds;
		//else if (m_backupds != null && IsBackup)
		//	return m_backupds;
        ...
        try
        {
            ...
            //
            // en webstart tengo problemas
            //            
            cpds.setConnectionCustomizerClassName(null);
            cpds.setDataSourceName("XendraDS");
            ...
            cpds.setIdleConnectionTestPeriod(1200); 	// cada que tiempo va a chequear el estado de la conexion, en segundos             
            cpds.setTestConnectionOnCheckin(false);		//  http://www.mchange.com/projects/c3p0/#testConnectionOnCheckin 
            cpds.setTestConnectionOnCheckout(false); 	//
            cpds.setAcquireRetryAttempts(4);			// cuantas veces reintentara conectarse hasta rendirse en caso no tenga conexion
            ...
            if (Ini.isClient())
            {
    			...
    			//java.util.regex.Matcher m = isInteger.matcher(connection.getPoolClient());    			
//    			Integer poolclient = 20;
//    			if (m.find())
//    			{
//    				poolclient = Integer.valueOf(connection.getPoolClient());
//    				if (poolclient == 0)
//    					poolclient = 10;
//    			}
    			Integer poolclient = Env.getPoolClient();
                //String spoolclient = connection.getPoolClient();
                //if (spoolclient.length() == 0)
                //Integer poolclient = Integer.valueOf(spoolclient);
                cpds.setMaxPoolSize(poolclient);
//                cpds.setMaxIdleTimeExcessConnections(900);
                //cpds.setMaxIdleTime(1200);
                //cpds.setMaxAdministrativeTaskTime(maxAdministrativeTaskTime)
                // captura  de putos errores.
                cpds.setDebugUnreturnedConnectionStackTraces(true);
                cpds.setMaxIdleTime(100);
                // https://forum.hibernate.org/viewtopic.php?t=934779
				//When a c3p0 DataSource attempts and fails to acquire a Connection, 
				//it will retry up to acquireRetryAttempts times, with a delay of acquireRetryDelay 
				//between each attempt. If all attempts fail, any clients waiting for Connections from 
				//the DataSource will see an Exception, indicating that a Connection could not be 
				//acquired. Note that clients do not see any Exception until a full round of attempts 
				//fail, which may be some time after the initial Connection attempt. If 
				...
				//after a failed round of Connection attempts, and future client requests will fail immediately.                 
                //cpds.setAcquireRetryAttempts(30);
                cpds.setAcquireRetryDelay(1000);
                cpds.setBreakAfterAcquireFailure(true);
                /* 
                 * Si este parametro se setea a menos valor, existe la posibilidad que falle la conexion al asumir que la demora en conexion
                 * especialmente al ser la primera conexion, lo cual por obvias razones sera la mas lenta, creyendo que hay un error y no
                 * dejando conectar
                 * 
                 * */
                cpds.setCheckoutTimeout(1000);
                //cpds.setUnreturnedConnectionTimeout(4);
                //cpds.setAutoCommitOnClose(true);
                //cpds.setStatementCacheNumDeferredCloseThreads(1);
                //m_maxbusyconnections = 10;
                // http://stackoverflow.com/questions/3730844/c3p0-apparent-deadlock-when-the-threads-are-all-empty
//                cpds.setMaxStatements(1000);
//                cpds.setMaxStatementsPerConnection(100);
                m_maxbusyconnections = poolclient;
            }
            else
            {
                cpds.setInitialPoolSize(10);
                cpds.setMinPoolSize(5);
                //Integer poolServer = Integer.valueOf(connection.getPoolServer());
                //Integer poolServer = 200;
                Integer poolserver = Env.getPoolServer();
                //if (poolServer == 0)
                //	poolServer = 50;
                //cpds.setMaxPoolSize(50);
                cpds.setMaxPoolSize(poolserver);
                cpds.setMaxIdleTimeExcessConnections(900);
                cpds.setMaxIdleTime(1200);
                cpds.setAcquireRetryAttempts(30);
                cpds.setAcquireRetryDelay(1000);                
                ...
                m_maxbusyconnections = poolserver;
            }
...
//            System.setProperty("com.mchange.v2.log.MLog", "com.mchange.v2.log.FallbackMLog");
//            System.setProperty("com.mchange.v2.log.FallbackMLog.DEFAULT_CUTOFF_LEVEL", "FINE");
//            ComboPooledDataSource cpds = new ComboPooledDataSource();
//            cpds.setDataSourceName("XendraDS");
//            cpds.setDriverClass(DRIVER);
//            //loads the jdbc driver
...
//            cpds.setDebugUnreturnedConnectionStackTraces(true);
//            // mala idea activarlo en produccion
//            // solo debe ser activado para entornos de programacion.
//            //cpds.setUnreturnedConnectionTimeout(30);
//            // este es el que desactiva. ---
//            //cpds.setCheckoutTimeout(60);
//            if (Ini.isClient())
//            {
//                cpds.setInitialPoolSize(2);
...
//                //cpds.setMaxPoolSize(15);
//                //cpds.setMaxPoolSize(8);
//                Integer xx = Integer.valueOf(connection.getPoolClient());
//                xx = 12;
//                cpds.setMaxPoolSize(xx);
//                cpds.setMaxIdleTimeExcessConnections(900);
//                //cpds.setMaxIdleTime(1200);
//                //cpds.setMaxAdministrativeTaskTime(maxAdministrativeTaskTime)
//                // captura  de putos errores.
...
//                //cpds.setStatementCacheNumDeferredCloseThreads(1);
//                //m_maxbusyconnections = 10;
//                m_maxbusyconnections = xx;
//            }
//            else
//            {
//                cpds.setInitialPoolSize(10);
//                cpds.setMinPoolSize(5);
//                Integer xx = Integer.valueOf(connection.getPoolServer());
//                //cpds.setMaxPoolSize(50);
//                cpds.setMaxPoolSize(xx);
//                cpds.setMaxIdleTimeExcessConnections(900);
//                cpds.setMaxIdleTime(1200);
...
//            cpds.setDebugUnreturnedConnectionStackTraces(true);
            ...
            try 
            {
            	...
            	testStatement.executeQuery("SELECT 1+1");
            }
            catch (Exception e)
            {
            	closeconn = true;            	
...
//		m_backupds = null;		
	...
	public static void main(String[] args)
	{
		...
		//
		String databaseName = "xendra";
		String uid = "xendra";
		String pwd = "xendra";
		String jdbcURL = postgresql.getConnectionURL("vpj", DEFAULT_PORT, databaseName);
	...
	public int getNextID(String name, String trxname) {
		...
		int m_sequence_id = DB.getSQLValue(trxname, "SELECT nextval('"+name.toLowerCase()+"')");
