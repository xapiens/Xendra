...
public class Doc_MatchInv extends Doc
{
	...
	{
		super(ass, MMatchInv.class, rs, REF_C_DocTypeDocBaseType.MatchInvoice, trxName);
	}   //  Doc_MatchInv
	...
	//	dr.setLocationFromLocator(m_receiptLine.getM_Locator_ID(), false);		//  to Loc
		...
		MInOut receipt = m_receiptLine.getParent();
		if (receipt.getMovementType().equals(REF_M_TransactionMovementType.VendorReturns))
		...
		updateProductInfo(as.getC_AcctSchema_ID(), 
			REF_C_AcctSchemaCostingMethod.StandardCosting.equals(as.getCostingMethod()));
		//
		facts.add(fact);
	...
	private boolean updateProductInfo (int C_AcctSchema_ID, boolean standardCosting)
	{
		...
		StringBuffer sql = new StringBuffer (
			...
			+ "pc.CostStandardCumAmt + currencyConvert(il.PriceActual,0,i.C_Currency_ID,a.C_Currency_ID,i.DateInvoiced,i.C_ConversionType_ID,i.AD_Client_ID,i.AD_Org_ID)*m.Qty, "
			+ "pc.CostAverageCumQty + m.Qty,"
			+ "pc.CostAverageCumAmt + currencyConvert(il.PriceActual,0,i.C_Currency_ID,a.C_Currency_ID,i.DateInvoiced,i.C_ConversionType_ID,i.AD_Client_ID,i.AD_Org_ID)*m.Qty "
			+ "FROM M_MatchInv m"
