 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: Fact.java,v 1.2 2007/06/19 20:42:42 sergioaguayo Exp $
 */
public final class Fact
{
	...
	/** Fact ID 			*/
	private int				m_Fact_ID = -1;

	/** Actual Balance Type */
	public static final String	POST_Actual = REF__PostingType.Actual;
	/** Budget Balance Type */
	public static final String	POST_Budget = REF__PostingType.Budget;
	/** Encumbrance Posting */
	public static final String	POST_Commitment = REF__PostingType.Commitment;
	/** Encumbrance Posting */
	public static final String	POST_Reservation = REF__PostingType.Reservation;
	...
	//	log.fine("createLine - " + account	+ " - Dr=" + debitAmt + ", Cr=" + creditAmt);
		...
		if (m_doc.getC_DocType_ID() > 0)
		{
			if (account.getAccount().isDocTypeAccount())
			{
				line.setC_DocType_ID(m_doc.getC_DocType_ID());
			}
		}
	...
	public FactLine createLineExtended (DocLine docLine, MAccount account,
			int C_Currency_ID, BigDecimal debitAmt, BigDecimal creditAmt)
	{
		return createLineExtended (docLine, account, C_Currency_ID, debitAmt, creditAmt, null, null, null);
	}
	
	public FactLine createLineExtended (DocLine docLine, MAccount account,
			int C_Currency_ID, BigDecimal debitAmt, BigDecimal creditAmt, Timestamp dateAcct, Timestamp dateDoc, Timestamp dateAcctPay)
		{
			if (account == null)
			{
				log.info("No account for " + docLine 
			...
			if (dateAcctPay != null)
				...
				line.setDocumentInfo(m_doc, docLine, dateAcct, dateDoc );
			line.setPostingType(m_postingType);
			line.setAccount(m_acctSchema, account);
	...
	/** 
	 *  Search y return for criteria
	 */
	public FactLine Search(MAccount acct)
	{
		for (int i = 0; i < m_lines.size(); i++)
		{
			FactLine line = (FactLine)m_lines.get(i);
			if ( line.getAccount().equals(acct) )
				return line;
		}
		return null;
	}		

	public FactLine Search(MAccount acct, Integer C_BPartner_ID)
	{
		for (int i = 0; i < m_lines.size(); i++)
		{
			FactLine line = (FactLine)m_lines.get(i);
			if ( line.getAccount().equals(acct) &&
				 line.getC_BPartner_ID() == C_BPartner_ID )
				return line;
		}
		return null;
	}		
	 ...
	 *  Search y return for criteria
	 */
	public FactLine Search(MAccount acct, Integer C_BPartner_ID, String m_DocumentNo)
	{
		for (int i = 0; i < m_lines.size(); i++)
		{
			FactLine line = (FactLine)m_lines.get(i);
			String documentno = "";
			if (line.getInvoiceDocumentNo() != null)
			{
				String doc = line.getInvoiceDocumentNo();
				Pattern p = Pattern.compile("\\d+");
				java.util.regex.Matcher m = p.matcher(doc);
				
				if (m.find())
				{
					documentno = line.getInvoiceDocumentNo();	
				}
			}			
			if ( line.getAccount().equals(acct) &&
				 line.getC_BPartner_ID() == C_BPartner_ID &&
				 documentno.equals(m_DocumentNo))
				return line;
		}
		return null;
	}	
	 ...
	 *  Search y return for criteria
	 */
	//public FactLine Search(MAccount acct, Integer Ref_Acct_ID, String m_serial, String m_DocumentNo)
	public FactLine Search(MAccount acct, String m_serial, String m_DocumentNo)
	{
		if (m_serial == null)
			m_serial = "";
		String serial = "";
		...
		for (int i = 0; i < m_lines.size(); i++)
		{
			...
			//&& line.getInvoiceDocumentNo() 
			//documentno = line.getInvoiceDocumentNo() == null ? "" : line.getInvoiceDocumentNo();	
			if ( line.getAccount().equals(acct) &&
				 //line.getRef_Acct_ID() == Ref_Acct_ID && 
				 serial.equals(m_serial) &&
				 documentno.equals(m_DocumentNo) )
				return line;
		}
		return null;
	...
	public FactLine Search(String m_DocumentNo)
	{
		String documentno = "";
		for (int i = 0; i < m_lines.size(); i++)
		{
			...
			if (line.getInvoiceDocumentNo() != null)
			{
				if ( documentno.equals(m_DocumentNo) )
					return line;
			}
		}
		return null;
...
//		line.setPostingType(m_postingType);
		...
		//  Amount
		MAccount gain = MAccount.get (m_acctSchema.getCtx(), m_acctSchema.getAcctSchemaDefault().getRealizedGain_Acct());
		MAccount loss = MAccount.get (m_acctSchema.getCtx(), m_acctSchema.getAcctSchemaDefault().getRealizedLoss_Acct());
		
		if (diff.signum() < 0)   //  negative balance => DR
		{
//			line.setAccount(m_acctSchema, loss);
			//line.setAmtSource(m_doc.getC_Currency_ID(), diff.abs(), Env.ZERO);
//			line.setAmtSource(m_acctSchema.getC_Currency_ID(), diff.abs(), Env.ZERO);
			
			FactLine fl = null;
			
			fl = Search("ADJUST");
			
			fl = m_doc.createFact(m_acctSchema, 
					fl, 
					this, 
					loss , 
					m_doc.getC_BPartner_ID() , 
					m_acctSchema.getC_Currency_ID() , 
					m_doc.getC_ConversionType_ID(),
					m_doc.getDateAcct(),
					m_doc.getDateDoc(),
					null,
					diff.abs(),
		...
		{
//			line.setAccount(m_acctSchema, gain);
			//line.setAmtSource(m_doc.getC_Currency_ID(), Env.ZERO, diff);
//			line.setAmtSource(m_acctSchema.getC_Currency_ID(), Env.ZERO, diff);
...
//		return line;
	...
	{
		if (segmentType.equals(REF_C_AcctSchemaElementType.Organization))
		{
	...
	public FactLine balanceAccounting()
	{
		...
		if (m_acctSchema.isCurrencyBalancing())
		{
			...
			// sergioaguayo (10/18/2007): Uses 2 different accounts for debit and credit differences
			if (isDR) {
				line.setAccount (m_acctSchema, m_acctSchema.getCurrencyBalancingLoss_Acct());
				drAmt = difference;
			}
			else {
				line.setAccount (m_acctSchema, m_acctSchema.getCurrencyBalancingIncome_Acct());
				crAmt = difference;
			}
	...
	public boolean distribute()
	{
		...
		for (int i = 0; i < m_lines.size(); i++)
		{
			...
			MDistribution[] distributions = MDistribution.get (dLine.getAccount(), 
				m_postingType, m_doc.getC_DocType_ID(), dLine.isCloseDocument());
			...
			// If a linked distribution, then should not create the reversal
			if (!distribution.isLink()) {
				//	Add Reversal
			...
			distribution.distribute(dLine.getAccount(), dLine.getSourceBalance(), m_acctSchema.getC_Currency_ID());
			...
			for (int j = 0; j < lines.length; j++)
			{
                ...
                if (REF_C_ElementValueAccountSign.Credit.equals(dl.getAccountSign())) {
                	if (dl.getAmt().signum()>=0)
                		//factLine.setAmtSource(dLine.getC_Currency_ID(), null, dl.getAmt());
                		factLine.setAmtSource(m_acctSchema.getC_Currency_ID(), null, dl.getAmt());
                	else
                		//factLine.setAmtSource(dLine.getC_Currency_ID(), dl.getAmt().abs(), null);
                		factLine.setAmtSource(m_acctSchema.getC_Currency_ID(), dl.getAmt().abs(), null);
                }
                	...
                	if (dl.getAmt().signum()>=0)
                		...
                		factLine.setAmtSource(m_acctSchema.getC_Currency_ID(), dl.getAmt(), null);
                	else
                		//factLine.setAmtSource(dLine.getC_Currency_ID(), null, dl.getAmt().abs());
                		factLine.setAmtSource(m_acctSchema.getC_Currency_ID(), null, dl.getAmt().abs());
                }
                else {
                	if (dl.getAmt().signum() < 0) {
                		//factLine.setAmtSource(dLine.getC_Currency_ID(), null, dl.getAmt().abs());
	...
	public FactLine Search(MAccount acct, Integer C_BPartner_ID,  String m_serial, String m_DocumentNo)
	{		
		String serial =  m_serial == null ? "" : m_serial;
		String documentno = "";
		for (int i = 0; i < m_lines.size(); i++)
		{
			FactLine line = (FactLine)m_lines.get(i);
	...
	public FactLine Search(int cCurrencyID, MAccount acct) {
		for (int i = 0; i < m_lines.size(); i++)
		{
			FactLine line = (FactLine)m_lines.get(i);
			if ( line.getAccount().equals(acct) &&
				 line.getC_Currency_ID() == cCurrencyID )
				return line;
		}
		return null;
	}
