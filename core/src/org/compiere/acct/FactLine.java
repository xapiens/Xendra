 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: FactLine.java,v 1.1 2007/06/15 00:03:25 sergioaguayo Exp $
 *  
 *  Contributor(s):
...
public final class FactLine extends X_Fact_Acct
{
	...
	public void setAccount (MAcctSchema acctSchema, MAccount acct)
	{
		...
		MAcctSchemaElement ud1 = m_acctSchema.getAcctSchemaElement(
				REF_C_AcctSchemaElementType.UserElement1);
		if (ud1 != null)
		{
		...
		MAcctSchemaElement ud2 = m_acctSchema.getAcctSchemaElement(
				REF_C_AcctSchemaElementType.UserElement2);
		if (ud2 != null)
		{
	...
	public boolean setAmtSource (int C_Currency_ID, BigDecimal AmtSourceDr, BigDecimal AmtSourceCr)
	{
		...
		if (AmtSourceDr != null && AmtSourceDr.scale() > precision)
		{
			...
			log.info("Source DR Precision " + AmtSourceDr + " -> " + AmtSourceDr1);			
		...
		if (AmtSourceCr != null && AmtSourceCr.scale() > precision)
		{
			...
			log.info("Source CR Precision " + AmtSourceCr + " -> " + AmtSourceCr1);
	...
	{
		setDocumentInfo(doc, docLine, null, null);
	}
	/**
	 *  Set Document Info
	 *  @param doc document
	 ...
	 */
	public void setDocumentInfo(Doc doc, DocLine docLine, Timestamp dateAcctx, Timestamp dateDoc)
	{
		m_doc = doc;
		m_docLine = docLine;
		//	reset
		...
		if (m_docLine != null && m_docLine.getDateDoc() != null && getDateTrx() == null)
			setDateTrx (m_docLine.getDateDoc());
		//	Date Acct
		if (dateAcctx != null)
		{
			setDateAcct (dateAcctx);
		}
		else
		{
...
//		return getAmtSourceDr().subtract(getAmtSourceCr());
	...
	public boolean convert ()
	{
		...
		setAmtAcctDr (MConversionRate.convert (getCtx(), getAmtSourceDr(), getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
		 	datedoc, C_ConversionType_ID, m_doc.getAD_Client_ID(), AD_Org_ID));
		if (getAmtAcctDr() == null)
			return false;
		System.out.print(datedoc+" "+C_ConversionType_ID+" AmtAcctDr:"+getAmtAcctDr());
		...
		setAmtAcctCr (MConversionRate.convert (getCtx(), getAmtSourceCr(), getC_Currency_ID(), m_acctSchema.getC_Currency_ID(),
			datedoc, C_ConversionType_ID, m_doc.getAD_Client_ID(), AD_Org_ID));
		System.out.print(datedoc+" "+C_ConversionType_ID+" AmtAcctCr:"+getAmtAcctCr());
		return true;
	}	//	convert
	...
	public String toString()
	{
		...
		sb.append(getAD_Table_ID()).append(":").append(getRecord_ID())
			...
			.append(", Ref=").append(getRef_Acct_ID())
			.append(", Document=").append(this.getInvoiceDocumentNo())
			.append("]");
		return sb.toString();
	}	//	toString
	...
	public int getAD_Org_ID()
	{
		...
		{
			if (REF_C_DocTypeDocBaseType.GLJournal.equals (m_doc.getDocumentType()))
			{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		if (newRecord)
		{
			log.fine(toString());
			//
			if (m_doc.getFact_ID().length() != 0)
				setFact_ID(m_doc.getFact_ID());
			getAD_Org_ID();
			getC_SalesRegion_ID();
			...
			if (m_doc.getDocumentType().equals(REF_C_DocTypeDocBaseType.ARInvoice) 
				&& m_docLine != null 
	...
	public void setDocLine(DocLine docLine) {
		m_docLine = docLine;
	}
