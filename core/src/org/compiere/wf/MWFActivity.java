 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MWFActivity.java 5822 2016-06-15 22:56:06Z xapiens $
 */
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
	...
	public MWFActivity (MWFProcess process, int AD_WF_Node_ID)
	{
		...
		//	Status
		super.setWFState(REF_WF_InstanceState.NotStarted);
		m_state = new StateEngine (getWFState());
		setProcessed (false);
		//	Set Workflow Node
	...
	//	log.fine("");
		...
		if (m_state.isClosed())
		{
			m_audit.setEventType(REF_WF_EventType.ProcessCompleted);
			long ms = System.currentTimeMillis() - m_audit.getCreated().getTime();
			m_audit.setElapsedTimeMS(new BigDecimal(ms));
		}
		else
			m_audit.setEventType(REF_WF_EventType.StateChanged);
		m_audit.save();
	}	//	updateEventAudit
	...
	public void setAD_WF_Node_ID (int AD_WF_Node_ID)
	{
		...
		//
		if (!REF_WF_InstanceState.NotStarted.equals(getWFState()))
		{
			super.setWFState(REF_WF_InstanceState.NotStarted);
			m_state = new StateEngine (getWFState());
		}
	...
	{
		return MRefList.getListName(getCtx(), REF_WF_InstanceState.Identifier, getWFState());
	}	//	getWFStateText
	...
	private boolean performWork (Trx trx) throws Exception
	{
		...
		/******	Sleep (Start/End)			******/
		if (REF_WF_Action.WaitSleep.equals(action))
		{
		...
		else if (REF_WF_Action.DocumentAction.equals(action))
		{
			...
			if (m_po instanceof DocAction)
			{
				...
				//	Post Immediate
				if (success && DocAction.STATUS_Completed.equals(doc.getDocStatus()) 
				...
				else if ( success && DocAction.STATUS_Voided.equals(doc.getDocStatus()))
				{
					MClient client = MClient.get(doc.getCtx(), doc.getAD_Client_ID());
					if (client.isPostImmediate() || MClient.isClientAccountingImmediate())
						m_postImmediate = doc;					
			...
			{
				success = false;
				processMsg = CLogger.retrieveErrorString(null);
				m_process.setProcessMsg(processMsg);
			}
			if (!success)
			{
		...
		/******	Process						******/
		else if (REF_WF_Action.AppsProcess.equals(action))
		{
		...
		*/
		...
		/******	EMail						******/
		else if (REF_WF_Action.EMail.equals(action))
		{
		...
		else if (REF_WF_Action.UserChoice.equals(action))
		{
				...
				&& getPO(trx) instanceof DocAction)
			{
				...
				else	//	fixed Approver
				{
					...
					{
						throw new XendraException("Support not implemented for "+resp);
					}
					else
					{
						throw new XendraException("@NotSupported@ "+resp);
					}
					// end MZ
				}
				if (autoApproval
	...
	{
		if (m_postImmediate.getDocStatus().equals(DocAction.STATUS_Completed) && MFactAcct.alreadyPosted(m_postImmediate.get_Table_ID(), m_postImmediate.get_ID(), m_postImmediate.get_TrxName()))
			return;
	...
	private void sendEMail()
	{
		...
		else if (recipient.equals(REF_AD_WF_NodeEMailRecipient.DocumentBusinessPartner))
		{
		...
		else if (recipient.equals(REF_AD_WF_NodeEMailRecipient.DocumentOwner))
			sendEMail(client, doc.getDoc_User_ID(), null, subject, message, pdf, text.isHtml()); 
		else if (recipient.equals(REF_AD_WF_NodeEMailRecipient.WFResponsible))
		{
			MWFResponsible resp = getResponsible();
			if (resp.isInvoker())
				sendEMail(client, doc.getDoc_User_ID(), null, subject, message, pdf, text.isHtml()); 
