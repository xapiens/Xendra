...
import org.compiere.model.persistence.X_AD_WF_Process;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MWFProcess.java 5583 2015-08-05 14:11:58Z xapiens $
 */
...
//TODO  m_pi = pi; red1 - never used  -check later	
		...
		if (getPO() == null)
		{
			...
			super.setWFState (REF_WF_InstanceState.Terminated);
		}
		else
			setTextMsg(getPO());
		//	Responsible/User
 ...
 */
	...
	public void checkActivities(String trxName, PO lastPO)
	{
		...
		for (int i = 0; i < activities.length; i++)
		{
			...
			if (activityState.isClosed())
			{
				...
				else if (!closedState.equals(activityState))
				{
					...
					if (REF_WF_InstanceState.Terminated.equals(activityState))
						closedState = activityWFState;
					//	Overwrite if activity aborted and no other terminated
					else if (REF_WF_InstanceState.Aborted.equals(activityState) && !REF_WF_InstanceState.Terminated.equals(closedState))
						closedState = activityWFState;
				}
			}
			else	//	not closed
		...
		if (activities.length == 0)
		{
			...
			closedState = REF_WF_InstanceState.Terminated;
		}
		if (closedState != null)
		{
			...
			setWFState(REF_WF_InstanceState.Suspended);
		else if (running)
			setWFState(REF_WF_InstanceState.Running);
	}	//	checkActivities
	...
	private boolean startNext (MWFActivity last, MWFActivity[] activities, PO lastPO, String trxName)
	{
		...
		if (REF_WF_Join_Split.AND.equals(last.getNode().getJoinElement()))
		{
			//	get previous nodes
		...
		for (int i = 0; i < transitions.length; i++)
		{
			...
			if (REF_WF_Join_Split.XOR.equals(split))
				return true;
		}	//	for all transitions
		return true;
	}	//	startNext
