 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: MWorkflow.java 5583 2015-08-05 14:11:58Z xapiens $
 * 
...
public class MWorkflow extends X_AD_Workflow
{
	...
	)
	{
		...
		if (s_cacheDocValue.isReset())
		{
			...
			List<MWorkflow> workflows = new Query(ctx, Table_Name, whereClause, trxName)
				.setParameters(new Object[]{REF_AD_WorkflowType.DocumentValue, true})
	...
	public MWorkflow (Properties ctx, int AD_Workflow_ID, String trxName)
	{
		...
		//	setValue (null);
		//	setName (null);
			setAccessLevel (REF_AD_TableAccessLevels.Organization);
			setAuthor ("ComPiere, Inc.");
			setDurationUnit(REF_WF_DurationUnit.Day);
			setDuration (1);
			setEntityType (ENTITYTYPE_UserMaintained);	// U
			setIsDefault (false);
			setPublishStatus (REF__PublishStatus.UnderRevision);	// U
			setVersion (0);
			//setCost (Env.ZERO);
			setWaitingTime (0);
			setWorkingTime (0);
...
//			}
	...
	public long getDurationBaseSec ()
	{
		if (getDurationUnit() == null)
			return 0;
		else if (REF_WF_DurationUnit.Second.equals(getDurationUnit()))
			return 1;
		else if (REF_WF_DurationUnit.Minute.equals(getDurationUnit()))
			return 60;
		else if (REF_WF_DurationUnit.Hour.equals(getDurationUnit()))
			return 3600;
		else if (REF_WF_DurationUnit.Day.equals(getDurationUnit()))
			return 86400;
		else if (REF_WF_DurationUnit.Month.equals(getDurationUnit()))
			return 2592000;
		else if (REF_WF_DurationUnit.Year.equals(getDurationUnit()))
			return 31536000;
		return 0;
	}	//	getDurationBaseSec
	...
	public String validate()
	{
		...
		//
		if (REF_AD_WorkflowType.DocumentValue.equals(getWorkflowType()) 
...
//		}
	...
	public static void main (String[] args)
	{
		...
		node10.setAction(REF_WF_Action.WaitSleep);
		...
		node20.setAction(REF_WF_Action.DocumentAction);
		node20.setDocAction(REF__DocumentAction.None);
		...
		node100.setAction(REF_WF_Action.DocumentAction);
		node100.setDocAction(REF__DocumentAction.Prepare);
		...
		node200.setAction(REF_WF_Action.DocumentAction);
		node200.setDocAction(REF__DocumentAction.Complete);
		...
		req.setDocStatus(REF__DocumentStatus.Drafted);
		req.save();
		Log.setTraceLevel(8);
		System.out.println("---------------------------------------------------");
