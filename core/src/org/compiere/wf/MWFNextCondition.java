...
import org.compiere.model.persistence.X_AD_WF_NextCondition;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MWFNextCondition.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MWFNextCondition extends X_AD_WF_NextCondition
{
	...
	public boolean isOr()
	{
		return REF_AD_FindAndOr.Or.equals(getAndOr());
	}	//	isOr
	...
	public boolean evaluate (MWFActivity activity)
	{
		...
		if (getOperation().equals(REF_AD_FindOperation.Sql))
			throw new IllegalArgumentException("SQL Operator not implemented yet: " + resultStr);
		if (getOperation().equals(REF_AD_FindOperation.X))
			resultStr += "{" + value2 + "}";
	...
	private boolean compareNumber (Number valueObj, String value1, String value2)
	{
		...
		if (REF_AD_FindOperation.Eq.equals(op))
			return valueObjB.compareTo(value1B) == 0;
		else if (REF_AD_FindOperation.Gt.equals(op))
			return valueObjB.compareTo(value1B) > 0;
		else if (REF_AD_FindOperation.GtEq.equals(op))
			return valueObjB.compareTo(value1B) >= 0;
		else if (REF_AD_FindOperation.Le.equals(op))
			return valueObjB.compareTo(value1B) < 0;
		else if (REF_AD_FindOperation.LeEq.equals(op))
			return valueObjB.compareTo(value1B) <= 0;
		else if (REF_AD_FindOperation.Like.equals(op))
			return valueObjB.compareTo(value1B) == 0;
		else if (REF_AD_FindOperation.NotEq.equals(op))
			return valueObjB.compareTo(value1B) != 0;
		//
		else if (REF_AD_FindOperation.Sql.equals(op))
			throw new IllegalArgumentException("SQL not Implemented");
		//
		else if (REF_AD_FindOperation.X.equals(op))
		{
