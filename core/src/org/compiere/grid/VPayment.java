 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: VPayment.java 4091 2012-04-10 20:33:02Z xapiens $
 */
	...
	implements ActionListener
{
	...
	//		return false;
		...
		{			
			...
			if (REF__PaymentRule.DirectDebit.equals(PaymentRule)			//	SO
				&& !m_isSOTrx)
				continue;
			else if (REF__PaymentRule.DirectDeposit.equals(PaymentRule)	//	PO 
				&& m_isSOTrx)
				continue;
		 ...
		 * 	Load Accounts
		 */
		SQL = "SELECT a.C_BP_BankAccount_ID, COALESCE(b.Name, ' ')||a.AccountNo AS Acct "
	...
	//	log.fine( "VPayment.actionPerformed - " + e.getActionCommand());
		...
		else if (e.getSource() == paymentCombo)
		{
			...
			if (pp != null)
			{
				...
				if (REF__PaymentRule.DirectDebit.equalsIgnoreCase(s))
					s = REF__PaymentRule.DirectDeposit.toLowerCase();
				s += "Panel";	
	...
	private boolean saveChanges()
	{
		...
		//	B (Cash)		(Currency)
		if (newPaymentRule.equals(REF__PaymentRule.Cash))
		{
		...
		//	K (CreditCard)  Type, Number, Exp, Approval
		else if (newPaymentRule.equals(REF__PaymentRule.CreditCard))
		{
		...
		else if (newPaymentRule.equals(REF__PaymentRule.DirectDeposit) 
			|| newPaymentRule.equals(REF__PaymentRule.DirectDebit) )
		{
			tAccountCombo.getSelectedItem();
		}

		//	P (PaymentTerm)	PaymentTerm
		else if (newPaymentRule.equals(REF__PaymentRule.OnCredit))
		{
		...
		//	S (Check)		(Currency) CheckNo, Routing
		else if (newPaymentRule.equals(REF__PaymentRule.Check))
		{
		...
		{
			String tender = REF_C_PaymentTenderType.CreditCard;
			if (newPaymentRule.equals(REF__PaymentRule.DirectDeposit))
				tender = REF_C_PaymentTenderType.DirectDeposit;
			else if (newPaymentRule.equals(REF__PaymentRule.DirectDebit))
				tender = REF_C_PaymentTenderType.DirectDebit;
			else if (newPaymentRule.equals(REF__PaymentRule.Check))
				tender = REF_C_PaymentTenderType.Check;
		}
		...
		if (!newPaymentRule.equals(m_PaymentRule))
		{
			...
			if (m_PaymentRule.equals(REF__PaymentRule.Cash))
			{
		...
		if ("KS".indexOf(newPaymentRule) != -1)
		{
			...
			if (newPaymentRule.equals(REF__PaymentRule.CreditCard))
			{
				m_mPayment.setCreditCard(REF_C_PaymentTrxType.Sales, newCCType,
			...
			//  Save/Post
			if (REF__DocumentStatus.Drafted.equals(m_mPayment.getDocStatus()))
			{
		...
		if (!newPaymentRule.equals(m_PaymentRule))
			m_mTab.setValue("PaymentRule", newPaymentRule);
		// sergioaguayo (10/18/2007): don't modify the document's date, because it's closed and shouldn't be touched
	...
	//	log.config( "VPayment.checkMandatory");
		...
		boolean dataOK = true;
		//	B (Cash)		(Currency)
		if (PaymentRule.equals(REF__PaymentRule.Cash))
		{
			KeyNamePair kp = (KeyNamePair)bCashBookCombo.getSelectedItem();
			if (kp != null)
		...
		//	K (CreditCard)  Type, Number, Exp, Approval
		else if (PaymentRule.equals(REF__PaymentRule.CreditCard))
		{
			vp = (ValueNamePair)kTypeCombo.getSelectedItem();
			if (vp != null)
				CCType = vp.getValue();
			...
			if (error.length() != 0)
			{
				kNumberField.setBackground(XendraPLAF.getFieldBackground_Error());
				if (error.indexOf('?') == -1)
				{
			...
			if(error.length() != 0)
			{
				kExpField.setBackground(XendraPLAF.getFieldBackground_Error());
				ADialog.error(m_WindowNo, this, error);
				dataOK = false;
			}
		}
		...
		//	T (Transfer)	BPartner_Bank
		else if (PaymentRule.equals(REF__PaymentRule.DirectDeposit)
			|| PaymentRule.equals(REF__PaymentRule.DirectDebit))
		{
			KeyNamePair bpba = (KeyNamePair)tAccountCombo.getSelectedItem();
			if (bpba == null)

			{
				tAccountCombo.setBackground(XendraPLAF.getFieldBackground_Error());
		...
		//	P (PaymentTerm)	PaymentTerm
		else if (PaymentRule.equals(REF__PaymentRule.OnCredit))
		{
			KeyNamePair kp = (KeyNamePair)pTermCombo.getSelectedItem();
			if (kp != null)
		...
		//	S (Check)		(Currency) CheckNo, Routing
		else if (PaymentRule.equals(REF__PaymentRule.Check))
		{
		//	sCurrencyCombo.getSelectedItem();
			KeyNamePair kp = (KeyNamePair)sBankAccountCombo.getSelectedItem();
			if (kp != null)
			...
			if (error.length() != 0)
			{
				sRoutingField.setBackground(XendraPLAF.getFieldBackground_Error());
				ADialog.error(m_WindowNo, this, error);
				dataOK = false;
			}
			...
			if (error.length() != 0)
			{
				sNumberField.setBackground(XendraPLAF.getFieldBackground_Error());
				ADialog.error(m_WindowNo, this, error);
				dataOK = false;
			}
			...
			if (error.length() != 0)
			{
				sCheckField.setBackground(XendraPLAF.getFieldBackground_Error());
				ADialog.error(m_WindowNo, this, error);
				dataOK = false;
			}
		}
		...
		{
			String tender = REF_C_PaymentTenderType.CreditCard;
			if (PaymentRule.equals(REF__PaymentRule.DirectDeposit))
				tender = REF_C_PaymentTenderType.DirectDeposit;
			else if (PaymentRule.equals(REF__PaymentRule.DirectDebit))
				tender = REF_C_PaymentTenderType.DirectDebit;
			else if (PaymentRule.equals(REF__PaymentRule.Check))
				tender = REF_C_PaymentTenderType.Check;
	...
	private void processOnline()
	{
		...
		//  --  CreditCard
		if (PaymentRule.equals(REF__PaymentRule.CreditCard))
		{
			vp = (ValueNamePair)kTypeCombo.getSelectedItem();
			String CCType = vp.getValue();

			m_mPayment.setCreditCard(REF_C_PaymentTrxType.Sales, CCType,
				kNumberField.getText(), "", kExpField.getText());
			m_mPayment.setAmount(m_C_Currency_ID, m_Amount);
			m_mPayment.setPaymentProcessor();
			m_mPayment.setC_BPartner_ID(m_C_BPartner_ID);
