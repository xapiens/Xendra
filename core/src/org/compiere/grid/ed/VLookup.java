...
import org.compiere.model.persistence.X_AD_WF_Activity;
 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: VLookup.java 5498 2014-12-22 06:24:51Z xapiens $
 *
	...
	implements VEditor, ActionListener, FocusListener
{
	...
	public Integer m_AD_Column_ID = 0;
	public Integer m_AD_Reference_Value_ID = 0;
	public Integer m_WindowNo = 0;
	public Integer m_DisplayType = 0;
	...
	{
		int AD_Column_ID = MColumn.getColumn_ID(X_C_Invoice.Table_Name, "C_BPartner_ID", null);    //  C_Invoice.C_BPartner_ID
		try
		{
	...
	public Boolean configure()
	{
		...
		if (m_AD_Column_ID > 0 && m_DisplayType > 0 && LookupName.length() > 0)
		{
			try {
				if (ValidationCode.length() > 0 && m_AD_Reference_Value_ID > 0)
				{
					MLookupInfo lookupInfo = MLookupFactory.getLookupInfo (Env.getCtx(), m_WindowNo, 
							m_AD_Column_ID, m_DisplayType,
							Env.getLanguage(Env.getCtx()), LookupName, m_AD_Reference_Value_ID,
							false, ValidationCode);
					
					Lookup lookup = new MLookup(lookupInfo, 0);
					setLookup(LookupName, isMandatory, isReadOnly, isUpdateable, lookup);
					if (lookup != null)
						sucess = true;
				}
				else
				{
					Lookup lookup = MLookupFactory.get(Env.getCtx(), m_WindowNo, 0, m_AD_Column_ID, m_DisplayType);
					setLookup(LookupName, isMandatory, isReadOnly, isUpdateable, lookup);
					if (lookup != null)
						sucess = true;
				}
	...
	/**
	 *  Create Optional Warehouse Search Lookup
	 *  @param WindowNo window
	 *  @return VLookup
	 */
	public static VLookup createWarehouse (int WindowNo)
	{
		int AD_Column_ID = 1151;	// M_Warehouse.M_Warehouse_ID
		try {
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.Search);
			return new VLookup("M_Warehouse_ID", false, false, true, lookup);
		}
		catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
	...
	public static VLookup create(String Tablename, String Columnname, int WindowNo) {
		try {
			int AD_Column_ID = MColumn.getColumn_ID(Tablename, Columnname, null);
			MColumn column = MColumn.get(Env.getCtx(), AD_Column_ID);
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0, 
				AD_Column_ID, column.getAD_Reference_ID());
			return new VLookup(column.getColumnName(), false, false, true, lookup);
		} catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
		return null;
	...
	{
		int AD_Column_ID = Util.getColumnID(MInvoiceLine.Table_ID,"M_Product_ID");    //  C_InvoiceLine.M_Product_ID
		try
		{
			Lookup lookup = MLookupFactory.get (Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.Search);
	...
	/**
	 * Create mandatory Tax Search Lookup
	 * @param WindowNo windows
	 * @return VLookup
	 */
	public static VLookup createTaxes (int WindowNo)
	{
		int AD_Column_ID = Util.getColumnID(MTax.Table_ID, "C_Tax_ID");	// C_Tax.C_Tax_ID
		try {
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.TableDir);
			return new VLookup("C_Tax_ID", true, false, true, lookup);
		}
		catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
	...
	/**
	 *  Create Optional Locator Search Lookup
	 *  @param WindowNo window
	 *  @return VLookup
	 */
	public static VLookup createLocator (int WindowNo)
	{
		int AD_Column_ID = 1389;	// M_Locator.M_Locator_ID
		try {
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.Search);
			return new VLookup("M_Locator_ID", false, false, true, lookup);
		}
		catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
	...
	{
		int AD_Column_ID = Util.getColumnID(X_AD_WF_Activity.Table_ID, "AD_User_ID");    //  AD_WF_Activity.AD_User_UD 10443
		try
		{
			Lookup lookup = MLookupFactory.get (Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.Search);
	...
	public static VLookup createDocType (int WindowNo, boolean isSOTrx, String BaseType)
	{
		String IsSOTrx;
		if (isSOTrx)
			IsSOTrx = "Y";
		...
		//int AD_Column_ID = 1501;	// C_DocType.C_DocType_ID
		int AD_Column_ID = Util.getColumnID(X_C_DocType.Table_ID, "C_DocType_ID");
		try {
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo,
				AD_Column_ID, DisplayType.TableDir, Env.getLanguage(Env.getCtx()), "C_DocType_ID", 0, false, "C_DocType.IsSOTrx='" + IsSOTrx + "' AND C_DocType.DocBaseType='" + BaseType + "'");
			return new VLookup("C_DocType_ID", true, false, true, lookup);
		}
		catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
	...
	/**
	 * Create mandatory Price List Version Search Lookup
	 * @param WindowNo windows
	 * @return VLookup
	 */
	public static VLookup createPriceListVersion(int WindowNo, boolean isSOTrx)
	{
		//int AD_Column_ID = 2987;	// M_PriceList_Version.M_PriceList_Version_ID
		int AD_Column_ID = Util.getColumnID(X_M_PriceList.Table_ID, "M_PriceList_Version_ID");
		try {
			Lookup lookup = MLookupFactory.get(Env.getCtx(), WindowNo, 0,
				AD_Column_ID, DisplayType.TableDir);
			return new VLookup("M_PriceList_Version_ID", true, false, true, lookup);
		}
		catch (Exception e) {
			log.log(Level.SEVERE, "", e);
		}
		...
		Lookup lookup)
	{
		...
		setLookup(columnName, mandatory, isReadOnly, isUpdateable, lookup);
	}	//	VLookup

	public void setLookup(String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable, Lookup lookup)
	{
	...
	/** determine if the ENTER KEY has been pressed */
	private boolean             m_EnterKeyPressed = false;
	...
	public void setReadWrite (boolean value)
	{
		...
		if (m_combo.isReadWrite() != value)
		{
			...
			//m_combo.getEditor().getEditorComponent().setBackground(m_combo.getBackground());					
			//AutoCompletion.setBackground();
			setUI (false);			
	...
	public void setField (GridField mField)
	{
		...
		if (m_mField != null && MRole.getDefault().isSuperUser())
	...
	private String getDirectAccessSQL (String text)
	{
		...
		else if (m_columnName.equals(X_C_Coupon.COLUMNNAME_C_Coupon_ID))
		{
			sql.append("SELECT C_Coupon_ID FROM C_Coupon WHERE UPPER(DocumentNo) LIKE ")
				.append(DB.TO_STRING(text));
		}
		...
		else if (m_columnName.equals("C_BOE_ID"))
		{
			sql.append("SELECT C_BOE_ID FROM C_BOE WHERE UPPER(DocumentNo) LIKE ")
				.append(DB.TO_STRING(text));
		}				
	...
	{
	 return m_EnterKeyPressed;
	}
	
	public void setName(String name) {
		super.setName(name);
		m_combo.setName(name);
	}
