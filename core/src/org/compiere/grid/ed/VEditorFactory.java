 ...
 *  @author  Jorg Janke
 *  @version $Id: VEditorFactory.java 4093 2012-04-14 11:47:34Z xapiens $
 */
public class VEditorFactory
{
	...
	public static VEditor getEditor (GridTab mTab, GridField mField, boolean tableEditor)
	{
		...
		{
			VPAttribute attrib = new VPAttribute (mandatory, readOnly, updateable, WindowNo, (MPAttributeLookup)mField.getLookup());
		...
		else if (displayType == DisplayType.Script)
		{
			VScript vs = new VScript(columnName, mandatory, readOnly, updateable, mField.getDisplayLength(), mField.getFieldLength());
			vs.setName(columnName);
			vs.setField(mField);
			editor = vs;
		}
	...
	//	label.setFont(XendraPLAF.getFont_Label());
	//	label.setForeground(XendraPLAF.getTextColor_Label());
		return label;
	}   //  getLabel
