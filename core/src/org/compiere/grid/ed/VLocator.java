 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: VLocator.java 1266 2010-06-07 05:05:59Z xapiens $
 */
	...
	implements VEditor, ActionListener
{
		...
		MLocatorLookup mLocator, int WindowNo)
	{
		...
		m_button.setIcon(new ImageIcon(org.compiere.Xendra.class.getResource("images/Locator10.gif")));
	...
	private void setValue (Object value, boolean fire)
	{
		if (value != null)
		{
			...
			m_mLocator.setInTransit(IsInTransit());
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		m_mLocator.setOnly_Product_ID(getOnly_Product_ID());
		m_mLocator.setInTransit(inTransit);
		m_mLocator.refresh();
		VLocatorDialog ld = new VLocatorDialog(Env.getFrame(this),
			Msg.translate(Env.getCtx(), m_columnName),
			m_mLocator, M_Locator_ID, isMandatory(), only_Warehouse_ID, inTransit);
		//	display
		ld.setVisible(true);
	...
	// cuando la aprobacion esta activada, puede acceder a todos los almacenes
	private boolean IsInTransit()
	{
		boolean transit = false;
		if (  getName().compareTo("M_LocatorTo_ID") != 0 )
			return transit;
		String InTransit = Env.getContext(Env.getCtx(), m_WindowNo, "IsInTransit", true);
		try 
		{
			if (InTransit != null && InTransit.length() > 0)
				transit = InTransit.equals("Y");
		}
		catch (Exception ex)
		{
		}
