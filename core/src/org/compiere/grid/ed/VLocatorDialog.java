 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: VLocatorDialog.java 1264 2010-06-06 23:39:40Z xapiens $
 */
	...
	implements ActionListener, KeyListener
{
	...
	public VLocatorDialog (Frame frame, String title, MLocatorLookup mLocator,
		int M_Locator_ID, boolean mandatory, int only_Warehouse_ID, boolean inTransit)
	{
	...
	private void jbInit() throws Exception
	{
		...
		fWarehouseInfo.setBackground(XendraPLAF.getFieldBackground_Inactive());
	...
	private void initLocator()
	{
		...
		//	Load Warehouse
		StringBuffer sql = new StringBuffer("SELECT M_Warehouse_ID, Name FROM M_Warehouse");
		if (m_only_Warehouse_ID != 0)
			sql.append(" WHERE M_Warehouse_ID=" + m_only_Warehouse_ID);
		String SQL = "";
		if (m_inTransit)
		{
			Integer AD_Client_ID = (Integer) Env.getContextAsInt(Env.getCtx(), m_WindowNo, "AD_Client_ID");
			if (sql.indexOf(" WHERE ") == -1)
				sql.append(" WHERE ");
			else
				sql.append(" AND ");

			sql.append(" AD_Client_ID="+AD_Client_ID+" ORDER BY 2");
			SQL = sql.toString();
		}
		else
		{
				SQL = MRole.getDefault().addAccessSQL(
				sql.toString(), "M_Warehouse", MRole.SQL_NOTQUALIFIED, MRole.SQL_RO)
				+ " ORDER BY 2";
		}
		try
		{
