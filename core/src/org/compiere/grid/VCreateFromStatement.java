 ...
 *
 *  @author Jorg Janke
 *  @version  $Id: VCreateFromStatement.java 1176 2010-04-01 04:11:40Z xapiens $
 */
public class VCreateFromStatement extends VCreateFrom implements VetoableChangeListener
{
	...
	private void loadBankAccount (int C_BankAccount_ID)
	{
		...
		String sql = "SELECT p.DateTrx,p.C_Payment_ID,p.DocumentNo, p.C_Currency_ID,c.ISO_Code, p.PayAmt,"
			//+ "currencyConvert(p.PayAmt,0,p.C_Currency_ID,ba.C_Currency_ID,?,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID),"   //  #1
			+ "currencyConvert(p.PayAmt,0,p.C_Currency_ID,ba.C_Currency_ID,p.DateTrx,p.C_ConversionType_ID,p.AD_Client_ID,p.AD_Org_ID),"   // Bug: conversion should be done with the document date, not the statement date
			+ " bp.Name "
	...
	protected boolean save()
	{
		...
		MBankAccount ba = new MBankAccount(Env.getCtx(), bs.getC_BankAccount_ID(), null);
		...
		for (int i = 0; i < rows; i++)
		{
			if (((Boolean)model.getValueAt(i, 0)).booleanValue())
			{
				...
				// Bug: is a payment is in a different currency than the account, then the statement line must
				// have the amount converted to the account's currency.
				bsl.setC_Payment_ID(C_Payment_ID);
				bsl.setC_Currency_ID(ba.getC_Currency_ID());
				bsl.setTrxAmt(StmtAmt);
				bsl.setStmtAmt(StmtAmt);
				bsl.setDescription((new MPayment(Env.getCtx(), C_Payment_ID, null)).getDescription());
