 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: VPanel.java 5204 2014-01-17 22:37:27Z xapiens $
 *  @contributor Victor Perez , e-Evolution.SC FR [ 1757088 ]
...
public final class VPanel extends CPanel
{
	...
	public void addField (VEditor editor, GridField mField)
	{
		...
		{
			if (REF_AD_FieldGroup.Collapse.equals(fieldGroupType))
			{
		...
		if (fieldGroupType.equals(REF_AD_FieldGroup.Tab))
		{
			CPanel m_tab = (CPanel)m_tablist.get(fieldGroup);
			m_tab.add(label, constraints);	
		}			
		else if (fieldGroupType.equals(REF_AD_FieldGroup.Collapse))
		{
	...
	public Component[] getComponentsRecursive()
	{
		...
		for (int i = 0; i < this.getComponentCount(); i++)
		{
			...
			if (this.getComponent(i) instanceof CPanel)
			{    
				//CPanel panel = (CPanel)this.getComponentAt(i);                    
				CPanel panel = (CPanel)this.getComponent(i);
