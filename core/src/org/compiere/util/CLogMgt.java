...
public class CLogMgt {
	...
	public static void initialize(boolean isClient) {
		...
		try {
			...
			/**
			 * Enumeration en = mgr.getLoggerNames(); while
			 * (en.hasMoreElements()) { Logger lll =
			 * Logger.getLogger(en.nextElement().toString());
			 * System.out.println(lll.getName() + " (" + lll + ")"); //
			 * System.out.println("- level=" + lll.getLevel()); //
			 * System.out.println("- parent=" + lll.getParent() +
			 * " - UseParentHandlers=" + lll.getUseParentHandlers()); //
			 * System.out.println("- filter=" + lll.getFilter()); handlers =
			 * lll.getHandlers(); // System.out.println("- handlers=" +
			 * handlers.length); for (int i = 0; i < handlers.length; i ++) {
			 * System.out.println("  > " + handlers[i]); if
			 * (!s_handlers.contains(handlers[i])) s_handlers.add(handlers[i]);
			 * } // System.out.println(); } /**
			 **/
		} catch (Exception e) {
			 ...
			 * Source) at java.util.logging.LogManager.access$900(Unknown
			 * Source) at
			 * java.util.logging.LogManager$RootLogger.getHandlers(Unknown
			 * Source) at org.compiere.util.CLogMgt.initialize(CLogMgt.java:67)
			 * at org.compiere.Adempiere.startup(Adempiere.java:389) at
			 * org.compiere.Adempiere.main(Adempiere.java:500) at
	...
	/** LOG Levels */
	public static final Level[] LEVELS = new Level[] { Level.OFF, Level.SEVERE,
	 ...
	 */
	public static void printProperties(Properties p, String description,
			boolean logIt) {
		if (p == null)
			return;
		if (logIt)
			log.info(description + " - Size=" + p.size() + ", Hash="
					+ p.hashCode() + "\n" + getLocalHost());
		else
			System.out.println("Log.printProperties = " + description
					+ ", Size=" + p.size() + ", Hash=" + p.hashCode() + "\n"
					+ getLocalHost());
	...
	/**
	 * Get Xendra System Info
	 * 
	 * @param sb
	 *            buffer to append or null
	...
	public static StringBuffer getInfo(StringBuffer sb) {
		...
		sb.append(getMsg("Schema")).append(eq).append(
				CConnection.get().getDbUid()).append(NL);
		//
		sb.append(getMsg("AD_User_ID")).append(eq).append(
				Env.getContext(Env.getCtx(), "#AD_User_Name")).append(NL);
		sb.append(getMsg("AD_Role_ID")).append(eq).append(
				Env.getContext(Env.getCtx(), "#AD_Role_Name")).append(NL);
		//
		sb.append(getMsg("AD_Client_ID")).append(eq).append(
				Env.getContext(Env.getCtx(), "#AD_Client_Name")).append(NL);
		sb.append(getMsg("AD_Org_ID")).append(eq).append(
				Env.getContext(Env.getCtx(), "#AD_Org_Name")).append(NL);
		//
		sb.append(getMsg("Date")).append(eq).append(
				Env.getContext(Env.getCtx(), "#Date")).append(NL);
		sb.append(getMsg("Printer")).append(eq).append(
				Env.getContext(Env.getCtx(), "#Printer")).append(NL);
		//
		...
		if (mf != null) {
			...
			if (atts != null) {
				...
				while (it.hasNext()) {
					Object key = it.next();
					if (key.toString().startsWith("Impl")
							|| key.toString().startsWith("Spec"))
						sb.append(key).append(eq).append(atts.get(key)).append(
								NL);
				}
			}
		}
		// Show Implementation Vendor / Version - teo_sarca, [ 1622855 ]
		sb.append(getMsg("ImplementationVendor")).append(eq).append(
				org.compiere.Xendra.getImplementationVendor()).append(NL);
		sb.append(getMsg("ImplementationVersion")).append(eq).append(
				org.compiere.Xendra.getImplementationVersion()).append(NL);
		//
		sb.append("XendraHome = ").append(Xendra.getXendraHome()).append(NL);
		sb.append("XendraProperties = ").append(Ini.getPropertyFileName())
				.append(NL);
		sb.append(Env.getLanguage(Env.getCtx())).append(NL);
		MClient client = MClient.get(Env.getCtx());
		sb.append(client).append(NL);
		sb.append(getMsg("IsMultiLingualDocument")).append(eq).append(
				client.isMultiLingualDocument()).append(NL);
		sb.append("BaseLanguage = ").append(
				Env.isBaseLanguage(Env.getCtx(), "AD_Window")).append("/")
				.append(Env.isBaseLanguage(Env.getCtx(), "C_UOM")).append(NL);
		sb.append(Xendra.getJavaInfo()).append(NL);
		sb.append("java.io.tmpdir=" + System.getProperty("java.io.tmpdir"));
		sb.append(Xendra.getOSInfo());
		//
		return sb;
	} // getInfo
	...
	public static StringBuffer getInfoDetail(StringBuffer sb, Properties ctx) {
		...
		sb.append(NL).append("=== Environment === ").append(
				Xendra.getCheckSum()).append(NL).append(
				Xendra.getSummaryAscii()).append(NL).append(getLocalHost())
				.append(NL).append(
						cc.getName() + " " + cc.getDbUid() + "@"
								+ cc.getConnectionURL()).append(NL).append(
						cc.getInfo()).append(NL);
		// Context
	...
	private static String getDatabaseInfo() {
		StringBuffer sb = new StringBuffer();
		sb.append(CConnection.get().getDbHost()).append(" : ").append(
	...
	private void testLog() {
		...
		log1.log(Level.INFO, "info message n", new Object[] { "1Param",
				"2Param" });
	} // testLog
