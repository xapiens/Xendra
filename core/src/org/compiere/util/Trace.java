 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: Trace.java,v 1.1 2007/06/14 23:44:22 sergioaguayo Exp $
 */
public class Trace
{
	/**
	 ...
	 */
	public static void printStack (boolean xendraOnly, boolean first9only)
	{
	...
	//	t.printStackTrace();
		...
		for (int i = 1; i < elements.length; i++)
		{
			if (elements[i].getClassName().indexOf("util.Trace") != -1)
				continue;
			if (!xendraOnly
				|| (xendraOnly && elements[i].getClassName().startsWith("org.compiere"))
				)
			{
