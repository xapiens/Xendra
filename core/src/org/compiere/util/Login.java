...
//import org.xendra.store.TransitionManager;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: Login.java 5497 2014-12-22 06:20:34Z xapiens $
 */
public class Login
{
	/**
	...
	//	logger.entering("Env", "initTest");
		if (!Xendra.startupEnvironment(true))
			System.exit (1);
		//  Test Context
	...
	private KeyNamePair[] getRoles (String app_user, String app_pwd, boolean force)
	{
		...
		if (system == null)
		{
			...
			//TransitionManager.main(null);
		}
		else if (system.isLDAP())
		{
		...
		try
		{
			...
			Env.setContext(m_ctx, "#IsChangePassword", rs.getString(5));
			//
			Ini.setProperty(Ini.P_UID, app_user);
			if (Ini.isPropertyBool(Ini.P_STORE_PWD))
				Ini.setProperty(Ini.P_PWD, app_pwd);
			
			m_connectionProfile = rs.getString(X_AD_User.COLUMNNAME_ConnectionProfile);		//	User Based
...
//			}
	...
	//	s_log.fine("loadClients - Role: " + role.toStringX());
		...
		try
		{
			...
			{
				m_connectionProfile = rs.getString(X_AD_Role.COLUMNNAME_ConnectionProfile);	//	Role Based
//				if (m_connectionProfile != null
		...
		KeyNamePair warehouse, java.sql.Timestamp timestamp, String printerName)
	{
		...
		try
		{
			...
			else
			{
				...
				Env.setContext(m_ctx, "$C_SPOT_ID", rs.getInt("C_SPOT_ID"));
				Env.setContext(m_ctx, "$IsRetention", rs.getString("isRetention"));
			}
			rs.close();
			pstmt.close();
		...
		//	Country
		MCountry country = MCountry.getDefault(m_ctx);
		if (country != null)
			Env.setContext(m_ctx, "#C_Country_ID", country.getC_Country_ID());
		else
			log.log(Level.SEVERE, "C_Country_ID by default is missing");
		return retValue;
	}	//	loadPreferences
