 ...
 *
 *  @author     Jorg Janke
 *  @version    $Id: DB.java 5822 2016-06-15 22:56:06Z xapiens $
 *  ---
...
public final class DB
{
	...
	private static String LastSQLState;
	...
	{
		//	Get Property File
		//String envName = Ini.getXendraHome();
		//if (envName == null)
		//	return;
		//envName += File.separator + "XendraEnv.properties";
		...
		if (machine == null)
			return;
		if (machine.get_ColumnIndex(X_A_Machine.COLUMNNAME_Properties) < 0)
			return;
		...
		//			return;
		//		}
		HashMap properties = machine.getProperties();
		String updated = (String) properties.get(Constants.MAIL_UPDATED);
		if (updated != null && updated.equals("Y"))
			return;

		//	See org.compiere.install.ConfigurationData
		String server = (String) properties.get(Constants.MAIL_SERVER);
		if (server == null || server.length() == 0)
			return;
		String adminEMail = (String) properties.get(Constants.ADMIN_EMAIL);
		if (adminEMail == null || adminEMail.length() == 0)
			return;
		String mailUser = (String) properties.get(Constants.MAIL_USER);
		if (mailUser == null || mailUser.length() == 0)
			return;
		String mailPassword = (String) properties.get(Constants.MAIL_PASSWORD);
		...
		//		try
		//		{
		//			env.setProperty("XENDRA_MAIL_UPDATED", "Y");
	...
	//	 *  @return Connection (r/w)
	//	 */
	//	public static Connection getBackupConnectionRW (boolean createNew)
	//	{
	//        return createConnection(true, true, false, Connection.TRANSACTION_READ_COMMITTED);
	//	}   //  getConnectionRW
	//	
	/**
	...
	{
		//		return createConnection (false, autoCommit, readOnly,trxLevel);
		//	}
		//    /**
		//     *  Create new Connection.
		//     *  The connection must be closed explicitly by the application
	 ...
	 *  @return Xendra Database Driver
	 */
	public static XendraDatabase getDatabase()
	{
		if (s_cc != null)
	 ...
	 */
	public static XendraDatabase getDatabase(String URL)
	{
	...
	public static boolean isDatabaseOK (Properties ctx)
	{
		...
		if (Xendra.DB_VERSION.equals(version))
			return true;

		String AD_Message = "DatabaseVersionError";
		String title = org.compiere.Xendra.getName() + " " +  Msg.getMsg(ctx, AD_Message, true);
		//  Code assumes Database version {0}, but Database has Version {1}.
		String msg = Msg.getMsg(ctx, AD_Message);   //  complete message
		msg = MessageFormat.format(msg, new Object[] {Xendra.DB_VERSION, version});
	...
	{
		//    Check Build
		String buildClient = Xendra.getImplementationVersion();
	...
	public static int executeUpdate (String sql, Object[] params, boolean ignoreError, String trxName, int timeOut)
	{
		...
		catch (Exception e)
		{
			...
			LastSQLState = ((PSQLException) e).getSQLState();			
			if (LastSQLState.equals("23505") && !sql.startsWith("UPDATE") && !sql.contains(" INDEX "))
			{
				AccessSqlParser parser = new AccessSqlParser();
				String table = parser.getIntoTableInfo(sql);
				String tableid = table+"_ID";
				int idc = MColumn.getColumn_ID(table, tableid, null);
				if (idc > 0) {
					...
					if (table.length() > 0)
					{
						...
						try {
							...
							Integer maxid = DB.getSQLValue(m_trx.getTrxName(), "SELECT MAX(".concat(tableid).concat(") FROM ").concat(table));
							maxid++;
							String tablasql = "SELECT setval('".concat(tableseq).concat("', ").concat(maxid.toString()).concat(")");
							DB.getSQLValue(m_trx.getTrxName(), tablasql);
							m_trx.commit();
						}
						catch (Exception e2)
						{
	...
	{
		Trx m_trx = Trx.get(Trx.createTrxName("nextid"), true);
		String trx = m_trx.getTrxName();
		m_trx.start();
		int m_sequence_id = 0;
		String seqname = TableName.toLowerCase()+"_seq";
		String seqrealname = DB.getSQLValueString(trx, "SELECT relname FROM pg_class WHERE relname = ? ;", seqname );			
		if (seqrealname != null)				
			m_sequence_id = CConnection.get().getDatabase().getNextID(seqname, trx);
		else
		{
			String tablename   = DB.getSQLValueString(trx, "SELECT relname FROM pg_class WHERE relname = ? ;", TableName.toLowerCase() );
			if (tablename != null)
			{
				m_sequence_id = DB.getSQLValue(trx,"SELECT MAX("+TableName+"_ID"+") from "+TableName);
				m_sequence_id ++ ;
			}
			if (m_sequence_id == 0)
				m_sequence_id = 1000000;
			CConnection.get().getDatabase().createSequence(seqname, 1, 0 , 99999999,  m_sequence_id, trx);
			m_sequence_id = CConnection.get().getDatabase().getNextID(seqname, trx);
		}
		m_trx.commit();
		m_trx.close();
	...
	public static String getDocumentNo (int AD_Client_ID, String TableName, String trxName, PO po)
	{
		...
		// asi nos evitamos el lock si no se hace un commit.
		String dn = MSequence.getDocumentNo (AD_Client_ID, TableName, trxName, po);
		if (dn == null)
	...
	private static boolean verifyTrx(String trxName, String sql) {
	...
	public static String getLastSQLState() {
		return LastSQLState;
	}
