 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: SecureEngine.java,v 1.1 2007/06/14 23:44:22 sergioaguayo Exp $
 */
public class SecureEngine
{
	/**
	 * 	Initialize Security
	 *	@param ctx context with XENDRA_SECURE class name
	 */
	public static void init (Properties ctx)
	{
		if (s_engine == null)
		{
			String className = ctx.getProperty(SecureInterface.XENDRA_SECURE);
	...
	private SecureEngine (String className)
	{
		...
		if (realClass == null || realClass.length() == 0)
			realClass = SecureInterface.XENDRA_SECURE_DEFAULT;
		...
		if (implementation == null)
		{
			String msg = "Could not initialize: " + realClass + " - " + cause.toString()
				+ "\nCheck start script parameter XENDRA_SECURE"; 
			log.severe(msg);
			System.err.println(msg);
			System.exit(10);
		}
