 ...
 *  </pre>
 * 	@author 	Jorg Janke
 * 	@version 	$Id: TableElement.java 5583 2015-08-05 14:11:58Z xapiens $
 * 
		...
		final int firstRow, final int nextPageRow, boolean isView)
	{
		...
		//if (m_columnHeader[col].toString().length() > 0)
		{
			...
			while (measurer.getPosition() < iter.getEndIndex())		//	print header
			{
				...
				if (alignment.equals(REF_AD_PrintFieldAlignment.Block))
				{
				...
				float penX = curX;
				if (alignment.equals(REF_AD_PrintFieldAlignment.Center))
					penX += (netWidth-layout.getAdvance())/2;
				else if ((alignment.equals(REF_AD_PrintFieldAlignment.TrailingRight) && layout.isLeftToRight())
						|| (alignment.equals(REF_AD_PrintFieldAlignment.LeadingLeft) && !layout.isLeftToRight()))
						...
						if (str.length() > 0)
						{
							...
							for (int lineNo = 0; lineNo < lines.length; lineNo++)
							{
								...
								while (measurer.getPosition() < iter.getEndIndex())		//	print element
								{
									...
									{
										if (alignment.equals(REF_AD_PrintFieldAlignment.Block) && measurer.getPosition() < iter.getEndIndex())
										{
											layout = layout.getJustifiedLayout(netWidth + 2);
											fastDraw = false;
										}
