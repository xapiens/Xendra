 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: LayoutEngine.java 5822 2016-06-15 22:56:06Z xapiens $
 * 
...
public class LayoutEngine implements Pageable, Printable, Doc
{
	...
	private void createStandardHeaderFooter()
	{
		...
		if (ci.getLogoReport_ID() > 0) {
			...
			element = new ImageElement(org.compiere.Xendra.getImageLogoSmall(true));	//	48x15
		}
	//	element = new ImageElement(org.compiere.Adempiere.getImageLogo());	//	100x30
		element.layout(48, 15, false, REF_AD_PrintFieldAlignment.LeadingLeft);
		...
		element.layout (m_header.width, 0, true, REF_AD_PrintFieldAlignment.Center);
		element.setLocation(m_header.getLocation());
		m_headerFooter.addElement(element);
		//
		//
		element = new StringElement("@Page@ @*Page@ @of@ @*PageCount@", font, color, null, true);
		element.layout (m_header.width, 0, true, REF_AD_PrintFieldAlignment.TrailingRight);
		element.setLocation(m_header.getLocation());
		m_headerFooter.addElement(element);
		...
		//
		element = new StringElement(Xendra.XENDRA_R, font, color, null, true);
	...
	//	element = new StringElement(Adempiere.NAME, font, color, null, true);
		element.layout (m_footer.width, 0, true, REF_AD_PrintFieldAlignment.LeadingLeft);
		...
		element.layout (m_footer.width, 0, true, REF_AD_PrintFieldAlignment.Center);
		element.setLocation(ft);
		m_headerFooter.addElement(element);
		//
		element = new StringElement("@*CurrentDateTime@", font, color, null, true);
		element.layout (m_footer.width, 0, true, REF_AD_PrintFieldAlignment.TrailingRight);
		element.setLocation(ft);
		m_headerFooter.addElement(element);
	}	//	createStandardHeaderFooter
		...
		for (int row = 0; row < m_data.getRowCount(); row++)
		{
			...
			//	log.fine("layoutForm - Row=" + row + " - #" + i + " - " + item);
				...
				if (item.isRelativePosition())
				{
					...
					{
						alignment = REF_AD_PrintFieldAlignment.LeadingLeft;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
					else if (item.isLineAlignCenter())
					{
						alignment = REF_AD_PrintFieldAlignment.Center;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
					else if (item.isLineAlignTrailing())
					{
						alignment = REF_AD_PrintFieldAlignment.TrailingRight;
						maxWidth = getAreaBounds().width;
						lineAligned = true;
					}
				}
				...
				else if (item.isTypeAccountSheet())
				{
					element = createAccountSheet(item, m_data);
				}
	...
	private PrintElement createAccountSheet(MPrintFormatItem item,
			PrintData data) {
		newLine();
		PrintElement element = null;
		//
		...
		query.addRestriction("Record_ID", MQuery.EQUAL, new Integer(Record_ID));
		query.addRestriction("AD_Table_ID", MQuery.EQUAL, MTable.getTable_ID(data.getTableName()));
		query.getRecordCount();
		String sql = "SELECT COUNT(*) FROM Fact_Acct WHERE " + query.getWhereClause();
		int no = DB.getSQLValue(m_TrxName, sql.toString());
		if ( no == 0 && data.getTableName().equals(MPayment.Table_Name))
		{
			query = new MQuery (MAllocationLine.Table_ID);
			query.addRestriction("C_Payment_ID", MQuery.EQUAL, new Integer(Record_ID));
			query.getRecordCount();			
			sql = "SELECT COUNT(*) FROM "+query.getTableName() + " WHERE " +query.getWhereClause();
			no = DB.getSQLValue(m_TrxName, sql.toString());	
			if (no > 0 )
			{
				sql = "SELECT C_AllocationHdr_ID FROM " +query.getTableName() + " WHERE " +query.getWhereClause();
				//
				...
				try
				{
					...
					StringBuffer sb = new StringBuffer();
					query = new MQuery ();
					query.addRestriction("AD_Table_ID", MQuery.EQUAL, MAllocationHdr.Table_ID);				
					...
					while (rs.next())
					{
						if (rs.isFirst())
							sb.append(" ( ");
						C_AllocationHdr_ID = rs.getInt(1);		//	required
						sb.append("Record_ID").append( MQuery.EQUAL).append(C_AllocationHdr_ID);
						if (!rs.isLast())
							sb.append(" OR ");
						else
							sb.append(" ) ");
				...
				//query.addRestriction("Record_ID", MQuery.EQUAL, new Integer(Record_ID));
				//query.addRestriction("AD_Table_ID", MQuery.EQUAL, new Integer(MTable.getTable_ID(data.getTableName())));				
			}
			else
				return null;
		}
		...
		// inicio test xapiens
//			log.fine(query.toString());
//			format = MPrintFormat.get (getCtx(), 1001416 , false); // T_DayliBook_4
//			query = new MQuery (format.getAD_Table_ID());
//			//query.addRestriction(item.getColumnName(), MQuery.EQUAL, new Integer(Record_ID));
//			//String where = "ad_table_id" + MQuery.EQUAL + item.get_Table_ID();
//			String where =  " record_id" + MQuery.EQUAL + new Integer(Record_ID);
//			query.addRestriction(where);
//			format.setTranslationViewQuery(query);
//			format.setLanguage(m_format.getLanguage());
//			if (m_format.isTranslationView())
//				format.setTranslationLanguage(m_format.getLanguage());
//			includedData = de.getPrintData(data.getCtx(), format, query);
		// fin test
...
//		includedData = de.getPrintData(data.getCtx(), format, query);
		...
		String FieldAlignmentType, boolean isForm)
	{
		...
		if (item.getColumnName().contains("AmtInWords") || item.getName().contains("AmtInWords"))
		{
		...
		else if (REF_AD_PrintFieldAlignment.Default.equals(FieldAlignmentType))
		{
			if (data.isNumeric())
				FieldAlignmentType = REF_AD_PrintFieldAlignment.TrailingRight;
			else
				FieldAlignmentType = REF_AD_PrintFieldAlignment.LeadingLeft;
		}
	...
	private PrintElement layoutTable (MPrintFormat format, PrintData printData,int xOffset)
	{
		...
		for (int c = 0; c < format.getItemCount(); c++)
		{
			...
			if (item.isPrinted())
			{
				...
				if (columnJustification[col] == null || columnJustification[col].equals(REF_AD_PrintFieldAlignment.Default))
					columnJustification[col] = REF_AD_PrintFieldAlignment.LeadingLeft;	//	when generated sets correct alignment
				//	Column Fonts
	...
	//	System.out.println("Rows=" + rows);
		...
		table.layout(0,0,false, REF_AD_PrintFieldAlignment.LeadingLeft);
