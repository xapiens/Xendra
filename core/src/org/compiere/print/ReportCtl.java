...
import org.compiere.model.MPrintForm;
 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: ReportCtl.java 2436 2011-02-15 17:33:01Z xapiens $
 * 
...
public class ReportCtl
{
	...
	static public boolean start (ASyncProcess parent, int WindowNo, ProcessInfo pi)
	{
		...
		MPrintForm pf = MPrintForm.get(ctx, Env.getAD_Client_ID(ctx));
		if (pf == null)
			s_log.log(Level.SEVERE, "No PrintForm defined for Client " + Env.getAD_Client_ID(ctx));
		/**
		 *	Order Print
		 */
        ...
        else if (pi.getAD_Process_ID() == pf.getBOE_Process_ID())
        	return startDocumentPrint(ReportEngine.BOE, pi.getRecord_ID(), parent, WindowNo, !pi.isPrintPreview());
