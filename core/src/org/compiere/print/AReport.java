...
import org.xendra.printdocument.Formats;
 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: AReport.java 1459 2010-07-30 03:21:29Z xapiens $
 */
public class AReport implements ActionListener
{
	...
	public AReport (int AD_Table_ID, JComponent invoker, MQuery	query, ASyncProcess parent, int WindowNo)
	{
		...
		//	See What is there
		getPrintFormats (AD_Table_ID, query, invoker);
	}	//	AReport
	...
	private ArrayList<KeyNamePair>	m_formats = new ArrayList<KeyNamePair>();
	/**	Logger			*/
	 ...
	 *  @param invoker component to display popup (optional)
	 */
	private void getPrintFormats (int AD_Table_ID, MQuery	query, JComponent invoker)
	{
		Icon report = Env.getImageIcon2("Report16");		
		Icon format = Env.getImageIcon2("Print16");
		int AD_Client_ID = Env.getAD_Client_ID(Env.getCtx());
		List<KeyNamePair> kplist = Formats.getFormat(AD_Table_ID, query);
		if (kplist != null)
		{
			for (KeyNamePair kp:kplist)
			{
				m_formats.add(kp);
				JMenuItem item = m_popup.add(kp.toString());
				item.setIcon(format);
		...
		try
		{
			while (rowSet.next())
			{
				...
				{
					m_list.add(pp);
					JMenuItem item = m_popup.add(pp.toString());
					item.setIcon(report);
					item.addActionListener(this);
				}
			}
		...
		else if ( (m_list.size() == 1 && m_formats.size() == 0) || invoker == null)
	 ...
	 * 	@param pp Key=AD_PrintFormat_ID
	 */
	private void launchFormat (KeyNamePair pp)
	{
		MPrinterDocumentFormat pf = new MPrinterDocumentFormat(Env.getCtx(), pp.getKey(), null);
		launchFormat (pf);
	}	//	launchReport
	 ...
	 * 	@param pf print format
	 */
	private void launchFormat (MPrinterDocumentFormat pf)
	{
		int Record_ID = 0;
		if (m_query.getRestrictionCount()==1 && m_query.getCode(0) instanceof Integer)
			Record_ID = ((Integer)m_query.getCode(0)).intValue();		
