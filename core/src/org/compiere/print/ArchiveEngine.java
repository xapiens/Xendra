 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: ArchiveEngine.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class ArchiveEngine
{
	...
	public byte[] archive (LayoutEngine layout, PrintInfo info)
	{
		...
		String aa = aaClient;
		if (aa == null)
			aa = REF_AD_ClientAutoArchive.None;
		if (aaRole != null)
		{
			if (aaRole.equals(REF_AD_ClientAutoArchive.AllReportsDocuments))
				aa = aaRole;
			else if (aaRole.equals(REF_AD_ClientAutoArchive.Documents) && !aaClient.equals(REF_AD_ClientAutoArchive.AllReportsDocuments))
				aa = aaRole;
		}
		//	Mothing to Archive
		if (aa.equals(REF_AD_ClientAutoArchive.None))
			return null;
		//	Archive External only
		if (aa.equals(REF_AD_ClientAutoArchive.ExternalDocuments))
		{
			if (info.isReport())
				return null;
		}
		//	Archive Documents only
		if (aa.equals(REF_AD_ClientAutoArchive.Documents))
		{
			if (info.isReport())
				return null;
		}
