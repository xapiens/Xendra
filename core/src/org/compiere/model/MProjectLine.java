...
import org.compiere.model.persistence.X_C_ProjectLine;
 ...
 *
 *	@author Jorg Janke
 *	@version $Id: MProjectLine.java 4138 2012-05-28 23:17:34Z xapiens $
 */
public class MProjectLine extends X_C_ProjectLine
{
	...
	{
		String sql = "UPDATE C_Project SET PlannedAmt=( SELECT COALESCE(SUM(pl.PlannedAmt),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" PlannedQty=( SELECT COALESCE(SUM(pl.PlannedQty),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" PlannedMarginAmt=( SELECT COALESCE(SUM(pl.PlannedMarginAmt),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" CommittedAmt=( SELECT COALESCE(SUM(pl.CommittedAmt),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" CommittedQty=( SELECT COALESCE(SUM(pl.CommittedQty),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" InvoicedAmt=( SELECT COALESCE(SUM(pl.InvoicedAmt),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) ," +
				" InvoicedQty=( SELECT COALESCE(SUM(pl.InvoicedQty),0) FROM C_ProjectLine pl WHERE pl.C_Project_ID=C_Project.C_Project_ID AND pl.IsActive='Y' ) " +
