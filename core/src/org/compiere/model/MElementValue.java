 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MElementValue.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MElementValue extends X_C_ElementValue
{
	...
	public MElementValue(Properties ctx, int C_ElementValue_ID, String trxName)
	{
		...
		//	setValue (null);
			setIsSummary (false);
			setAccountSign (REF_C_ElementValueAccountSign.Natural);
			setAccountType (REF_C_ElementValueAccountType.Expense);
	...
	public void set (X_I_ElementValue imp)
	{
		...
		//
		X_C_Subject subject = new Query(Env.getCtx(), X_C_Subject.Table_Name, "Value = ? ", imp.get_TrxName()).setParameters(imp.getSubject_value()).first();
		if (subject != null)
			this.setC_Subject_ID(subject.getC_Subject_ID());
		//
		subject = new Query(Env.getCtx(), X_C_Subject.Table_Name, "Value = ?", imp.get_TrxName()).setParameters(imp.getEGPF_C_Subject_value()).first();
		if (subject != null)
			this.setEGPF_C_Subject_ID(subject.getC_Subject_ID());
		//
		subject = new Query(Env.getCtx(), X_C_Subject.Table_Name, "Value = ?", imp.get_TrxName()).setParameters(imp.getEGPN_C_Subject_value()).first();
		if (subject != null)
			this.setEGPN_C_Subject_ID(subject.getC_Subject_ID());
		setisDocTypeAccount(imp.isDocTypeAccount());
		setTypeGroupingClose(imp.getTypeGroupingClose());		
	...
	{
		if (newRecord)
			insert_Tree(REF_AD_TreeTypeType.ElementValue, getC_Element_ID());
	...
	{
		if (success)
			delete_Tree(REF_AD_TreeTypeType.ElementValue);
		return success;
	}	//	afterDelete
