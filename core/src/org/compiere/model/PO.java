...
import org.compiere.model.reference.REF_EventChangeLog;
 ...
 *
 *  @author Jorg Janke
 *  @version $Id: PO.java 5842 2016-06-20 01:36:32Z xapiens $
 *
 ...
 * @author Xapiens
 * 			added replication by JMS
 */
public abstract class PO extends Observable
implements Serializable, Comparator, Evaluatee
{
	...
	private static final String USE_TIMEOUT_FOR_UPDATE = "org.xendra.po.useTimeoutForUpdate";
	private static final int QUERY_TIME_OUT = 10;
	//private ReplicationEngine replication = null;
	/**
	...
	protected final boolean set_Value (int index, Object value)
	{
		...
		else
		{
			...
			if (p_info.getColumnClass(index) == String.class)
			{
				...
				if (p_info.getColumn(index).DisplayType != 45 && p_info.getColumn(index).DisplayType != 14)
				{
	...
	public void setProcessedOn(String ColumnName, Object value, Object oldValue) {
		if ("Processed".equals(ColumnName)
			...
			if (get_ColumnIndex("ProcessedOn") > 0) {
				...
				Timestamp ts = DB.getSQLValueTS(null, "SELECT CURRENT_TIMESTAMP ");
	...
	public final void setForceValueOfColumn(String columnName, Object value)
	{
		try {
			int index = get_ColumnIndex(columnName);
			if (index == -1)
			...
			boolean before = p_info.isColumnUpdateable(index);
			p_info.setColumnUpdateable(index, true);
			if (!value.getClass().equals(clazz))
			{
				Object value2 = null;
				if (clazz.getSimpleName().equals(Timestamp.class.getSimpleName()))
				{
					value2 = Timestamp.valueOf((String) value);
				}
				else
				{
					try {
						Constructor<?> constructor = clazz.getDeclaredConstructor(new Class[]{String.class});
						value2 = constructor.newInstance(new Object[] {value});
					} catch (Exception e) {
						return;
					}
				}
				set_Value(index, value2);		
			}
			else
			{
				set_Value(index, value);
			}		
			p_info.setColumnUpdateable(index, before);
		}
		catch (Exception e)
		{
			e.printStackTrace();
		...
		if (from.getClass() != to.getClass())
		{
			for (int i1 = 0; i1 < from.m_oldValues.length; i1++)
			{
				...
				if (colName.startsWith("Created")
						...
						|| colName.equals("Fact_ID")
						|| colName.equals("DateAcct")
						)
					;	//	ignore
				else
				{
	...
	protected boolean load (ResultSet rs)
	{
		...
		for (index = 0; index < size; index++)
		{
			...
			try
			{
				...
				else if (DisplayType.isHStore(dt))
				{
					HashMap props = (HashMap) rs.getObject(columnName);
					if (props == null)
						props = new HashMap();
					m_oldValues[index] = props;
				}
	...
	/**
	 * Update Value or create new record.
	 * @throws XendraException
	 * @see #save()
	 */
	public void saveEx() throws XendraException
	{
		if (!save()) {
			String msg = null;
	 ...
	 * @see #saveEx(String)
	 */
	public void saveEx(String trxName) throws XendraException
	{
		set_TrxName(trxName);
		saveEx();
	}
	...
	{		
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (DisplayType.isLOB(dt) || DisplayType.isHStore(dt))
			{
				lobAdd (value, i, dt);
					...
					)
			{
				...
				MChangeLog cLog = session.changeLog (
						...
						get_ID(), getAD_Client_ID(), getAD_Org_ID(), oldV, newV, REF_EventChangeLog.Update);
				if (cLog != null)
		...
		if (changes)
		{
			...
			{
				ok = lobSave();				
				if (p_info.getReplicationType().equals(REF_ReplicationType.Merge))
				{
					try {										
						X_AD_ReplicationTable r = new Query(Env.getCtx(), X_AD_ReplicationTable.Table_Name, "TableIdentifier = ?", null)
						.setParameters(get_ValueAsString(X_AD_Table.COLUMNNAME_Identifier)).first();
						if (r == null) {
							r = new X_AD_ReplicationTable(Env.getCtx(), 0, null);
							r.setAD_Table_ID(get_Table_ID());
							r.setTableIdentifier(get_ValueAsString(X_AD_Table.COLUMNNAME_Identifier));
						}
						r.setProcessed(false);
						r.save();
						//						ReplicationEngine r = new ReplicationEngine();
						//						String err = r.processtable(this, 0);
						//						if (err.length() == 0) {
						//							String msg = r.build();
						//							if (msg != null && msg.length() > 0) {
						//								String identifier = (String) get_Value(X_AD_Table.COLUMNNAME_Identifier);							
					...
					//							ReplicationEngine.getInstance().clear();
					//							m_replmsgerror = ReplicationEngine.getInstance();
					//									.processtable(this, 0);
					//							if (m_replmsgerror.length() == 0)
					//								m_replmsg = ReplicationEngine.getInstance().build();													
					...
					//					try {
					//						ReplicationEngine.getInstance().clear();
					//						m_replmsgerror = ReplicationEngine.getInstance().processtable(this, 0);
					//						if (m_replmsgerror.length() == 0)
					//							m_replmsg = ReplicationEngine.getInstance().build();						
					//					}
					//					catch (Exception e)
	...
	{		
		...
		index = p_info.getColumnIndex(X_AD_Table.COLUMNNAME_Identifier);
		if (index != -1)
		{
			String value = (String)get_Value(index);
			if (value == null || value.length() == 0)
			{
				value = Util.getUUID();
				set_ValueNoCheck(X_AD_Table.COLUMNNAME_Identifier, value);
			}
		}
		//boolean updated = false;
		//boolean updatedBy = false;		
		...
		for (int i = 0; i < size; i++)
		{
			...
			try
			{
				...
				//						mapstring += String.format("\"%s\"=>", key);
				//						Object mapvalue = map.get(key);						
				//						mapstring += String.format("\"%s\"", mapvalue);												
				//					}
				//					if (mapstring.contains("\\"))
				//						mapstring = mapstring.replace("\\", "\\\\");
				//					sqlValues.append(String.format("'%s'", mapstring));
				//				}
				else
					sqlValues.append (saveNewSpecial (value, i));
			}
					...
					)
			{
				...
				MChangeLog cLog = session.changeLog (
						m_trxName, AD_ChangeLog_ID,
						p_info.getAD_Table_ID(), p_info.getColumn(i).AD_Column_ID,
						get_ID(), getAD_Client_ID(), getAD_Org_ID(), null, value, REF_EventChangeLog.Insert);
				if (cLog != null)
					AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
			}
		...
		if (ok)
		{
			...
			if (p_info.getReplicationType().equals(REF_ReplicationType.Merge))
			{
				...
				r.setProcessed(false);
				r.save();				
				//new ReplicationEngine().process(this, 0);
			}			
		}
		else
		{
										...
										)
								{
									...
									MChangeLog cLog = session.changeLog (
											m_trxName != null ? m_trxName : localTrxName, AD_ChangeLog_ID,
													...
													Record_ID, getAD_Client_ID(), getAD_Org_ID(), value, null, REF_EventChangeLog.Delete);
									if (cLog != null)
										AD_ChangeLog_ID = cLog.getAD_ChangeLog_ID();
								}
							}	//   for all fields
	 ...
	 * @see #delete(boolean)
	 */
	public void deleteEx(boolean force) throws XendraException
	{
		if (!delete(force)) {
			String msg = null;
			ValueNamePair err = CLogger.retrieveError();
	 ...
	 */
	public void deleteEx(boolean force, String trxName) throws XendraException
	{
		set_TrxName(trxName);
		deleteEx(force);
	}
			...
			String acctBaseTable, String whereClause)
	{
		...
		.append(", p.C_AcctSchema_ID, p.AD_Client_ID,0,'Y', CURRENT_TIMESTAMP,")
		.append(getUpdatedBy()).append(",CURRENT_TIMESTAMP,").append(getUpdatedBy());
		for (int i = 0; i < s_acctColumns.size(); i++)
	...
	protected boolean insert_Tree (String treeType, int C_Element_ID)
	{
		...
		.append(" (AD_Client_ID,AD_Org_ID, IsActive,Created,CreatedBy,Updated,UpdatedBy, "
				...
				+ "SELECT t.AD_Client_ID, 0, 'Y', CURRENT_TIMESTAMP, "+getUpdatedBy()+", CURRENT_TIMESTAMP, "+getUpdatedBy()+","
	...
	protected final static String   XML_ATTRIBUTE_Table_ID = "Table_ID";
	protected final static String   XML_ATTRIBUTE_Element = "Element";
	...
	public Document get_xmlDocument(boolean noComment)
	{
		...
		try
		{
			...
			if (!noComment)
				document.appendChild(document.createComment(Xendra.getSummaryAscii()));
		}
		catch (Exception e)
		{
			log.log(Level.SEVERE, "", e);
		...
		//	Root
		Element root = document.createElement(XML_ATTRIBUTE_Element);
		...
		root.setAttribute(XML_ATTRIBUTE_Table_ID, table.getIdentifier());
		root.setAttribute("TableName", get_TableName());
		...
		for (int i = 0; i < size; i++)
		{
			...
			else if (c == Boolean.class)
			{
				...
				col.appendChild(document.createTextNode(value.toString()));
			else if (c == String.class && dt == DisplayType.XML)
			{	
				try {
					StringReader reader = new StringReader( value.toString() );			
					InputSource inputSource = new InputSource( reader);
					Document docu = builder.parse(inputSource);								
					col.appendChild(document.importNode(docu.getDocumentElement(),true));
				}
				catch (Exception e)
				{
					e.printStackTrace();
		...
		{
			StringBuffer sql = new StringBuffer ("SELECT AD_Language, name, printname, description, ")
			.append(" help, po_name, po_printname, po_description, po_help ")
			.append(" FROM ").append(p_info.getTableName()).append("_Trl WHERE ")
			.append(m_KeyColumns[0]).append("=? AND istranslated='Y'");					
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
				...
				{						
					...
					translation.appendChild(translate);
					translate.setAttribute(X_AD_Client.COLUMNNAME_AD_Language,rs.getString("AD_Language"));
					translate.setAttribute(X_AD_Client.COLUMNNAME_Name,rs.getString("name"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_PrintName,rs.getString("printname"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_Description,rs.getString("description"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_Help, rs.getString("help"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_PO_Name, rs.getString("po_name"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_PO_PrintName, rs.getString("po_printname"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_PO_Description, rs.getString("po_description"));
					translate.setAttribute(X_AD_Element.COLUMNNAME_PO_Help,rs.getString("po_help"));							
				}
			} catch (Exception e) {	}
			finally {
				DB.close(rs, pstmt);
	...
	private String m_replmsg = "";
	private String m_replmsgerror = "";
	...
	public String getReplicateMessageError() {		
		return m_replmsgerror;
	}
	...
	public String getReplicationType() {
		return p_info.getReplicationType();
	}
}   //  PO