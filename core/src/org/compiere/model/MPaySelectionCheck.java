 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: MPaySelectionCheck.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public final class MPaySelectionCheck extends X_C_PaySelectionCheck
{
	...
	public static MPaySelectionCheck createForPayment (Properties ctx, int C_Payment_ID, String trxName)
	{
		...
		String PaymentRule = REF__PaymentRule.Check;
		if (payment.getTenderType().equals(REF_C_PaymentTenderType.CreditCard))
			PaymentRule = REF__PaymentRule.CreditCard;
		else if (payment.getTenderType().equals(REF_C_PaymentTenderType.DirectDebit))
			PaymentRule = REF__PaymentRule.DirectDebit;
		else if (payment.getTenderType().equals(REF_C_PaymentTenderType.DirectDeposit))
			PaymentRule = REF__PaymentRule.DirectDeposit;
	//	else if (payment.getTenderType().equals(MPayment.TENDERTYPE_Check))
	//		PaymentRule = MPaySelectionCheck.REF__PaymentRule.Check;
	...
	private static String[] getBPartnerInfo (int C_BPartner_ID)
	{
		...
		String sql = "SELECT bp.Value, bp.Name, c.Name AS Contact, "
			...
			+ " FROM C_BPartner bp LEFT OUTER JOIN AD_User c ON bp.C_BPartner_ID = c.C_BPartner_ID" +
					", C_BPartner_Location l, " +
					"C_Location a LEFT OUTER JOIN C_Region r ON a.C_Region_ID = r.C_Region_ID, C_Country cc "
	...
	public static int confirmPrint (MPaySelectionCheck[] checks, MPaymentBatch batch)
	{
		...
		for (int i = 0; i < checks.length; i++)
		{
			...
			{
				//	Update check number
				if (check.getPaymentRule().equals(REF__PaymentRule.Check))
				{
			...
			else	//	New Payment
			{
				...
				else if (check.getPaymentRule().equals(REF__PaymentRule.CreditCard))
					payment.setTenderType(REF_C_PaymentTenderType.CreditCard);
				else if (check.getPaymentRule().equals(REF__PaymentRule.DirectDeposit)
					|| check.getPaymentRule().equals(REF__PaymentRule.DirectDebit))
				...
				payment.setTrxType(REF_C_PaymentTrxType.CreditPayment);
	...
	public MPaySelectionCheck (MPaySelectionLine line, String PaymentRule)
	{
		...
		else if (REF__PaymentRule.DirectDeposit.equals(PaymentRule))
		{
			MBPBankAccount[] bas = MBPBankAccount.getOfBPartner (line.getCtx(), C_BPartner_ID); 
			for (int i = 0; i < bas.length; i++) 
			{
	...
	{
		return (REF__PaymentRule.DirectDeposit.equals(getPaymentRule())
			|| REF__PaymentRule.DirectDebit.equals(getPaymentRule()));
	}	//	isDirect
