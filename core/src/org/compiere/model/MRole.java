 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MRole.java 5721 2016-01-02 23:13:51Z xapiens $
 */
public final class MRole extends X_AD_Role
{
	...
	public MRole (Properties ctx, int AD_Role_ID, String trxName)
	{
		...
		//	setName (null);
			...
			setUserLevel (REF_AD_RoleUserLevel.Organization);
			setPreferenceType(REF_AD_RolePreferenceType.Organization);			
	...
	//	{
			if (getAD_Client_ID() == 0)
				setUserLevel(REF_AD_RoleUserLevel.System);
			else if (getUserLevel().equals(REF_AD_RoleUserLevel.System))
			{
	...
	public String updateAccessRecords ()
	{
		...
		if (REF_AD_RoleUserLevel.System.equals(getUserLevel()))
			roleAccessLevel = "('4','7','6')";
		else if (REF_AD_RoleUserLevel.Client.equals(getUserLevel()))
			roleAccessLevel = "('7','6','3','2')";
		else if (REF_AD_RoleUserLevel.ClientPlusOrganization.equals(getUserLevel()))
	...
	public boolean isCanReport (int AD_Table_ID)
	{
		...
		for (int i = 0; i < m_tableAccess.length; i++)
		{
			if (!REF_AD_Table_AccessRuleType.Reporting.equals(m_tableAccess[i].getAccessTypeRule()))
				continue;
	...
	public boolean isCanExport (int AD_Table_ID)
	{
		...
		for (int i = 0; i < m_tableAccess.length; i++)
		{
			if (!REF_AD_Table_AccessRuleType.Exporting.equals(m_tableAccess[i].getAccessTypeRule()))
				continue;
			if (m_tableAccess[i].isExclude())		//	Exclude
			{
	...
	public boolean isTableAccess (int AD_Table_ID, boolean ro)
	{
		...
		for (int i = 0; i < m_tableAccess.length; i++)
		{
			if (!REF_AD_Table_AccessRuleType.Accessing.equals(m_tableAccess[i].getAccessTypeRule()))
				continue;
			if (m_tableAccess[i].isExclude())		//	Exclude
	...
	public boolean isTableAccessLevel (int AD_Table_ID, boolean ro)
	{
		...
		if (roleAccessLevel.equals(REF_AD_TableAccessLevels.All))
			return true;
		//	User Level = SCO
		String userLevel = getUserLevel();
		//	
		if (userLevel.charAt(0) == 'S'
			&& (roleAccessLevel.equals(REF_AD_TableAccessLevels.SystemOnly) 
				|| roleAccessLevel.equals(REF_AD_TableAccessLevels.SystemPlusClient)))
			return true;
		if (userLevel.charAt(1) == 'C'
			&& (roleAccessLevel.equals(REF_AD_TableAccessLevels.ClientOnly) 
				|| roleAccessLevel.equals(REF_AD_TableAccessLevels.SystemPlusClient)))
			return true;
		if (userLevel.charAt(2) == 'O'
			&& (roleAccessLevel.equals(REF_AD_TableAccessLevels.Organization) 
				|| roleAccessLevel.equals(REF_AD_TableAccessLevels.ClientPlusOrganization)))
			return true;
...
//	}
		...
		boolean fullyQualified, boolean rw)
	{
		...
		int posGroup = SQL.lastIndexOf(" GROUP BY ");
		if (posGroup != -1)
		{
			orderBy = SQL.substring(posGroup);
			retSQL.append(SQL.substring(0,posGroup));
		}		
		...
		for (int i = 0; i < ti.length; i++)
		{
			...
			// when a table relation with the same table (C_Invoice_ID, C_Invoice_Ref) the Data Access generate a no access
			if (TableName.equals(TableNameIn) && !ti[i].getSynonym().equals(ti[0].getSynonym())) continue;			
	...
	public boolean canView(Properties ctx, String TableLevel)
	{
		...
		//	7 - All
		if (REF_AD_TableAccessLevels.All.equals(TableLevel))
			retValue = true;
			 
		//	4 - System data requires S
		else if (REF_AD_TableAccessLevels.SystemOnly.equals(TableLevel) 
			&& userLevel.charAt(0) != 'S')
			retValue = false;

		//	2 - Client data requires C
		else if (REF_AD_TableAccessLevels.ClientOnly.equals(TableLevel) 
			&& userLevel.charAt(1) != 'C')
			retValue = false;

		//	1 - Organization data requires O
		else if (REF_AD_TableAccessLevels.Organization.equals(TableLevel) 
			&& userLevel.charAt(2) != 'O')
			retValue = false;

		//	3 - Client Shared requires C or O
		else if (REF_AD_TableAccessLevels.ClientPlusOrganization.equals(TableLevel)
			&& (!(userLevel.charAt(1) == 'C' || userLevel.charAt(2) == 'O')) )
				retValue = false;

		//	6 - System/Client requires S or C
		else if (REF_AD_TableAccessLevels.SystemPlusClient.equals(TableLevel)
	...
	{
		return !REF_AD_RolePreferenceType.None.equals(getPreferenceType());
	}	//	isShowPreference
