...
import org.compiere.model.persistence.X_AD_Sequence;
 ...
 *  @author Jorg Janke
 *  @version $Id: MSequence.java 5704 2015-11-20 12:22:28Z xapiens $
 */
public class MSequence extends X_AD_Sequence
{
	...
	public static int getNextID (int AD_Client_ID, String TableName, String trxName)
	{
		...
		//	Check XendraSys
		boolean xendraSys = Ini.isPropertyBool(Ini.P_XENDRASYS);
		if (xendraSys && AD_Client_ID > 11)
			xendraSys = false;
		//
		if (CLogMgt.isLevel(LOGLEVEL))
			s_log.log(LOGLEVEL, TableName + " - XendraSys=" + xendraSys  + " [" + trxName + "]");
		  //begin vpj-cd e-evolution 09/02/2005 PostgreSQL
		String selectSQL = null;
		selectSQL = String.format("SELECT CurrentNext, CurrentNextSys, IncrementNo, AD_Sequence_ID "
			+ "FROM AD_Sequence "
			+ "WHERE Name='%s'"
			+ " AND IsActive='Y' AND IsTableID='Y' AND IsAutoSequence='Y' ", TableName);
			//+ " FOR UPDATE OF AD_Sequence ",TableName);
//		USE_PROCEDURE=false;
...
//					pstmt.setQueryTimeout(QUERY_TIME_OUT);
				...
				if (rs.next())
				{
					...
					if (queryProjectServer && ( ! xendraSys ) && ( ! isExceptionCentralized(TableName) ) ) {
					...
					if (! gotFromHTTP) {
						...
						//if (USE_PROCEDURE)
						//{
							retValue = nextID(conn, AD_Sequence_ID, xendraSys);
						//}
						//else
						//{
							//	PreparedStatement updateSQL;
...
//	 */
	private static int nextID (Connection conn, int AD_Sequence_ID, boolean xendraSys)
	{
		...
		try
		{
			...
			cstmt.setString(2, xendraSys ? "Y" : "N");
		...
		//{
			if (isStartNewYear) {
				selectSQL = "SELECT y.CurrentNext, s.CurrentNextSys, s.IncrementNo, s.Prefix, s.Suffix, s.DecimalPattern, s.AD_Sequence_ID "
						...
						//+ "FOR UPDATE OF AD_Sequence NOWAIT";
			} else {
...
//				pstmt.setQueryTimeout(QUERY_TIME_OUT);
			...
			if (rs.next())
			{
				...
				//{
					next = isStartNewYear
						...
						: nextID(conn, AD_Sequence_ID, xendraSys);
				//}
				//else
						//{
		...
		catch (Exception e)
		{
			...
			if (DBException.isTimeout(e))
				throw new XendraException("GenerateDocumentNoTimeOut", e);
			else
				throw new XendraException("GenerateDocumentNoError", e);
		}
		finally
		{
			//Finish
...
//				pstmt.setQueryTimeout(QUERY_TIME_OUT);
	...
	{
		StringBuilder where = new StringBuilder("IsActive='Y' AND IsView='N' AND ")
		.append("AD_Table_ID in (SELECT AD_Table_ID FROM AD_Column WHERE ColumnName = 'DocumentNo' OR ColumnName = 'Value')")
		.append("AND 'DocumentNo_' || TableName NOT IN (SELECT Name FROM AD_Sequence s WHERE s.AD_Client_ID=?)");
		List<X_AD_Table> tables = new Query(Env.getCtx(), X_AD_Table.Table_Name,  where.toString(), null)
			.setParameters(AD_Client_ID).list();
		int counter = 0;
		boolean success = true;		
		for (X_AD_Table table:tables) {
			MSequence seq = new MSequence (ctx, AD_Client_ID, table.getTableName(), trxName);
			if (seq.save())
				counter++;
			else
			{
...
//		return seq.save();
	 ...
	 *  @param COLUMNNAME_AD_Client_ID client
	 *  @param TableName table name
	 * 	@param trxName optional Transaction Name
