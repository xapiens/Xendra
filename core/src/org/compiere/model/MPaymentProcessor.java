...
import org.compiere.model.persistence.X_C_PaymentProcessor;
 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MPaymentProcessor.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MPaymentProcessor extends X_C_PaymentProcessor
{
		...
		int AD_Client_ID, int C_Currency_ID, BigDecimal Amt, String trxName)
	{
		...
		if (REF_C_PaymentTenderType.DirectDeposit.equals(tender))
			sql.append(" AND AcceptDirectDeposit='Y'");
		else if (REF_C_PaymentTenderType.DirectDebit.equals(tender))
			sql.append(" AND AcceptDirectDebit='Y'");
		else if (REF_C_PaymentTenderType.Check.equals(tender))
			sql.append(" AND AcceptCheck='Y'");
		//  CreditCards
		else if (REF_C_PaymentCreditCardType.ATM.equals(CCType))
			sql.append(" AND AcceptATM='Y'");
		else if (REF_C_PaymentCreditCardType.Amex.equals(CCType))
			sql.append(" AND AcceptAMEX='Y'");
		else if (REF_C_PaymentCreditCardType.Visa.equals(CCType))
			sql.append(" AND AcceptVISA='Y'");
		else if (REF_C_PaymentCreditCardType.MasterCard.equals(CCType))
			sql.append(" AND AcceptMC='Y'");
		else if (REF_C_PaymentCreditCardType.Diners.equals(CCType))
			sql.append(" AND AcceptDiners='Y'");
		else if (REF_C_PaymentCreditCardType.Discover.equals(CCType))
			sql.append(" AND AcceptDiscover='Y'");
		else if (REF_C_PaymentCreditCardType.PurchaseCard.equals(CCType))
	...
	public boolean accepts (String TenderType, String CreditCardType)
	{
		...
		if ((REF_C_PaymentTenderType.DirectDeposit.equals(TenderType) && isAcceptDirectDeposit())
			|| (REF_C_PaymentTenderType.DirectDebit.equals(TenderType) && isAcceptDirectDebit())
			|| (REF_C_PaymentTenderType.Check.equals(TenderType) && isAcceptCheck())
			//
			|| (REF_C_PaymentCreditCardType.ATM.equals(CreditCardType) && isAcceptATM())
			|| (REF_C_PaymentCreditCardType.Amex.equals(CreditCardType) && isAcceptAMEX())
			|| (REF_C_PaymentCreditCardType.PurchaseCard.equals(CreditCardType) && isAcceptCorporate())
			|| (REF_C_PaymentCreditCardType.Diners.equals(CreditCardType) && isAcceptDiners())
			|| (REF_C_PaymentCreditCardType.Discover.equals(CreditCardType) && isAcceptDiscover())
			|| (REF_C_PaymentCreditCardType.MasterCard.equals(CreditCardType) && isAcceptMC())
			|| (REF_C_PaymentCreditCardType.Visa.equals(CreditCardType) && isAcceptVisa()))
			return true;
		return false;
	}	//	accepts
