 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MCurrency.java 5530 2015-04-16 05:35:09Z xapiens $
 */
public class MCurrency extends X_C_Currency
{
	...
	public static MCurrency[] getMandatory() {
		String sql = "SELECT C_Currency_ID FROM C_Currency WHERE IsActive='Y' AND IsMandatory='Y' ";
		ArrayList<MCurrency> list = new ArrayList<MCurrency>();
		...
		try
		{
			...
			while (rs.next ())
				list.add (MCurrency.get(Env.getCtx(), rs.getInt(1)));
			rs.close ();
			pstmt.close ();
			pstmt = null;
		}
		...
		//
		MCurrency[] retValue = new MCurrency[list.size()];
	...
	public static MCurrency getCurrencyDolar()
	{
		...
		String sql = "SELECT C_Currency_ID  "
			+ "FROM C_Currency "
			+ "WHERE IsDolar = 'Y'"	;
		C_Currency_ID = DB.getSQLValue(null, sql);
		if (C_Currency_ID > 0)
			currency = new MCurrency(Env.getCtx(), C_Currency_ID, null);
		return currency;
	}
