 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MPriceList.java 3654 2011-11-04 01:49:49Z xapiens $
 */
public class MPriceList extends X_M_PriceList
{
	...
	public MPriceList[] getChilds()
	{
		ArrayList<MPriceList> list = new ArrayList<MPriceList> ();
		String sql = "SELECT * FROM M_PriceList WHERE basepricelist_id=?";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql, get_TrxName());			
			...
			while (rs.next())
			{
				MPriceList pl = new MPriceList(getCtx(), rs, get_TrxName());
				list.add(pl);
			}			
			rs.close();
			pstmt.close();
		...
		MPriceList[] childs = new MPriceList[list.size ()];
		list.toArray (childs);
		return childs;		
	}
	
	public static MPriceList get(String Name, String trxName)
	{
		String sql = "SELECT * FROM M_PriceList WHERE name=?";
		//
		MPriceList retValue = null;
		PreparedStatement pstmt = null;
		try
		{
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next())
				retValue = new MPriceList (Env.getCtx(), rs, trxName);
			rs.close();
			pstmt.close();
			pstmt = null;
		}
	...
	{
		MCurrency c = MCurrency.get(Env.getCtx(), getC_Currency_ID());
		StringBuffer sb = new StringBuffer (String.format("%s %s", getName(), c.getDescription()));
		return sb.toString();
	}
