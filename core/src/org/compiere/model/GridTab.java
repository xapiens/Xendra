...
import org.compiere.model.persistence.X_AD_PInstance_Log;
...
//import org.xendra.rules.Callout.Rules;
 ...
 *  </pre>
 *  @author 	Jorg Janke
 *  @version 	$Id: GridTab.java 5822 2016-06-15 22:56:06Z xapiens $
 *  
...
//		m_Chats = null;
	...
	public void query (boolean onlyCurrentRows, int onlyCurrentDays, int maxRows)
	{
		...
		if (m_vo.onlyCurrentDays > 0)
		{
			...
			where.append("CURRENT_TIMESTAMP-").append(m_vo.onlyCurrentDays);
		}
		//	Detail Query
		if (isDetail())
		{
	...
	public int getRowbyID(int keyNo)
	{
		int row = 0;
	...
	public String getTrxInfo()
	{
		...
		else if (m_vo.TableName.startsWith("C_Order") || m_vo.TableName.startsWith("C_Invoice"))
		{
			...
			StringBuffer sql = new StringBuffer("SELECT COUNT(*) AS Lines,c.ISO_Code,o.TotalLines,o.GrandTotal,"
				+ "currencyBase(o.GrandTotal,o.C_Currency_ID,o.DateAcct,o.C_ConversionType_ID, o.AD_Client_ID,o.AD_Org_ID) AS ConvAmt ");
			if (isOrder)
			{
			...
			sql.append("GROUP BY o.C_Currency_ID, c.ISO_Code, o.TotalLines, o.GrandTotal, o.DateAcct, o.C_ConversionType_ID, o.AD_Client_ID, o.AD_Org_ID");
	...
	private void loadDependentInfo()
	{
		...
		if (m_vo.TableName.equals("C_Order"))
		{
			...
			String sql = "SELECT DocSubType FROM C_DocType WHERE C_DocType_ID=?";
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
			{
...
//					value = mField.getDefault();
	...
	public String processCallout (GridField field)
	{
		...
		if (Agenda != null)
		{
			...
			int AD_Rule_ID = field.getAD_Rule_ID();
			if (AD_Rule_ID == 0)
			{
				AD_Rule_ID = m_vo.AD_Rule_ID;
			}			
			if (AD_Rule_ID > 0 && Agenda.length() > 0)
			{
				...
				if (kb != null)
				{
					...
					ksession.setGlobal("ctx", m_vo.ctx);		
					ksession.setGlobal("value", value);
					ksession.setGlobal("WindowNo", m_vo.WindowNo);
					ksession.setGlobal("result", result);									
					ksession.insert(field);						
					ksession.insert(this);
				}		
				else
				{
					String error = Env.getKieError(AD_Rule_ID);
					if (error != null)
						return error;
				}		
		...
		else
		{
			...
			while (st.hasMoreTokens())      //  for each callout
			{
				...
				if (cmd.toLowerCase().startsWith(MRule.SCRIPT_PREFIX)) {
					...
					if ( !  (rule.getEventType().equals(REF_AD_Rule_EventType.Callout) 
							&& rule.getRuleType().equals(REF_RuleType.JSR223ScriptingAPIs))) {
				...
				} // Xapiens - dravios - implement drools callout
				else if (cmd.toLowerCase().startsWith(MRule.RULEAGENDA_PREFIX)) {
					try {					
						String agenda = cmd.substring(MRule.RULEAGENDA_PREFIX.length());
						KieBase kb = Env.startRule(1000012);
						if (kb != null)
						{
							ksession = kb.newKieSession();
							ksession.addEventListener(new DebugAgendaEventListener());
							ksession.addEventListener(new DebugRuleRuntimeEventListener());
							Env.setContext(m_vo.ctx, X_AD_Rule.COLUMNNAME_AD_Rule_ID, String.valueOf(1000056));
							ksession.setGlobal("ctx", m_vo.ctx);		
							ksession.setGlobal("value", value);
							ksession.setGlobal("WindowNo", m_vo.WindowNo);
							ksession.setGlobal("result", result);									
						...
						{
							String error = Env.getKieError(1000012);
							if (error != null)
								return error;
						}		
						ksession.getAgenda().getAgendaGroup( agenda ).setFocus();
						...
						if (result.length() > 0)
							return result;
						//					String rule = cmd.substring(MRule.RULES_PREFIX.length());
						//					ProcessManager pm = ProcessManager.getInstance();
						//					pm.getrules.insert("tab",this);
						//					rules.insert("Field", field);
						//					rules.insert("Value", value);		            
						//					rules.insert("OldValue", oldValue);
						//					rules.insert("Ctx", m_vo.ctx);
						//					rules.insert("WindowNo",m_vo.WindowNo);
						//		            rules.execute();
					} catch (Throwable t) {
						t.printStackTrace();
					}				
