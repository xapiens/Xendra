 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: MAllocationHdr.java 5822 2016-06-15 22:56:06Z xapiens $
 */
public final class MAllocationHdr extends X_C_AllocationHdr implements DocAction
{
	...
	private boolean reverseIt() 
	{
		...
		setDocStatus(REF__DocumentStatus.Reversed);	//	for direct calls
		if (!save() || isActive())
	...
	private void updateBP(HashSet<Integer> bps)
	{
		...
		while (it.hasNext())
		{
			...
			if (info.getC_BPartnerCashTrx_ID() != bp.getC_BPartner_ID())
			{
