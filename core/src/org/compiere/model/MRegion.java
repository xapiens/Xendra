 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: MRegion.java 3654 2011-11-04 01:49:49Z xapiens $
 */
	...
	implements Comparator, Serializable
{
	...
	{
		List<MRegion> regions = new Query(Env.getCtx(), MRegion.Table_Name, "C_Country_ID = ?", null)
			.setParameters(C_Country_ID).list();		
		MRegion[] retValue = new MRegion[regions.size()];
		regions.toArray(retValue);
		Arrays.sort(retValue, new MRegion(ctx, 0, null));
		return retValue;
//		if (s_regions == null || s_regions.size() == 0)
//			loadAllRegions(ctx);
...
//		return retValue;
	...
	public static MRegion getbyCountry(MCountry country) {
		MRegion retvalue = null;
		Integer RegionID = 0;
		s_regions = new CCache<String,MRegion>("C_Region", 100);
		String sql = "SELECT * FROM C_Region WHERE IsActive='Y' AND C_Country_ID = "+country.getC_Country_ID();
		try
		{
			Statement stmt = DB.createStatement();
			ResultSet rs = stmt.executeQuery(sql);
			while(rs.next())
			{
				...
				RegionID = r.getC_Region_ID();
				s_regions.put(RegionID.toString(), r);
				if (r.isDefault())
					s_default = r;
			}
			rs.close();
		...
		if (s_default != null)
			...
			retvalue = s_regions.get(RegionID.toString());
		return retvalue;
	}	
}	//	MRegion
