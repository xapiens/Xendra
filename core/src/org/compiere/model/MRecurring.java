...
import org.compiere.model.persistence.X_C_Recurring;
 ...
 *
 *	@author Jorg Janke
 *	@version $Id: MRecurring.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MRecurring extends X_C_Recurring
{
	public MRecurring (Properties ctx, int C_Recurring_ID, String trxName)
	{
		...
		//	setC_Recurring_ID (0);		//	PK
			...
			setFrequencyType (REF_C_RecurringFrequency.Monthly);
			setFrequency(1);
		//	setName (null);
	...
	public String toString()
	{
		...
		if (getRecurringType().equals(REF_C_RecurringType.Order))
			sb.append(",C_Order_ID=").append(getC_Order_ID());
		else if (getRecurringType().equals(REF_C_RecurringType.Invoice))
			sb.append(",C_Invoice_ID=").append(getC_Invoice_ID());
		else if (getRecurringType().equals(REF_C_RecurringType.Project))
			sb.append(",C_Project_ID=").append(getC_Project_ID());
		else if (getRecurringType().equals(REF_C_RecurringType.GLJournal))
	...
	private void setDateNextRun()
	{
		...
		//
		if (getFrequencyType().equals(REF_C_RecurringFrequency.Daily))
			cal.add(Calendar.DAY_OF_YEAR, frequency);
		else if (getFrequencyType().equals(REF_C_RecurringFrequency.Weekly))
			cal.add(Calendar.WEEK_OF_YEAR, frequency);
		else if (getFrequencyType().equals(REF_C_RecurringFrequency.Monthly))
			cal.add(Calendar.MONTH, frequency);
		else if (getFrequencyType().equals(REF_C_RecurringFrequency.Quarterly))
