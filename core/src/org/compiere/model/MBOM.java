...
import org.compiere.model.persistence.X_M_BOM;
 ...
 * 	BOM Model
 *  @author Jorg Janke
 *  @version $Id: MBOM.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MBOM extends X_M_BOM
{
	...
	public MBOM (Properties ctx, int M_BOM_ID, String trxName)
	{
		...
		//	setM_Product_ID (0);
		//	setName (null);
			setBOMType (REF_M_BOMType.CurrentActive);	// A
			setBOMUse (REF_M_BOMUse.Master);	// A
		}
	}	//	MBOM
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord || is_ValueChanged("BOMType"))
		{
			...
			if (getBOMType().equals(REF_M_BOMType.CurrentActive))
			{
			...
			//	Only one MTO
			else if (getBOMType().equals(REF_M_BOMType.Make_To_Order))
			{
				MBOM[] boms = getOfProduct(getCtx(), getM_Product_ID(), get_TrxName(), 
					"IsActive='Y'");
				if (boms.length == 0	//	only one = this 
