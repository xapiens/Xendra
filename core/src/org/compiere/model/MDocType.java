 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MDocType.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MDocType extends X_C_DocType
{
	...
	{
		boolean isSOTrx = REF_C_DocTypeDocBaseType.SalesOrder.equals(getDocBaseType()) 
			|| REF_C_DocTypeDocBaseType.MaterialDelivery.equals(getDocBaseType())
	...
	{
		return REF_C_DocTypeSubType.Quotation.equals(getDocSubType())
			&& REF_C_DocTypeDocBaseType.SalesOrder.equals(getDocBaseType());
	}	//	isQuotation
	...
	{
		return REF_C_DocTypeSubType.Proposal.equals(getDocSubType())
			&& REF_C_DocTypeDocBaseType.SalesOrder.equals(getDocBaseType());
	}	//	isProposal
	...
	{		
		return (REF_C_DocTypeSubType.Proposal.equals(getDocSubType())
				|| REF_C_DocTypeSubType.Quotation.equals(getDocSubType()))
			&& REF_C_DocTypeDocBaseType.SalesOrder.equals(getDocBaseType());
	}	//	isOffer
	 ...
	 *	@return array of doc types
	 */
	static public MDocType[] getOfDocBaseTypeSOTrx (Properties ctx,boolean isSOTrx)
	{
		ArrayList<MDocType> list = new ArrayList<MDocType>();
		String sql = "select * from C_DocType where DocBaseType IN ('ARI', 'API','ARC','APC') and ad_client_id=? AND IsSOTrx=? ";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement (sql, null);
	 ...
	 *	@return array of doc types
	 */
	static public MDocType[] getDocSubType(Properties ctx, String SubType)
	{
		ArrayList<MDocType> list = new ArrayList<MDocType>();
		String sql = "SELECT * FROM C_DocType WHERE AD_Client_ID=? AND DocSubType=?";
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement (sql, null);
