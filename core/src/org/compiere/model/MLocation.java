 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MLocation.java 5721 2016-01-02 23:13:51Z xapiens $
 */
public class MLocation extends X_C_Location implements Comparator
{
	 ...
	 *	@return MLocation
	 */
	public static MLocation get (Properties ctx, int C_Country_ID, int C_Region_ID, String City, String trxName)
	{
		MLocation loc = null;
		String sql = "SELECT * FROM C_Location WHERE C_Country_ID=? AND C_Region_ID=? AND City=?";
		try
		{
		...
		{
			s_log.log(Level.SEVERE, sql + " - " + C_Country_ID + " - " +  C_Region_ID + " - " + City, e);
			loc = null;
		}
		return loc;		
	}	//	get info	
	...
	public MLocation (Properties ctx, int C_Location_ID, String trxName)
	{
		...
		if (C_Location_ID == 0)
		{
			...
			System.out.println("nuevo registro de MLocation pais por defecto "+defaultCountry.toString());
			setCountry(defaultCountry);
			MRegion defaultRegion = MRegion.getbyCountry(defaultCountry);
	 ...
	 *	@param trxName transaction
	 */
	public MLocation (Properties ctx, int C_Country_ID, int C_Region_ID, int C_City_ID, String trxName)
	{
		super(ctx, 0, trxName);
		setC_Country_ID(C_Country_ID);
		setC_Region_ID(C_Region_ID);
		MCity city = new Query(Env.getCtx(), MCity.Table_Name, "C_City_ID = ?", null)
			.setParameters(C_City_ID).first();
		setCity(city);
	}	//	MLocation
	 ...
	 *	@return city
	 */
	public MCity getCityObject()
	{
		if (m_city == null)
			if (getC_City_ID() != 0)
				m_city = new MCity(Env.getCtx(), getC_City_ID(), null);
			else
				m_city = new MCity(Env.getCtx(), 0, null);
		return m_city;
	}	//	get City
	...
	public void setCity(MCity city)
	{
		...
		else
		{
			...
			super.setC_City_ID(m_city.getC_City_ID());
			if (m_city.getC_Region_ID() != getC_Region_ID())
			{
				setC_Region_ID(m_city.getC_Region_ID());
			}
		}		
	}
