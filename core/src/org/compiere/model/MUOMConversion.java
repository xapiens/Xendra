...
import org.compiere.model.persistence.X_C_UOM_Conversion;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MUOMConversion.java 5822 2016-06-15 22:56:06Z xapiens $
 */
...
//	static public BigDecimal convertBuyProductTo (Properties ctx, int M_Product_ID, int C_UOM_To_ID, BigDecimal qtyPrice)
//	{
//		ADialog.ask(0, null, "");
...
//				return qtyPrice;
//			
//			BigDecimal retValue = getBuyProductRateTo (ctx, M_Product_ID, C_UOM_To_ID);
//			if (retValue != null)
//			{
...
//	 */
//	static public BigDecimal getBuyProductRateTo (Properties ctx, int M_Product_ID, int C_UOM_To_ID)
//	{
//		if (M_Product_ID == 0)
//			return null;
//		MUOMConversion[] rates = getBuyProductConversions(ctx, M_Product_ID);
//		if (rates.length == 0)
//		{
...
//	static public BigDecimal convertBuyProductFrom	 (Properties ctx, int M_Product_ID, int C_UOM_To_ID, BigDecimal qtyPrice)
//	{
//		//	No conversion
...
//		}
//		
//		BigDecimal retValue = getBuyProductRateFrom (ctx, M_Product_ID, C_UOM_To_ID);
//		if (retValue != null)
//		{
//			if (Env.ONE.compareTo(retValue) == 0)
//				return qtyPrice;
...
//	static public BigDecimal getBuyProductRateFrom (Properties ctx, int M_Product_ID, int C_UOM_To_ID)
//	{
//		MUOMConversion[] rates = getBuyProductConversions(ctx, M_Product_ID);
//		if (rates.length == 0)
//		{
...
//	static public MUOMConversion[] getBuyProductConversions (Properties ctx, int M_Product_ID)
//	{
//		if (M_Product_ID == 0)
//			return new MUOMConversion[0];
//		Integer key = new Integer (M_Product_ID);
//		MUOMConversion[] result = (MUOMConversion[])s_buyconversionProduct.get(key);
//		if (result != null)
//			return result;
//		
...
//				+ "WHERE c.M_Product_ID=p.M_Product_ID AND c.C_UOM_ID=p.C_UOM_Order_ID)"
//			+ " AND c.IsActive='Y'";  
//		PreparedStatement pstmt = null;
//		try
//		{
...
//	}	//	getBuyProductConversions
	...
	static public MUOMConversion[] getProductConversions (Properties ctx, int M_Product_ID)
	{
		...
		ArrayList<MUOMConversion> list = new ArrayList<MUOMConversion>();
		//	Add default conversion
		MUOMConversion defRate = new MUOMConversion (MProduct.get(ctx, M_Product_ID, null));
		list.add(defRate);
		//
		String sql = "SELECT * FROM C_UOM_Conversion c "
			+ "WHERE c.M_Product_ID=?"
...
//	private static CCache<Integer,MUOMConversion[]>	s_buyconversionProduct 
//	= new CCache<Integer,MUOMConversion[]>("C_UOMConversion", 20); 
	...
	public String toString ()
	{
	 ...
	 /**************************************************************************
	    *     Obtiene la conversion de unidades entre 2 productos
	    *  @param M_Product_ID_From Producto con la unidad de medida origen
	    *  @param M_Product_ID_To to Producto con la unidad de medida destino
	    *  @param StdPrecision    Si es true, precision standard, si es false precision de costeo.
	    *
	    *     @return conversion multiplier or 1 si no existe
	    */
	   public static BigDecimal convertUomBetweenProducts(int M_Product_ID_From,int M_Product_ID_To,
	           boolean StdPrecision, String trxname) {
	       BigDecimal retValue = null;
	       int precision = 2;
	       ...
	       String sql = "select c.multiplyrate,uomTo.StdPrecision,uomTo.CostingPrecision "
	           + "from C_UOM_Conversion c "
	           + "INNER JOIN C_UOM uomTo ON (c.C_UOM_TO_ID=uomTo.C_UOM_ID)"
	           + "where c.c_uom_id = (select c_uom_id from m_product where m_product_id=?) and "
	           + "c.c_uom_to_id = (select c_uom_id from m_product where m_product_id=?) ";

	       PreparedStatement pstmt = null;
	       try
	       {
	           pstmt = DB.prepareStatement(sql, trxname);
	           pstmt.setInt(1, M_Product_ID_From);
	           pstmt.setInt(2, M_Product_ID_To);
	           ResultSet rs = pstmt.executeQuery();
	           if (rs.next())
	           {
	               retValue = rs.getBigDecimal(1);
	       ...
	       if (retValue == null)
	       {
	           s_log.info ("NOT found - UOM Conversion From Product=" + M_Product_ID_From
	               + ", To Product=" + M_Product_ID_To);
	           return new BigDecimal(1.00);
	       }

	       if (precision <= 0) {
	           s_log.warning("convertUomBetweenProducts - La precision a usarse es menor o igual que cero");
	       }
