...
public class MGLBookPeriod  extends X_GL_BookPeriod
{
	...
	public static String getConditionalWhere(int AD_Org_ID, int AD_Client_ID, int table_ID, int docType_ID, String issotrx, Timestamp dateacct) {
	...
	public static int getGLBookID(int AD_Org_ID, int AD_Client_ID, int table_ID, int docType_ID, String issotrx, Timestamp dateacct) {
		String s = getConditionalWhere(AD_Org_ID, AD_Client_ID, table_ID, docType_ID, issotrx, dateacct);
		int GL_Book_ID = 0;
		StringBuffer sql = new StringBuffer("SELECT count(*) FROM "+getconditional(s));			
		int count = DB.getSQLValue(null ,sql.toString());
		if (count==1)
		{
			try {
				sql = new StringBuffer("SELECT glb.gl_book_id from "+getconditional(s));
				GL_Book_ID = DB.getSQLValue(null ,sql.toString());
			} 
			catch (Exception e)
			{
				e.printStackTrace();
	...
	public static String getID(int AD_Org_ID, int AD_Client_ID, int table_ID, int docType_ID, String issotrx, Timestamp dateacct) {
		String sequence = "NSD";
		int GL_Book_ID = getGLBookID(AD_Org_ID, AD_Client_ID, table_ID, docType_ID, issotrx, dateacct);
		if ( GL_Book_ID > 0 )
		{	
			String orgValue = "ORGVALUE";
			if (AD_Org_ID != 0)
			...
			Trx  m_trx = Trx.get(Trx.createTrxName("GLBookPeriod"), true);
			...
			{
				try {
					StringBuffer sql = new StringBuffer("SELECT COUNT(*) FROM gl_bookperiod glbp WHERE ")
					...
					int count = DB.getSQLValue(trxName,sql.toString());
	...
	public static String oldgetID(int AD_Org_ID, int AD_Client_ID, int table_ID, int docType_ID, String issotrx, Timestamp dateacct) {
		int GL_Book_ID = 0;
		String orgValue = "ORGVALUE";
		...
		StringBuffer sql = new StringBuffer("SELECT count(*) FROM "+getconditional(sb.toString()));			
		int count = DB.getSQLValue(null ,sql.toString());
		String sequence = "NSD";
		//if (count==1 || count == 0)
		if (count==1)
		{
			...
			try {
				...
				sql = new StringBuffer("SELECT glb.gl_book_id from "+getconditional(sb.toString()));
	...
	private static String getconditional(String sb) {
