...
public class Deletecash0 
{
 
@XendraFunction(Name="deletecash",Output="void",Owner="xendra",Language="plpgsql",
Identifier="0b0508e7-da8b-cf6c-dd8e-cc86f63637a8",Arguments="p_pinstance_id numeric",Extension="",
Synchronized="2013-09-29 01:27:28.0")
	public static final String Identifier = "0b0508e7-da8b-cf6c-dd8e-cc86f63637a8";
	...
	public static final String getCode() 
{
	...
	sb.appendln("	p_C_CashBook_ID	NUMERIC(10);");
	sb.appendln("	p_DateStm	DATE;");
	sb.appendln("	mycash_id	NUMERIC(10);");
	sb.appendln("	Message		VARCHAR(2000);");
	sb.appendln("	p_IsPaid	VARCHAR(1);");
	sb.appendln("	p		RECORD;");
	sb.appendln("BEGIN");
	sb.appendln("	p_AD_Org_ID:=0;");
	sb.appendln("	p_C_CashBook_ID:=0;");
	sb.appendln("	mycash_id:=0;");
	sb.appendln("	Message:='';");
	...
	sb.appendln("		ELSIF (p.ParameterName = 'C_CashBook_ID') THEN");
	sb.appendln("			p_C_CashBook_ID := p.P_Number;");
	sb.appendln("		ELSIF (p.Parametername = 'StatementDate') THEN");
	sb.appendln("			p_DateStm := p.P_Date;");
	sb.appendln("		END IF;");
	sb.appendln("	END LOOP;	--	Get Parameter");
	sb.appendln("	--workflow");
	sb.appendln("	SELECT");
	sb.appendln("		C_Cash_ID INTO mycash_ID");
	sb.appendln("	FROM");
	sb.appendln("		C_Cash");
	sb.appendln("	WHERE");
	sb.appendln("		    StatementDate=p_DateStm");
	sb.appendln("		AND C_CashBook_ID=p_C_CashBook_ID");
	sb.appendln("		AND AD_Org_ID=p_AD_Org_ID");
	sb.appendln("		AND Processed='N'");
	sb.appendln("		AND Posted='N'");
	sb.appendln("	;");
	sb.appendln("	SELECT");
	...
	sb.appendln("			WHERE");
	sb.appendln("				C_Cash_ID=mycash_id");
	sb.appendln("			)");
	sb.appendln("	;");
	sb.appendln("	IF p_IsPaid='Y' THEN");
	sb.appendln("		Message:='Cash '||mycash_id||' is already paid. Cannot delete.';");
	sb.appendln("		UPDATE");
	...
	sb.appendln("		AND Record_ID=mycash_id");
	sb.appendln("		AND AD_Org_ID=p_AD_Org_ID");
	sb.appendln("	;");
	sb.appendln("	DELETE FROM");
	sb.appendln("		C_CashLine");
	...
	sb.appendln("		C_Cash_ID=mycash_id");
	sb.appendln("	;");
	sb.appendln("	Message:='Cash '||mycash_id||' successfully deleted.';");
	sb.appendln("	UPDATE");
	sb.appendln("		AD_PInstance");
	sb.appendln("	SET");
	sb.appendln("		Updated=now(),");
	...
	public static final String getComments() 
{
	...
	sb.appendln("@Identifier=0b0508e7-da8b-cf6c-dd8e-cc86f63637a8");
	return sb.toString();
}
}
