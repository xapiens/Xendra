 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: AccessSqlParser.java 5105 2013-11-19 20:33:43Z xapiens $
 */
public class AccessSqlParser
{
	...
	private static final int 		INTO_LENGTH = INTO.length();
	...
	private String[] getSubSQL (String[] sqlIn)
	{
		...
		for (int sqlIndex = 0; sqlIndex < sqlIn.length; sqlIndex++)
		{
			...
			while (index != -1)
			{
				...
				//System.out.println("org.compiere.model.AccessSqlParser:199");
	...
	/**
	 * Get Table Info for UPDATE SQL
	 * @param sql
	 * @return array of table info for sql.
	 */
	public String getUpdateTableInfo (String sql)
	{
		String into = "";
		...
		if (updateIndex != -1)
		{
			...
			if ( setIndex != -1 )
			{
				into = sql.substring(updateIndex+UPDATE_LENGTH, setIndex);
			}
		}
		return into.trim();
	}
	...
	/**
	 * Get Table Info for INSERT SQL
	 * @param sql
	 * @return array of table info for sql.
	 */
	public String getIntoTableInfo (String sql)
	{
		int intoIndex = sql.indexOf(INTO);
		String into = sql.substring(intoIndex+INTO_LENGTH);
		int index = into.indexOf('(');
		if (index != -1)
			into = into.substring(0, index);
		else
			into = "";
		return into.trim();
	}
	/**
	 * 	Get Table Info for SELECT SQL
	 *	@param sql sql
	 *	@return array of table info for sql
	 */
