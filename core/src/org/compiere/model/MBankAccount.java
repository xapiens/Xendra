 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MBankAccount.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MBankAccount extends X_C_BankAccount
{
	...
	public static MBankAccount get (Properties ctx, String BankAccountType)
	{
		String where = "bankaccounttype='".concat(BankAccountType).concat("'");
		MBankAccount retValue = new Query(Env.getCtx(), Table_Name, where, null).first();
		if (retValue == null)
			return null;
		if (retValue.get_ID() != 0)
		{
			if (s_cache.get (retValue.get_ID()) == null)			
	...
	public MBankAccount (Properties ctx, int C_BankAccount_ID, String trxName)
	{
		...
		{
			setIsDefault (false);
			setBankAccountType (REF_C_BankAccountType.Checking);
