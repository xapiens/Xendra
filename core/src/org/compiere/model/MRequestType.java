...
import org.compiere.model.persistence.X_R_RequestType;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MRequestType.java 5583 2015-08-05 14:11:58Z xapiens $
 *  
...
public class MRequestType extends X_R_RequestType
{
	...
	private synchronized void updateStatistics()
	{
		...
		String sql = "SELECT "
			...
			+ "(SELECT COUNT(*) FROM R_Request r "
			+ "WHERE r.R_RequestType_ID=x.R_RequestType_ID AND Created>addDays(CURRENT_TIMESTAMP,-30)) AS New30No, "
			+ "(SELECT COUNT(*) FROM R_Request r"
			+ " INNER JOIN R_Status s ON (r.R_Status_ID=s.R_Status_ID AND s.IsClosed='Y') "
			+ "WHERE r.R_RequestType_ID=x.R_RequestType_ID AND r.Updated>addDays(CURRENT_TIMESTAMP,-30)) AS Closed30No "
			//
		...
		String MeasureScope, String MeasureDataType, Timestamp reportDate, MRole role)
	{
		...
		if (REF_PA_MeasureDataType.QtyAmountInTime.equals(MeasureDataType)
