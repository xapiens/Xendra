 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MYear.java 4885 2013-05-10 21:01:18Z xapiens $
 */
public class MYear extends X_C_Year
{
	...
	public static MYear get(int C_Calendar_ID, Timestamp acctDate, String trxName) {
		String where = "date_trunc('day',?::timestamp) between date_trunc('day',startdate) AND date_trunc('day',enddate) AND AD_Client_ID = ?";
		MYear year = null;
		MPeriod period = new Query(Env.getCtx(), MPeriod.Table_Name, where, trxName).setParameters(acctDate, Env.getAD_Client_ID(Env.getCtx())).firstOnly();
		if (period != null)
		{		
			int C_Year_ID = period.getC_Year_ID();
			if (C_Year_ID > 0)
			{
				year = new MYear(Env.getCtx(), C_Year_ID, trxName);
			}			
		}
		else
		{
			System.out.println(" no se encontro periodo para la fecha "+acctDate.toString());
		}
		return year;
	}
}	//	MYear
