 ...
 *  @version $Id: GridField.java 5691 2015-10-16 22:11:33Z xapiens $
 */
	...
	implements Serializable, Evaluatee
{
	...
	public void loadLookup()
	{
		...
		else if (m_vo.displayType == DisplayType.QAttribute)    //  not cached
		{
			MQAttributeLookup pa = new MQAttributeLookup (m_vo.ctx, m_vo.WindowNo);
			m_lookup = pa;
		}				
	}   //  m_lookup
	...
	//		retValue = false;
			...
			|| m_vo.displayType == DisplayType.QAttribute)
			retValue = true;
...
//	  Do we have a mandatory rule
		...
		if (m_gridTab != null &&  // if gridtab doesn't exist then it's not a window field (probably a process parameter field)
			  (   (m_vo.IsKey && m_vo.ColumnName.endsWith("_ID"))
				...
				|| m_vo.ColumnName.equals("Q_AttributeSetInstance_ID")
			  )
			)
			return false;
	...
	public boolean isEditable (boolean checkContext)
	{
		...
		if (checkContext && "Y".equals(Env.getContext(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo, "IsActive"))
				&& (   m_vo.ColumnName.equals("Processing")
					...
					|| m_vo.ColumnName.equals("IsPrinted")
	...
	public Object getDefault()
	{
		...
		if (m_vo.ColumnName.equals(X_AD_Table.COLUMNNAME_Identifier))
			return Util.getUUID();
		
		//	Set Client & Org to System, if System access
		if (REF_AD_TableAccessLevels.SystemOnly.equals(Env.getContext(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo, GridTab.CTX_AccessLevel))
		...
		else if (REF_AD_TableAccessLevels.SystemPlusClient.equals(Env.getContext(m_vo.ctx, m_vo.WindowNo, m_vo.TabNo, GridTab.CTX_AccessLevel))
