...
import org.compiere.*;
import org.compiere.model.persistence.X_M_Cost;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MCost.java 5583 2015-08-05 14:11:58Z xapiens $
 *  
...
public class MCost extends X_M_Cost
{
	...
	public static BigDecimal getCurrentCost (MProduct product,
		...
		boolean zeroCostsOK, Timestamp DateAcct, String trxName)
	{
		...
		return getCurrentCost (
			...
			C_OrderLine_ID, zeroCostsOK, DateAcct, trxName);
	}	//	getCurrentCost
		...
		boolean zeroCostsOK, Timestamp DateAcct, String trxName)
	{
		...
		if (DateAcct != null) {
			materialCostEach = getSeedCosts(product, M_ASI_ID,
					as, Org_ID, costingMethod, C_OrderLine_ID, DateAcct);

			if (materialCostEach == null) return null;
			...
			//	Seed Initial Costs
			if (materialCostEach.signum() == 0)		//	no costs (or not created yet)
			{
				/* Last chance to get a cost here */
				materialCostEach = getSeedCosts(product, M_ASI_ID,
						as, Org_ID, costingMethod, C_OrderLine_ID, DateAcct);
		...
		if (REF_C_AcctSchemaCostingMethod.Fifo.equals(costingMethod)
			|| REF_C_AcctSchemaCostingMethod.Lifo.equals(costingMethod))
		{
	...
	public static BigDecimal getSeedCosts (MProduct product, int M_ASI_ID,
		MAcctSchema as, int Org_ID, String costingMethod, int C_OrderLine_ID, Timestamp DateAcct)
	{
		BigDecimal retValue = null;
		//	Direct Data
		if (REF_C_AcctSchemaCostingMethod.AverageInvoice.equals(costingMethod))
			retValue = calculateAverageInv(product, M_ASI_ID, as, Org_ID);
		else if (REF_C_AcctSchemaCostingMethod.AveragePO.equals(costingMethod))
			retValue = calculateAveragePO(product, M_ASI_ID, as, Org_ID, DateAcct);
		else if (REF_C_AcctSchemaCostingMethod.Fifo.equals(costingMethod))
			retValue = calculateFiFo(product, M_ASI_ID, as, Org_ID);
		else if (REF_C_AcctSchemaCostingMethod.Lifo.equals(costingMethod))
			retValue = calculateLiFo(product, M_ASI_ID, as, Org_ID);
		else if (REF_C_AcctSchemaCostingMethod.LastInvoice.equals(costingMethod))
			retValue = getLastInvoicePrice(product, M_ASI_ID, Org_ID, as.getC_Currency_ID());
		else if (REF_C_AcctSchemaCostingMethod.LastPOPrice.equals(costingMethod))
		{
			if (C_OrderLine_ID != 0)
		...
		else if (REF_C_AcctSchemaCostingMethod.StandardCosting.equals(costingMethod))
		{
			//	migrate old costs
		...
		else if (REF_C_AcctSchemaCostingMethod.UserDefined.equals(costingMethod))
			;
		else
		...
		if (!REF_C_AcctSchemaCostingMethod.StandardCosting.equals(costingMethod))
		{
			MCostElement ce = MCostElement.getMaterialCostElement(as, REF_C_AcctSchemaCostingMethod.StandardCosting);
		...
		//	PO first
		if (REF_C_AcctSchemaCostingMethod.AveragePO.equals(costingMethod)
			|| REF_C_AcctSchemaCostingMethod.LastPOPrice.equals(costingMethod)
			|| REF_C_AcctSchemaCostingMethod.StandardCosting.equals(costingMethod))
		{
			//	try Last PO
			retValue = getLastPOPrice(product, M_ASI_ID, Org_ID, as.getC_Currency_ID());
	...
	{
		BigDecimal retValue = null;
		String sql = "SELECT currencyConvert(il.PriceActual,0, i.C_Currency_ID, ?, i.DateAcct, i.C_ConversionType_ID, il.AD_Client_ID, il.AD_Org_ID) "
	...
	{
		BigDecimal retValue = null;
		String sql = "SELECT currencyConvert(ol.PriceCost,0, o.C_Currency_ID, ?, o.DateAcct, o.C_ConversionType_ID, ol.AD_Client_ID, ol.AD_Org_ID),"
	...
	public static BigDecimal getPOPrice (MProduct product, int C_OrderLine_ID, int C_Currency_ID)
	{
		BigDecimal retValue = null;
		String sql = "SELECT currencyConvert(ol.PriceCost,0, o.C_Currency_ID, ?, o.DateAcct, o.C_ConversionType_ID, ol.AD_Client_ID, ol.AD_Org_ID),"
			+ " currencyConvert(ol.PriceActual, 0, o.C_Currency_ID, ?, o.DateAcct, o.C_ConversionType_ID, ol.AD_Client_ID, ol.AD_Org_ID) "
			//	,ol.PriceCost,ol.PriceActual, ol.QtyOrdered, o.DateOrdered, ol.Line
			+ "FROM C_OrderLine ol"
			+ " INNER JOIN C_Order o ON (ol.C_Order_ID=o.C_Order_ID) "
			...
			while (rs.next ())
			{
				...
				for (int i = 0; i < ass.length; i++)
				{
					BigDecimal cost = getCurrentCost(product, 0, ass[i], 0, 
						null, Env.ONE, 0, false, null, trxNameUsed);		//	create non-zero costs
	...
	public static void create (MProduct product)
	{
		...
		//	Cost Elements
		MCostElement[] ces = MCostElement.getCostingMethods(product.getCtx(),product.getAD_Client_ID(),product.get_TrxName());
		MCostElement ce = null;
		for (int i = 0; i < ces.length; i++)
		{
			if (REF_C_AcctSchemaCostingMethod.StandardCosting.equals(ces[i].getCostingMethod()))
			{
				ce = ces[i];
				break;
			}
		...
		for (int i = 0; i < mass.length; i++)
		{
			...
			//	Create Std Costing
			if (REF_C_AcctSchemaCostingLevel.Client.equals(cl))
			{
			...
			else if (REF_C_AcctSchemaCostingLevel.Organization.equals(cl))
			{
				if (orgs == null)
	...
	public static BigDecimal calculateAveragePO (MProduct product, int M_AttributeSetInstance_ID, 
		MAcctSchema as, int AD_Org_ID, Timestamp DateAcct)
	{
		if (DateAcct == null)
			DateAcct = new Timestamp(System.currentTimeMillis());

//		String sql = "SELECT t.MovementQty, ol.QtyOrdered, ol.QtyOrdered, ol.PriceCost, ol.PriceActual,"	// 1..5
//			+ " o.C_Currency_ID, o.DateAcct, o.C_ConversionType_ID,"	// 6..8
//			+ " o.AD_Client_ID, o.AD_Org_ID, o.isSOTrx, t.M_Transaction_ID "		// 9..12
//			+ "FROM M_Transaction t"
...
//			+ "WHERE t.M_Product_ID=?"
//			+ " AND i.DateAcct<=? ";
		String sql = "SELECT record_id, qtysource + qtydestiny as Qty, PriceActual,"	// 1..5
			+ " AD_Client_ID, AD_Org_ID, Costed "		// 9..12
			+ "FROM m_carding k"
			+ " WHERE Product_ID=?"
			+ " AND movementdate<=? ";
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				boolean isSOTrx = rs.getString(6).equals("Y")?true:false;
...
//				}
				...
				//Timestamp DateAcct2 = rs.getTimestamp(7);
				//int C_ConversionType_ID = rs.getInt(8);
				int Client_ID = rs.getInt(4);
				int Org_ID = rs.getInt(5);
				BigDecimal cost = rs.getBigDecimal(3);
//				BigDecimal cost = MConversionRate.convert(product.getCtx(), price, 
//					C_Currency_ID, as.getC_Currency_ID(), 
//					DateAcct2, C_ConversionType_ID, Client_ID, Org_ID);
				//
				if (!isSOTrx) {
					if (newAverageAmt == null || newAverageAmt.compareTo(Env.ZERO)==0) {
					 newAverageAmt = cost;
					}
					BigDecimal oldAverageAmt = newAverageAmt;
					BigDecimal averageCurrent = oldStockQty.multiply(oldAverageAmt);
					//BigDecimal averageIncrease = matchQty.multiply(cost);
					BigDecimal averageIncrease = movementQty.multiply(cost);
					BigDecimal newAmt = averageCurrent.add(averageIncrease);
					newAmt = newAmt.setScale(as.getCostingPrecision());
					newAverageAmt = newAmt.divide(newStockQty, as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
				}
	 ...
	 */
	public static MCost[] getOfProduct (Properties ctx, int M_Product_ID, String trxName)
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				list.add (new MCost (ctx, rs, trxName));
			}
			rs.close ();
			pstmt.close ();
			pstmt = null;
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
			if (ce.isCalculated() 
				|| REF_M_CostElementType.Material.equals(ce.getCostElementType()) 
				&& getPercent() != 0)
				setPercent(0);
		}
		if (getPercent() != 0)
