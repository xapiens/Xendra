 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MRefList.java 5583 2015-08-05 14:11:58Z xapiens $
 *  
...
public class MRefList extends X_AD_Ref_List
{
	...
	public static String getListName (Properties ctx,String Identifier, String Value)
	{		
		String AD_Language = Env.getAD_Language(ctx);
		//String key = AD_Language + "_" + AD_Reference_ID + "_" + Value;
		String key = AD_Language + "_" + Identifier + "_" + Value;
		...
		if (isBaseLanguage) {
			...
			sql = "SELECT t.Name FROM AD_Ref_List_Trl t";
			sql += " INNER JOIN AD_Ref_List r ON (r.AD_Ref_List_ID=t.AD_Ref_List_ID) ";
			sql += " JOIN ad_reference ar ON r.ad_reference_id = ar.ad_reference_id ";
			sql += "WHERE ar.Identifier=? AND r.Value=? AND t.AD_Language=?";
		}
	...
	public static ValueNamePair[] getList(String Identifier) {
		X_AD_Reference ref = new Query(Env.getCtx(), X_AD_Reference.Table_Name, "Identifier = ? ", null)
			.setParameters(Identifier).first();
		if (ref != null) {
			return getList(ref.getAD_Reference_ID(), false);
		}
		return null;
	}
	
	public static ValueNamePair getItem(String Identifier, String value) {
		ValueNamePair vp = null;
		X_AD_Reference ref = new Query(Env.getCtx(), X_AD_Reference.Table_Name, "Identifier = ? ", null)
			.setParameters(Identifier).first();
		if (ref != null) {
			ValueNamePair[] pairs =  getList(ref.getAD_Reference_ID(), false);
