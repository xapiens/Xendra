 ...
 *
 *  @author Jorg Janke
 *  @version $Id: POInfo.java 5583 2015-08-05 14:11:58Z xapiens $
...
public class POInfo implements Serializable
{
	...
	public static void remove(int AD_Table_ID)
	{
		Integer key = new Integer(AD_Table_ID);
		if (s_cache.get(key) != null)
			s_cache.remove(key);
	}
	...
	/** Access Level			*/
	private String		m_AccessLevel = REF_AD_TableAccessLevels.Organization;
	/** Columns             	*/
	...
	private String m_ReplicationType = "L";
	...
	private void loadInfo (boolean baseLanguage, String trxName)
	{
		...
		sql.append("SELECT t.TableName, c.ColumnName,c.AD_Reference_ID,"    //  1..3
			...
			+ "c.IsAllowLogging,t.IsChangeLog, t.replicationtype ");			// 21..23
		...
		try
		{
			...
			while (rs.next())
			{
				...
				m_ReplicationType=rs.getString(23);
