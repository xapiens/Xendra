 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MProductPrice.java 3654 2011-11-04 01:49:49Z xapiens $
 */
public class MProductPrice extends X_M_ProductPrice
{
	...
	public static MProductPrice get (Properties ctx, int M_PriceList_Version_ID, int M_Product_ID,
		String trxName)
	{
		MProductPrice retValue = new Query(Env.getCtx(), X_M_ProductPrice.Table_Name, "M_PriceList_Version_ID=? AND M_Product_ID=?" , trxName)
			.setParameters(M_PriceList_Version_ID, M_Product_ID).first(); 
		//MProductPrice retValue = null;
//		String sql = "SELECT * FROM M_ProductPrice pp WHERE pp.M_PriceList_Version_ID=? AND pp.M_Product_ID=?";
