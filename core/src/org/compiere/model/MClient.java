...
import org.compiere.model.persistence.X_AD_UserMail;
 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MClient.java 5762 2016-02-18 02:16:12Z xapiens $
 *  
...
public class MClient extends X_AD_Client
{
	...
	public MClient (Properties ctx, int AD_Client_ID, boolean createNew, String trxName)
	{
		...
		if (AD_Client_ID == 0)
		{
			...
			//	setName (null);
				...
				setAutoArchive(REF_AD_ClientAutoArchive.None);
				setMMPolicy (REF__MMPolicy.FiFo);	// F
	...
	public boolean setupClientInfo (String language, String TaxID)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//
				if (value.equals(REF_AD_TreeTypeType.Organization))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_Org_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.BPartner))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_BPartner_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.Project))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_Project_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.SalesRegion))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_SalesRegion_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.Product))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_Product_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.ElementValue))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					m_AD_Tree_Account_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.Campaign))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_Campaign_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.Activity))
				{
					tree = new MTree_Base (this, name, value);
					success = tree.save();
					AD_Tree_Activity_ID = tree.getAD_Tree_ID();
				}
				else if (value.equals(REF_AD_TreeTypeType.Menu))	//	No Menu
					success = true;
	...
	public boolean isAutoArchive()
	{
		...
		return aa != null && !aa.equals(REF_AD_ClientAutoArchive.None);
	}	//	isAutoArchive
	...
	public boolean sendEmailNow(MUser from, MUser to, EMail email)
	{
		...
		else
		{
			...
			um.setIsDelivered(REF__YesNo.No);
		}
		um.save();
