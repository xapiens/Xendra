 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MWarehousePrice.java 3654 2011-11-04 01:49:49Z xapiens $
 */
public class MWarehousePrice extends PO 
{
	/**
	...
	private BigDecimal getQtyAvailable() {
		// TODO Auto-generated method stub
		return null;
	}

	public static Object[] findAll (int M_PriceList_Version_ID, int M_Warehouse_ID,int Org_ID)
	{
		String sql = "select m_product_id,value,name,uomsymbol,pricelist,pricestd,pricelimit," +
				"qtyavailable from RV_WAREHOUSEPRICE "+
			 "WHERE isactive='Y' AND M_PriceList_Version_ID=? AND M_Warehouse_ID=? AND ad_org_id=?";
		...
		try
		{
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next())
				list.add(new Object[]{rs.getObject("m_product_id"),//0
						rs.getObject("value"),//1
						rs.getObject("name"),//2
						rs.getObject("uomsymbol"),//3
						rs.getObject("pricelist"),//4
						rs.getObject("pricestd"),//5
						rs.getObject("pricelimit"),//6
						rs.getObject("qtyavailable"),//7
						null,//tax_id //8
						null});//taxamount //9
			rs.close();
			pstmt.close();
	...
	{
		Object[][] Columns={
			{Msg.translate(Env.getCtx(), "M_Product_ID"),"0"},	//0
			{Msg.translate(Env.getCtx(), "Value"),"70"},			//1
			{Msg.translate(Env.getCtx(), "Name"),"280"},		//2
				{Msg.translate(Env.getCtx(), "UOM"),"40"},		//3
			{Msg.translate(Env.getCtx(), "POSPriceList"),"60"},		//4
				{Msg.translate(Env.getCtx(), "POSPriceStd"),"60"},			//5	
				{Msg.translate(Env.getCtx(), "POSPriceLimit"),"60"},	//6
			{Msg.translate(Env.getCtx(), "QtyAvailable"),"60"},	//7
			{Msg.translate(Env.getCtx(), "TaxName"),"0"},//8
			{Msg.translate(Env.getCtx(), "C_Tax_ID"),"0"}//9
			};
		return Columns;
	}	//	find
