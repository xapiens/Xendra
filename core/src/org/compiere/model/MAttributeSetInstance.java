 ...
 *
 *	@author Jorg Janke
 *	@version $Id: MAttributeSetInstance.java 3810 2011-12-09 17:03:56Z xapiens $
 */
public class MAttributeSetInstance extends X_M_AttributeSetInstance
{
	...
	{
		return get(ctx, M_AttributeSetInstance_ID, M_Product_ID, 0);
	}	
	/**
	 ...
	 */
	public static MAttributeSetInstance get (Properties ctx, 
		int M_AttributeSetInstance_ID, int M_Product_ID, int Parent_ID)
	{
		...
		try
		{
			...
			//	M_AttributeSetInstance_ID = rs.getInt(2);	//	needed ?
				...
				if (Parent_ID > 0)
					retValue.setRef_AttributeSetInstance_ID(Parent_ID);
			}
			rs.close();
			pstmt.close();
			pstmt = null;
