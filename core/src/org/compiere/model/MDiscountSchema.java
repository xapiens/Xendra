 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MDiscountSchema.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MDiscountSchema extends X_M_DiscountSchema
{
	...
	public MDiscountSchema (Properties ctx, int M_DiscountSchema_ID, String trxName)
	{
		...
		{
		//	setName();
			setDiscountType (REF_M_DiscountType.FlatPercent);
			setFlatDiscount(Env.ZERO);
			setIsBPartnerFlatDiscount (false);
			setIsQuantityBased (true);	// Y
			setCumulativeLevel(REF_M_DiscountCumulativeLevel.Line);
	...
	// el_man 22/02/08
	public MDiscountSchema get(Timestamp version)
	{
		String sql = "SELECT * FROM M_DiscountSchema WHERE validfrom=?";
		//
		MDiscountSchema retValue = null;
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement(sql, get_TrxName());		
			pstmt.setTimestamp(1, version);							
			ResultSet rs = pstmt.executeQuery();
			if (rs.next())
				retValue = new MDiscountSchema (getCtx(), rs, get_TrxName());
			rs.close();
			pstmt.close();
			pstmt = null;
		}
		...
		BigDecimal BPartnerFlatDiscount)
	{
		...
		if (Price == null || Env.ZERO.compareTo(Price) == 0)
			return Price;
		//
		BigDecimal discount = calculateDiscount(Qty, Price, M_Product_ID, M_Product_Category_ID, BPartnerFlatDiscount);
		...
		BigDecimal BPartnerFlatDiscount)
	{
		...
		//
		if (REF_M_DiscountType.FlatPercent.equals(getDiscountType()))
		{
		...
		//	Not supported
		else if (REF_M_DiscountType.Formula.equals(getDiscountType())
			|| REF_M_DiscountType.Pricelist.equals(getDiscountType()))
		{
