 ...
 *
 *	@author Jorg Janke
 *	@version $Id: MJournalLine.java 4122 2012-05-07 18:11:53Z xapiens $
 */
...
//			+ "WHERE GL_Journal_ID=" + getGL_Journal_ID();	
		String sql = "UPDATE gl_journal j "
			...
			+ "     totalcr = f.totalcr FROM (SELECT COALESCE(SUM(jl.AmtAcctDr),0) as totaldr, COALESCE(SUM(jl.AmtAcctCr),0) as totalcr FROM gl_journalLine jl "
			+ " WHERE jl.isactive = 'Y' AND jl.GL_Journal_ID = "+getGL_Journal_ID()+") f "
			+ " WHERE j.GL_Journal_ID = "+ getGL_Journal_ID();		
...
//				+ getGL_Journal_ID() + ")";
		sql = "UPDATE gl_journalbatch "
			+ " SET TotalDr=( SELECT COALESCE(SUM(TotalDr),0) FROM GL_Journal j WHERE GL_JournalBatch.GL_JournalBatch_ID=j.GL_JournalBatch_ID ) , "
			+ " TotalCr=( SELECT COALESCE(SUM(TotalCr),0) FROM GL_Journal j WHERE GL_JournalBatch.GL_JournalBatch_ID=j.GL_JournalBatch_ID )  "
