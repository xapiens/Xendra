...
import org.compiere.model.reference.REF_C_BP_GroupPriorityBase;
 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MRequest.java 5583 2015-08-05 14:11:58Z xapiens $
 */
public class MRequest extends X_R_Request
{
	...
	public MRequest(Properties ctx, int R_Request_ID, String trxName)
	{
		...
		if (R_Request_ID == 0)
		{
			setDueType (REF_R_RequestDueType.Due);
		//  setSalesRep_ID (0);
		//	setDocumentNo (null);
			setConfidentialType (REF_R_RequestConfidential.PublicInformation);	// A
			setConfidentialTypeEntry (REF_R_RequestConfidential.PublicInformation);	// A
			setProcessed (false);
			setRequestAmt (Env.ZERO);
			setPriorityUser (REF__PriorityRule.Low);
	...
	public void setDueType()
	{
		...
		//
		String DueType = REF_R_RequestDueType.Due;
		if (now.before(due))
			DueType = REF_R_RequestDueType.Scheduled;
		else if (now.after(overdue))
			DueType = REF_R_RequestDueType.Overdue;
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if (confidentialType != null)
				{
					...
					if (ru.getConfidentialTypeEntry().equals(REF_R_RequestConfidential.PrivateInformation)
						&& !confidentialType.equals(REF_R_RequestConfidential.PrivateInformation))
						continue;
					//	Internal not if Customer/Public
					if (ru.getConfidentialTypeEntry().equals(REF_R_RequestConfidential.Internal)
						&& (confidentialType.equals(REF_R_RequestConfidential.PartnerConfidential)
							|| confidentialType.equals(REF_R_RequestConfidential.PublicInformation)))
						continue;
					//	No Customer if public
					if (ru.getConfidentialTypeEntry().equals(REF_R_RequestConfidential.PartnerConfidential)
						&& confidentialType.equals(REF_R_RequestConfidential.PublicInformation))
						continue;
				}
				list.add(ru);
	...
	{
		return getUpdates(REF_R_RequestConfidential.PublicInformation);
	}	//	getUpdatesPublic
	...
	{
		return getUpdates(REF_R_RequestConfidential.PartnerConfidential);
	}	//	getUpdatesCustomer
	...
	{
		return getUpdates(REF_R_RequestConfidential.Internal);
	}	//	getUpdatesInternal
	...
	{
		return REF_R_RequestDueType.Overdue.equals(getDueType());
	}	//	isOverdue
	...
	{
		return REF_R_RequestDueType.Due.equals(getDueType());
	}	//	isDue
	...
	{
		return MRefList.getListName(getCtx(), REF_R_RequestDueType.Identifier, getDueType());
	}	//	getDueTypeText
	...
	{
		return MRefList.getListName(getCtx(), REF__PriorityRule.Identifier, getPriority());
	}	//	getPriorityText
	...
	{
		return MRefList.getListName(getCtx(), REF__PriorityRule.Identifier, getPriorityUser());
	}	//	getPriorityUserText
	...
	{
		return MRefList.getListName(getCtx(), REF_R_RequestConfidential.Identifier, getConfidentialType());
	}	//	getConfidentialText
	...
	{
		return MRefList.getListName(getCtx(), REF_R_RequestConfidential.Identifier, getConfidentialTypeEntry());
	...
	private void setPriority()
	{
		...
		if (getBPartner() != null)
		{
			...
			if (prioBase != null && !prioBase.equals(REF_C_BP_GroupPriorityBase.Same))
			{
				char targetPrio = getPriorityUser().charAt(0);
				if (prioBase.equals(REF_C_BP_GroupPriorityBase.Lower))
					targetPrio += 2;
				else
					targetPrio -= 2;
				if (targetPrio < REF__PriorityRule.High.charAt(0))	//	1
					targetPrio = REF__PriorityRule.High.charAt(0);
				if (targetPrio > REF__PriorityRule.Low.charAt(0))	//	9
					targetPrio = REF__PriorityRule.Low.charAt(0);
				if (getPriority() == null)
	...
	public void setConfidentialTypeEntry (String ConfidentialTypeEntry)
	{
		...
		//
		if (REF_R_RequestConfidential.Internal.equals(getConfidentialType()))
			super.setConfidentialTypeEntry (REF_R_RequestConfidential.Internal);
		else if (REF_R_RequestConfidential.PrivateInformation.equals(getConfidentialType()))
		{
			if (REF_R_RequestConfidential.Internal.equals(ConfidentialTypeEntry)
				|| REF_R_RequestConfidential.PrivateInformation.equals(ConfidentialTypeEntry))
				super.setConfidentialTypeEntry (ConfidentialTypeEntry);
			else
				super.setConfidentialTypeEntry (REF_R_RequestConfidential.PrivateInformation);
		}
		else if (REF_R_RequestConfidential.PartnerConfidential.equals(getConfidentialType()))
		{
			if (REF_R_RequestConfidential.Internal.equals(ConfidentialTypeEntry)
				|| REF_R_RequestConfidential.PrivateInformation.equals(ConfidentialTypeEntry)
				|| REF_R_RequestConfidential.PartnerConfidential.equals(ConfidentialTypeEntry))
				super.setConfidentialTypeEntry (ConfidentialTypeEntry);
			else
				super.setConfidentialTypeEntry (REF_R_RequestConfidential.PartnerConfidential);
		}
		else if (REF_R_RequestConfidential.PublicInformation.equals(getConfidentialType()))
			super.setConfidentialTypeEntry (ConfidentialTypeEntry);
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getConfidentialType() == null)
		{
			...
			if (getConfidentialType() == null)
				setConfidentialType(REF_R_RequestConfidential.PublicInformation);
		}
...
//		setLastResult(msg);
	...
	public void sendNotices(ArrayList list)
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if (NotificationType == null)
					NotificationType = REF_AD_UserNotificationType.EMail;
		...
		//			continue;
				...
				if (AD_Role_ID == -1 
					&& (getConfidentialTypeEntry().equals(REF_R_RequestConfidential.Internal)
						|| getConfidentialTypeEntry().equals(REF_R_RequestConfidential.PrivateInformation)))
					continue;
				
				if (REF_AD_UserNotificationType.None.equals(NotificationType))
				{
					log.config("Opt out: " + Name);
					continue;
				}
				if ((REF_AD_UserNotificationType.EMail.equals(NotificationType)
					|| REF_AD_UserNotificationType.EMailPlusNotice.equals(NotificationType))
					&& (email == null || email.length() == 0))
				{
					if (AD_Role_ID >= 0)
						NotificationType = REF_AD_UserNotificationType.Notice;
					else
					{
				...
				if (REF_AD_UserNotificationType.Notice.equals(NotificationType)
					&& AD_Role_ID >= 0)
				{
	...
	public void doEscalate(boolean user)
	{
		if (user)
		{
			...
			if (REF__PriorityRule.Urgent.equals(Importance))
				;	//	high as it goes
			else if (REF__PriorityRule.High.equals(Importance))
				setPriorityUser(REF__PriorityRule.Urgent);
			else if (REF__PriorityRule.Medium.equals(Importance))
				setPriorityUser(REF__PriorityRule.High);
			else if (REF__PriorityRule.Low.equals(Importance))
				setPriorityUser(REF__PriorityRule.Medium);
			else if (REF__PriorityRule.Minor.equals(Importance))
				setPriorityUser(REF__PriorityRule.Low);
		}
		else
		{
			...
			if (REF__PriorityRule.Urgent.equals(Importance))
				...
				setPriority(REF__PriorityRule.Urgent);
			else if (REF__PriorityRule.Medium.equals(Importance))
				setPriority(REF__PriorityRule.High);
			else if (REF__PriorityRule.Low.equals(Importance))
				setPriority(REF__PriorityRule.Medium);
			else if (REF__PriorityRule.Minor.equals(Importance))
				setPriority(REF__PriorityRule.Low);
		}
	}	//	doEscalate
