 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: Tax.java 5721 2016-01-02 23:13:51Z xapiens $
 */
public class Tax
{
	/**	Logger							*/
		...
		Timestamp billDate, int billFromC_Location_ID, int billToC_Location_ID)
	{
		...
		for (int i = 0; i < taxes.length; i++)
		{
			...
			if (tax.getC_TaxCategory_ID() != C_TaxCategory_ID
				...
				continue;
			if (IsSOTrx && REF_C_TaxSPPOType.PurchaseTax.equals(tax.getSOPOType()))
				continue;
			if (!IsSOTrx && REF_C_TaxSPPOType.SalesTax.equals(tax.getSOPOType()))
				continue;
		...
		}	//	for all taxes in the same category find the default.		
		// in case don't exist a default in the tax category, try to the first 		
		// this is because set a default in the same category is preferred for POS in first place
		for (int i = 0; i < taxes.length; i++)
		{
			MTax tax = taxes[i];
			log.finest(tax.toString());
