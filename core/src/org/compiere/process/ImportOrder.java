...
import org.compiere.model.persistence.X_I_Order;
 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: ImportOrder.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="Import_Order",
name="Import Orders",
...
Identifier="3adaf467-d579-de9c-943a-7add85f9ba7f",
classname="org.compiere.process.ImportOrder",
...
public class ImportOrder extends SvrProcess
{
	...
	@XendraProcessParameter(Name="Client",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="fe0ca8c8-3367-6c05-8387-a119ea9e1472")	
	...
	@XendraProcessParameter(Name="Organization",
                ...
                DisplayLogic="",
                ReadOnlyLogic="",
                Identifier="97ed0aed-44a1-c2ed-8cd8-cc37257fe42b")
	...
	@XendraProcessParameter(Name="Delete old imported records",
                ...
                DisplayLogic="",
                ReadOnlyLogic="",
                Identifier="d44d2bd9-3109-557c-95ee-6ea9bdb2648c")
	...
	@XendraProcessParameter(Name="Document Action",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="8c93c15a-b839-3c8b-cf3c-5a11c1c47e7a")	
	private String			m_docAction = REF__DocumentAction.Prepare;
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql = new StringBuffer ("UPDATE I_Order o "
			  ...
			  + " AND DUMP(o.C_Region_ID::text)=DUMP(l.C_Region_ID::text) AND DUMP(o.C_Country_ID::text)=DUMP(l.C_Country_ID::text)) "
		...
		**/
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				if (imp.getLot() != null && imp.getM_Product_ID() > 0)
				{
					MProduct product = MProduct.get(Env.getCtx(), imp.getM_Product_ID(), get_TrxName());
					MAttributeSetInstance masi = MAttributeSetInstance.create(Env.getCtx(), product, get_TrxName());
					masi.setLot(imp.getLot());
					if (imp.getGuaranteeDate() != null)
						masi.setGuaranteeDate(imp.getGuaranteeDate());
					masi.save();
					line.setM_AttributeSetInstance_ID(masi.getM_AttributeSetInstance_ID());
				}
				line.save();
