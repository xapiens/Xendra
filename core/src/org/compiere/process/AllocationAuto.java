...
public class AllocationAuto extends SvrProcess
{
	...
	DisplayLogic="",
	ReadOnlyLogic="",
	Identifier="2a867366-569f-7237-fd4d-536b306a7df5")					
			...
			for (int i = 0; i < m_invoices.length; i++)
			{
				...
				if (payment.getC_Currency_ID() == invoice.getC_Currency_ID())
				{
					...
					if (difference.signum() == 0)
					{
						...
						{
							throw new XendraSystemError("Cannot create Allocation");
						}
						processAllocation();
						count++;
	...
	private int allocateBPOldestFirst() throws Exception
	{
		...
		{
			throw new XendraSystemError("Allocated Payments=" + allocatedPayments
	...
	private boolean processAllocation()
	{
		if (m_allocation == null)
			return true;
		boolean success = m_allocation.processIt(REF__DocumentAction.Complete);
		if (success)
