 ...
 *  @version $Id: DocumentEngine.java 5762 2016-02-18 02:16:12Z xapiens $
 */
public class DocumentEngine implements DocAction
{
	...
	public boolean processIt (String action)
	{
		...
		if (ACTION_Restore.equals(m_action))
			return RestoreIt();
	 ...
	 * 	Status: Restore
	 * 	@return true if success 
	 * 	@see org.compiere.process.DocAction#RestoreIt()
	 */
	public boolean RestoreIt()
	{
		if (!isValidAction(ACTION_Restore))
			return false;
		if (m_document != null)
		{
			if (m_document.RestoreIt())
			{
				m_status = STATUS_Drafted;
				m_document.setDocStatus(m_status);
				return true;
	...
	public String[] getActionOptions()
	{
		...
		if (isPortfolio())
			return new String[] {ACTION_Discount, ACTION_Warranty, ACTION_Receivables};
		...
		if (isCompleted())
			return new String[] {ACTION_Close, ACTION_ReActivate, 
				...
				ACTION_Post, ACTION_Void, ACTION_Restore};
