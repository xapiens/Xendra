 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: RegisterSystem.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="AD_Registration",
name="Register Now!",
description="Registrations help us to better serve the Xendra User Base.",
help="We will NOT make the data available to any third party or use the information for other than statistical purposes. It will help us, if you would allow to publish your use of Xendra. We will contact you directly before we publish any information.",
Identifier="a5b3b096-1c16-385b-112d-17e80be913a6",
classname="org.compiere.process.RegisterSystem",
...
public class RegisterSystem extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (sys.getName().equals("?") || sys.getName().length()<2)
			throw new XendraUserError("Set System Name in System Record");
		if (sys.getUserName().equals("?") || sys.getUserName().length()<2)
			throw new XendraUserError("Set User Name (as in Web Store) in System Record");
		if (sys.getPassword().equals("?") || sys.getPassword().length()<2)
			throw new XendraUserError("Set Password (as in Web Store) in System Record");
		//	Registration
		...
		if (reg.getC_Location_ID() > 0)
		{
			...
			if (loc.getCity() == null || loc.getCity().length() < 2)
				throw new XendraUserError("No City in Address");
		}
		if (loc == null)
			throw new XendraUserError("Please enter Address with City");

		//	Create Query String
		String enc = WebEnv.ENCODING;
		//	Send GET Request
		StringBuffer urlString = new StringBuffer ("http://www.xendra.org")
			.append("/register_erp?");
		//	System Info
