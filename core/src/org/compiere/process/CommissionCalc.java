 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: CommissionCalc.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="C_Commission_Process",
name="Generate Commission",
description="Generate Commission",
help="",
Identifier="263d4b22-d25e-13c9-a5d9-8112505f374c",
classname="org.compiere.process.CommissionCalc",
updated="2015-06-20 10:10:12")	
public class CommissionCalc extends SvrProcess
{
	@XendraProcessParameter(Name="Start Date",
			                ColumnName="StartDate",
			                Description="First effective day (inclusive)",
			                Help="The Start Date indicates the first or starting date",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="5fa177f4-460e-3292-2d8d-750aee722ed5")
	...
	protected String doIt() throws Exception
	{
		...
		if (m_com.get_ID() == 0)
			throw new XendraUserError ("No Commission");
		...
		if (!comRun.save())
			throw new XendraSystemError ("Could not save Commission Run");
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!comAmt.save())
				throw new XendraSystemError ("Could not save Commission Amt");
			//
			StringBuffer sql = new StringBuffer();
			if (REF_C_CommissionDocBasis.Receipt.equals(m_com.getDocBasisType()))
			{
			...
			else if (REF_C_CommissionDocBasis.Order.equals(m_com.getDocBasisType()))
			{
				if (m_com.isListDetails())
				{
			...
			if (lines[i].isCommissionOrders())
			{
				...
				if (users == null || users.length == 0)
					throw new XendraUserError ("Commission Business Partner has no Users/Contact");
				if (users.length == 1)
				{
	...
	private void setStartEndDate()
	{
		...
		//	Yearly
		if (REF_C_CommissionFrequency.Yearly.equals(m_com.getFrequencyType()))
		{
		...
		//	Quarterly
		else if (REF_C_CommissionFrequency.Quarterly.equals(m_com.getFrequencyType()))
		{
		...
		//	Weekly
		else if (REF_C_CommissionFrequency.Weekly.equals(m_com.getFrequencyType()))
		{
		...
		if (REF_C_CommissionFrequency.Quarterly.equals(m_com.getFrequencyType()))
