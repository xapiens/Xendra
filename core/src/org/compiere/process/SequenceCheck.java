 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: SequenceCheck.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="AD_Sequence_Check",
name="Sequence Check",
...
Identifier="22eb03db-6d14-e448-2143-167f6a4f04aa",
...
public class SequenceCheck extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		//
		//
		// este SQL sirve para detectar nombres duplicados x alguna razon. 
		// Primero analizamos si existe registros duplicados Ej: (AD_Analyze, Ad_Analyze).
		//
		String sql = "SELECT  TableName FROM AD_Table t , AD_Sequence s WHERE UPPER(s.Name)=UPPER(t.TableName) AND t.IsView='N'  group by 1 HAVING COUNT(*) > 1 ";
		String table = DB.getSQLValueString(get_TrxName(), sql);
		if (table  != null)
		{
			addLog(0, null, null, "ERROR: TableName=" + table + " - esta duplicada en sequence!");
			return "@Error@";
		}		 
		//
		...
		try
		{
			...
			while (rs.next())
			{
				...
				try {
					...
					ResultSet tables = dbm.getTables(null, null, seq.getName().toLowerCase(), null );
					lcontinue = tables.getRow() >= 1;
				} catch (SQLException e) {
					e.printStackTrace();
				}				
				if (lcontinue && seq.validateTableIDValue())
				{
