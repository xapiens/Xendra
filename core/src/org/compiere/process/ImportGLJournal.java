 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: ImportGLJournal.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="Import_GLJournal",
name="Import Journal",
description="Import General Ledger Batch/Journal/Line",
help="The Parameters are default values for null import record values, they do not overwrite any data.",
Identifier="6e0612e5-ffe5-142b-0965-2579a7ac2a68",
classname="org.compiere.process.ImportGLJournal",
...
public class ImportGLJournal extends SvrProcess
{
	...
	@XendraProcessParameter(Name="Client",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="83dc97ab-072b-e98f-7d08-1dbbc9712160")	
...
@XendraProcessParameter(Name="Organization",
                ...
                DisplayLogic="",
                ReadOnlyLogic="",
                Identifier="4b2f9d28-8791-78a0-6135-af9e70038f4e")
	...
	@XendraProcessParameter(Name="Accounting Schema",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="ed2f2f75-0a56-c8c9-83e1-b519efe8b718")	
	...
	@XendraProcessParameter(Name="Account Date",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="36032cf9-1e2b-08de-5675-61ee3a054931")	
...
@XendraProcessParameter(Name="Delete old imported records",
                ...
                DisplayLogic="",
                ReadOnlyLogic="",
                Identifier="d0ae7b5b-785e-3fa7-03b0-a92dc232ca4f")
	private boolean			m_DeleteOldImported = false;
	/**	Don't import					*/
	@XendraProcessParameter(Name="Only Validate Data",
			                ColumnName="IsValidateOnly",
			                Description="Validate the date and do not process",
			                Help="",
			                AD_Reference_ID=DisplayType.YesNo,
			                SeqNo=50,
			                ReferenceValueID="",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="e2efd3c3-c034-b0d8-a8af-5408c2796c69")	
	private boolean			m_IsValidateOnly = false;
	/** Import if no Errors				*/
	@XendraProcessParameter(Name="Import only if No Errors",
			                ColumnName="IsImportOnlyNoErrors",
			                Description="Only start the import, if there are no validation Errors",
			                Help="",
			                AD_Reference_ID=DisplayType.YesNo,
			                SeqNo=60,
			                ReferenceValueID="",
			                ...
			                DisplayLogic="",
			                ReadOnlyLogic="",
			                Identifier="40ce3c09-963f-18a1-69a2-c791df191939")	
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		sql.append(" Updated = COALESCE (Updated, CURRENT_TIMESTAMP) "
			  + "WHERE I_IsImported<>'Y' OR I_IsImported IS NULL");
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		sql = new StringBuffer ("UPDATE I_GLJournal i "
			+ "SET DateAcct=CURRENT_TIMESTAMP "
	   ...
	   //	ORDER BY ValidFrom DESC
		...
		sql = new StringBuffer ("SELECT * FROM I_GLJournal "
			+ "WHERE I_IsImported='N'").append (clientCheck)
			.append(" ORDER BY COALESCE(BatchDocumentNo, I_GLJournal_ID::Text),	COALESCE(JournalDocumentNo, I_GLJournal_ID::Text), C_AcctSchema_ID, PostingType, C_DocType_ID, GL_Category_ID, C_Currency_ID, TRUNC(DateAcct), Line, I_GLJournal_ID");
		try
		{
			pstmt = DB.prepareStatement (sql.toString (), get_TrxName());
			ResultSet rs = pstmt.executeQuery ();
