...
import org.compiere.model.persistence.X_I_BankStatement;
 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: BankStatementPayment.java 5583 2015-08-05 14:11:58Z xapiens $
 */
@XendraProcess(value="C_BankStatement Payment",
name="Create Payment",
description="Create Payment from Bank Statement Info",
help="<NULL>",
Identifier="8dfd9a54-2f4a-eb68-153d-8830b5ede723",
classname="org.compiere.process.BankStatementPayment",
...
public class BankStatementPayment extends SvrProcess
{
	...
	private String createPayment (X_I_BankStatement ibs) throws Exception
	{
		...
		if (ibs.getC_Invoice_ID() == 0 && ibs.getC_BPartner_ID() == 0)
			throw new XendraUserError ("@NotFound@ @C_Invoice_ID@ / @C_BPartner_ID@");
		if (ibs.getC_BankAccount_ID() == 0)
			throw new XendraUserError ("@NotFound@ @C_BankAccount_ID@");
		//
		...
		if (payment == null)
			throw new XendraSystemError("Could not create Payment");
		...
		String Description, int AD_Org_ID)
	{
		...
		payment.setTenderType(REF_C_PaymentTenderType.Check);
		if (DateTrx != null)
		...
		payment.save();
		//
		payment.processIt(REF__DocumentAction.Complete);
		payment.save();
		return payment;		
	}	//	createPayment
