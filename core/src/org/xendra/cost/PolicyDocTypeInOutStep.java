...
public class PolicyDocTypeInOutStep extends AbstractStep implements ActionListener {
	...
	private static CLogger log = CLogger.getCLogger(PolicyDocTypeInOutStep.class);
	public PolicyDocTypeInOutStep(DataModel data) {
		super(ResourceLoader.getString("materialpolicywizard", "typeinout"),
				ResourceLoader.getString("materialpolicywizard", "typeinout_description"));
		this.data = data;		
		setCanGoNext(true);
	}
	@Override
	public void prepareRendering() {
		if (!isRender) {
			List<X_C_DocType> dts = new Query(Env.getCtx(), X_C_DocType.Table_Name, "transactiontype = ?", null)
			.setParameters(REF_M_TransactionTransactionType.OnlyMovement).list();
			...
			if (noModel.getSize() == 0) {
				dts = new Query(Env.getCtx(), X_C_DocType.Table_Name, "DocBaseType = ?", null)
				.setParameters(REF_C_DocTypeDocBaseType.MaterialProductionReversed).list();
				for (X_C_DocType dt:dts) {
					ValueNamePair pp = new ValueNamePair(dt.getDocBaseType(), dt.getName());
					noModel.addElement(pp);							
				}
	...
	protected JComponent createComponent() {
		...
		if (Integer.valueOf(PolicyID) > 0) {
			...
			String dtonlyio = (String) p.getProperties().get("dtonlyio");
			StringTokenizer st = new StringTokenizer(dtonlyio, ",");
		...
		//
		data.registerDataLookup(REF_M_TransactionTransactionType.OnlyMovement,new DefaultDataLookup(yesList, method, null));
		JComponent policy = new JPanel(new MigLayout());
		policy.add(new JLabel(ResourceLoader.getString("materialpolicywizard", "typeinout_description")),"wrap");
		policy.add(comp);
		return policy;
	}
