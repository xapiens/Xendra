...
public class PolicyDocTypeAdjustStep extends AbstractStep implements ActionListener {
	...
	private static CLogger log = CLogger.getCLogger(PolicyDocTypeAdjustStep.class);
	public PolicyDocTypeAdjustStep(DataModel data) {
		super(ResourceLoader.getString("materialpolicywizard", "adjust"),
				ResourceLoader.getString("materialpolicywizard", "adjust_description"));
		this.data = data;		
		setCanGoNext(true);
	}
	...
	public void prepareRendering() {
		if (!isRender) {			
			...
			.setParameters(REF_M_TransactionTransactionType.Adjustment, clientid).list();
			...
			if (noModel.getSize() == 0) {
				X_C_DocType dt = new Query(Env.getCtx(), X_C_DocType.Table_Name, "DocBaseType = ?", null)
				.setParameters(REF_C_DocTypeDocBaseType.MaterialAdjust).first();
				if (dt != null) {
					ValueNamePair pp = new ValueNamePair(dt.getDocBaseType(), dt.getName());
					noModel.addElement(pp);							
				}
	...
	protected JComponent createComponent() {
		...
		if (Integer.valueOf(PolicyID) > 0) {
			...
			String adjust = (String) p.getProperties().get("adjust");
			StringTokenizer st = new StringTokenizer(adjust, ",");
		...
		data.registerDataLookup(REF_M_TransactionTransactionType.Adjustment,new DefaultDataLookup(yesList, method, null));
		JComponent policy = new JPanel(new MigLayout());
		policy.add(new JLabel(ResourceLoader.getString("materialpolicywizard", "adjust_description")),"wrap");
		policy.add(comp);
		return policy;								
	}
