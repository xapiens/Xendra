 ...
 *  Xendra PLAF Editor.
 *  <p>
 *  start with <code>new XendraPLAFEditor()</code>
 *
 *  @author     Jorg Janke
 *  @version    $Id: PLAFEditor.java,v 1.1 2007/06/14 23:44:19 sergioaguayo Exp $
 */
	...
	implements ActionListener
{
	...
	private CComboBox lfField = new CComboBox(XendraPLAF.getPLAFs());
	private CLabel themeLabel = new CLabel();
	private CComboBox themeField = new CComboBox(XendraPLAF.getThemes());
	...
	private void setLFSelection()
	{
		...
		for (int i = 0; i < XendraPLAF.getPLAFs().length; i++)
		{
			ValueNamePair vp = XendraPLAF.getPLAFs()[i];
		...
		themeField.setModel(new DefaultComboBoxModel(XendraPLAF.getThemes()));
		if (metal)
		{
			theme = null;
			...
			if (metalTheme != null)
			{
				...
				for (int i = 0; i < XendraPLAF.getThemes().length; i++)
				{
					ValueNamePair vp = XendraPLAF.getThemes()[i];
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			new MiniBrowser("http://wiki.xendra.org");
		}
		...
		else if (e.getSource() == lfField && !m_setting)
		{
			...
			//  set new theme
			XendraPLAF.setPLAF((ValueNamePair)lfField.getSelectedItem(), null, true);
			XendraPLAF.updateUI(this);
			setLFSelection();
		...
		else if (e.getSource() == themeField && !m_setting)
		{
			...
			XendraPLAF.setPLAF((ValueNamePair)lfField.getSelectedItem(), (ValueNamePair)themeField.getSelectedItem(),true);
			XendraPLAF.updateUI(this);
		}
