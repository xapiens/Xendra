...
public class PLAFEditorPanel extends CPanel {
	...
	private JList lookList = new JList(XendraPLAF.getPLAFs());
	...
	private void setupUI() {
		...
		label.setForeground(XendraPLAF.getPrimary1());
		...
		scrollPane.setBorder(BorderFactory.createLineBorder(XendraPLAF.getSecondary1(), 1));
	...
	protected void themeSelectionChanged(ListSelectionEvent e) {
		...
		XendraPLAF.setPLAF(laf, theme, false);
	...
	protected void lookAndFeelSelectionChanged(ListSelectionEvent e) {
		...
		LookAndFeel currentLaf = UIManager.getLookAndFeel();
		MetalTheme currentTheme = MetalLookAndFeel.getCurrentTheme();
		XendraPLAF.setPLAF(laf, null, false);
		previewPanel.refresh(currentLaf,currentTheme);
		SwingUtilities.updateComponentTreeUI(previewPanel);
		updatePreviewComponents();
		setLFSelection();
	...
	private void setupPreview() {
		...
		previewPanel.setBorder(BorderFactory.createLineBorder(XendraPLAF.getSecondary1(), 1));
		...
		jtree.setBorder(BorderFactory.createLineBorder(XendraPLAF.getSecondary1(), 1));
	...
	private void setLFSelection()
	{
		...
		themeList.setModel(new DefaultComboBoxModel(XendraPLAF.getThemes()));
		if (metal)
		{
			theme = null;
		...
		if (capture) {
			...
			if (laf != null) {
				...
				if (laf instanceof MetalLookAndFeel)
					XendraPLAF.setCurrentMetalTheme((MetalLookAndFeel)laf, theme);
				try {
