 ...
 *
 *  @author Jorg Janke 
 *  @version $Id: InOutGenerate.java 1217 2010-05-16 23:33:36Z xapiens $
 */
public class InOutLineGenerate extends SvrProcess
{
	/**	Manual Selection		*/
	...
	// Shipment Date
	private Timestamp   p_DateShipment = null;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("DateShipment"))
				p_DateShipment = (Timestamp)para[i].getParameter();
		...
		if (p_DateShipment == null)
		{
			Timestamp today = new Timestamp(System.currentTimeMillis());
			p_DateShipment = today;
		}
	}	//	prepare
...
//				+ "AND T_Selection.AD_PInstance_ID=? ";
			m_sql = "SELECT DISTINCT o.* FROM  C_Order o LEFT OUTER JOIN C_OrderLine ol "
				+ " ON o.C_Order_ID = ol.C_Order_ID "
				+ " JOIN t_selection2 t "
				+ " ON ol.C_Orderline_ID = t.t_selection_ID  AND o.AD_Client_ID = ? AND t.AD_PInstance_ID = ?";
		}
		else
		{
	...
	private String generate (PreparedStatement pstmt)
	{
		...
		try
		{
			...
			while (rs.next ())		//	Order
			{
				...
				if (order.getM_Warehouse_ID() != p_M_Warehouse_ID)
				{
				...
				String where = " AND C_OrderLine.C_OrderLine_ID IN (SELECT T_Selection_ID from  T_Selection2 t WHERE T.AD_PInstance_ID = "+getAD_PInstance_ID()+") ";
				if (p_DatePromised != null)
				...
				for (int i = 0; i < lines.length; i++)
				{
					...
					//	continue;
					// xapiens : sobreescribimos el almacen de la orden x el almacen destino real.
					// xq el vendedor no necesariamente sabe donde hay stock (no es POS).
					if (line.getM_Warehouse_ID() != p_M_Warehouse_ID)
					{
						BigDecimal reserved = line.getQtyReserved();
						//line.setQtyReserved(Env.ZERO);
						line.setM_Warehouse_ID(p_M_Warehouse_ID);
						line.setQtyReserved(reserved);
						line.save();
					}
					log.fine("check: " + line);
					BigDecimal onHand = Env.ZERO;
					//BigDecimal QtyOrdered = DB.getSQLValueBD(null, "select t.query_id from t_selection2 where AD_PInstance_ID=? AND T_Selection_ID=?", getAD_PInstance_ID(),line.getC_OrderLine_ID());
					//BigDecimal toDeliver = line.getQtyOrdered()
					BigDecimal toDeliver = getQtyOrdered(line);
					...
					if ((product == null || !product.isStocked())
						&& (getQtyOrdered(line).signum() == 0 	//	comments
				...
				if (completeOrder && REF_C_OrderDeliveryRule.CompleteOrder.equals(order.getDeliveryRule()))
				{
					for (int i = 0; i < lines.length; i++)
					{
						...
						BigDecimal toDeliver = getQtyOrdered(line).subtract(line.getQtyDelivered());
		...
		MStorage[] storages, boolean force)
	{
		...
		if (m_shipment == null)
		{
			...
			m_shipment.setShipDate(p_DateShipment);
		...
		if (storages == null)
		{
			...
			//if (orderLine.getQtyEntered().compareTo(getQtyOrdered(orderLine)) != 0)
			//	line.setQtyEntered(qty
			//		.multiply(orderLine.getQtyEntered())
			//		.divide(getQtyOrdered(orderLine), 12, BigDecimal.ROUND_HALF_UP));
	...
	private BigDecimal getQtyOrdered(MOrderLine orderLine) {
		BigDecimal QtyOrdered = DB.getSQLValueBD(get_TrxName(), "Select t.query_id from t_selection2 t where AD_PInstance_ID=? AND T_Selection_ID=?", getAD_PInstance_ID(),orderLine.getC_OrderLine_ID());
		return QtyOrdered;
	}
