...
//		}
		...
		{
			model.addElement(profile.getName());
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (action.equals("CLOSE")) {
			...
			String inputValue = JOptionPane.showInputDialog(
					GlobalResourceLoader.getString(RESOURCE_PATH, "profiles","enter_name"), 
					"");			
			if (inputValue != null) {
				X_A_Profile p = new X_A_Profile(Env.getCtx(), 0, null);
				p.setName(inputValue);
				p.save();
...
//			}
			...
			if (ProfileManager.getInstance().getCurrentProfile().getName().equals(selection)) {
				...
				X_A_Profile p = ProfileManager.getInstance().getProfileForName(selection);
				if (p != null) {
