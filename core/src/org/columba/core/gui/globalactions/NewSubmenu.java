...
public class NewSubmenu extends IMenu implements ActionListener {
	...
	public NewSubmenu(IFrameMediator controller) {
		...
		try {
			frameHandler = PluginManager.getInstance().getExtensionHandler(IExtensionHandlerKeys.ORG_COLUMBA_CORE_FRAME);
		...
		try {
			newItemHandler = PluginManager.getInstance().getExtensionHandler(IExtensionHandlerKeys.ORG_COLUMBA_CORE_NEWITEM);
		} catch (PluginHandlerNotFoundException e) {
			e.printStackTrace();
			return;
		}
