...
//	}
	...
	public static JButton createButton(AbstractColumbaAction action) {
		...
		WITH_TEXT = Config.getInstance().getOptions().getGui().getToolbar().isEnable_text();
		//ALIGNMENT = item.getBoolean("toolbar", "text_position");
		ALIGNMENT = Config.getInstance().getOptions().getGui().getToolbar().isText_position();

		ImageIcon icon = (ImageIcon) action.getValue(AbstractColumbaAction.LARGE_ICON);
		if (icon == null) {
