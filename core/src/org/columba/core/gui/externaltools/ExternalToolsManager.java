...
import org.columba.core.config.Tool;
 ...
 * <p>
 * 
 * @see org.columba.api.plugin.PluginServlet.xml
 * 
 * @author fdietz
 */
public class ExternalToolsManager {
	...
	public File getLocationOfExternalTool(String toolID) {
		...
		Tool tool = getConfiguration(toolID);

		//if (root == null) {
			// create xml node			
			//XmlElement parent = Config.getInstance().get("external_tools").getElement("tools");
		...
		if (firsttime) {
			...
			if (launcher.isFinished()) {
				...
				Tool r = getConfiguration(toolID);
				//File file = new File(r.getAttribute("location"));
				File file = new File(r.getLocation());

				return file;
			}
		} else {
			//String location = root.getAttribute("location");
			String location = tool.getLocation();
	...
	public Tool getConfiguration(String id) {
		Tool ret = null;
		for (Tool tool:Config.getInstance().getTools().getTool())
		{
