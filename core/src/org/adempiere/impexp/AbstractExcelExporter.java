...
//		m_workbook.setRepeatingRowsAndColumns(m_sheetCount, 0, 0, 0, 0);
	...
	protected void createHeaderFooter(HSSFSheet sheet)
	{
		...
		footer.setLeft(Xendra.XENDRA_R);
		...
		for (int rownum = 0, xls_rownum = 1; rownum < getRowCount(); rownum++, xls_rownum++)
		{
			...
			for (int col = 0; col < getColumnCount(); col++)
			{
				...
				if (isColumnPrinted(col))
				{
					...
					if (obj == null)
						;
					else if (DisplayType.isDate(displayType) && obj.getClass() == Timestamp.class) {
						...
						if (obj instanceof Boolean)
							...
							value = "Y".equals(obj);
						cell.setCellValue(new HSSFRichTextString(Msg.getMsg(getLanguage(), value  ? "Y" : "N")));
					}
					else {
