...
public class ContactBox extends JPanel implements IComponentBox {
	...
	private IRuleFolder selectedFolder;
	...
	public ContactBox() {
		...
		if (selectedFolder != null)
			list.addAll(selectedFolder.getChildrens());
		...
		FolderListMenu popup = new FolderListMenu(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				//selectedFolder = (IRuleFolder) model.getFolder(id);
				//List<AbstractFolder> contactList = selectedFolder.getChildrens();
				//list.setModel(new FilteringModel());
		...
		list.addMouseListener(new DoubleClickListener() {
			...
			public void doubleClick(MouseEvent event) {
				X_AD_Rule selected = (X_AD_Rule) list.getSelectedValue();
	...
	private void openEditContactDialog(X_AD_Rule selected) {

		RuleEditorTreeModel model = RuleEditorTreeModel.getInstance();
		selectedFolder = (IRuleFolder) model.getFirstFolder();

		X_AD_Rule card = null;
		try {
			card = (X_AD_Rule) selectedFolder.get(selected.getAD_Rule_ID());
		} catch (Exception e) {
			if (CLogMgt.DEBUG)
				e.printStackTrace();
