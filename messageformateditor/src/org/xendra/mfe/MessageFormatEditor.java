...
public class MessageFormatEditor {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					PluginManager.getInstance().addExtensionHandlers("org/xendra/mfe/plugin/extensionhandler.xml");

					String path = "org/columba/core/plugin/plugin.xml";
					PluginManager.getInstance().addPlugin(path);

					path = "org/xendra/mfe/plugin/plugin.xml";
					PluginManager.getInstance().addPlugin(path);
					//Main.mainClassLoader = new MainClassLoader(Main.class.getClassLoader());
					ComponentManager.getInstance().init();
					FrameManager.getInstance().openView("MessageFormatEditor");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
