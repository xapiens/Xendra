...
public class AcceptInventory extends SvrProcess {
	...
	protected String doIt() throws Exception {
		log.log(Level.WARNING, "ejecutando AcceptorInventory");
		...
		if (kb == null)
		{
			...
			if (m_processMsg == null)
				m_processMsg = "Error in Rule AcceptInventory";
				...
				for (X_A_Xendrian_Message message:messages)
				{
					...
					try {
						if (error.length() == 0)
						{
							error = p.commit();
							if (error.length() == 0) {
								InventoryPolicyRequest spr = new InventoryPolicyRequest();		
								...
								if ( count * 100 / messages.size() != percent )
								{
									...
									System.out.print("Inventory "+percent);
									System.out.println("%");
								}
								count ++;				
							} else {
		...
		System.out.println("terminado AcceptorInventory");
		return null;		
	}
}
