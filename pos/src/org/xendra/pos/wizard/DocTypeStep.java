...
public class DocTypeStep extends AbstractStep implements ActionListener {
	...
	public void prepareRendering() {
		...
		String query =  "DocBaseType = ? AND C_DocType.IsSOTrx='Y' AND DocSubType IN (?,?,?) AND IsActive = 'Y' AND NOT EXISTS(SELECT 1 FROM C_POSDocType pdt WHERE pdt.c_doctype_id = c_doctype.c_doctype_id AND pdt.c_pos_id = ?) ";
		List<X_C_DocType> dtavailable = new Query(Env.getCtx(), X_C_DocType.Table_Name, query, null)
		.setParameters(REF_C_DocTypeDocBaseType.SalesOrder, REF_C_DocTypeSubType.Proposal, REF_C_DocTypeSubType.Quotation, REF_C_DocTypeSubType.POSOrder, C_POS_ID).list();
		...
		List<X_C_POSDocType> dtselected = new Query(Env.getCtx(), X_C_POSDocType.Table_Name, "C_POS_ID = ? AND IsActive = 'Y'", null)
		.setParameters(C_POS_ID).list();
		if (dtselected.size() > 0)
			setCanGoNext(true);
		else
			setCanGoNext(false);		
		for (X_C_POSDocType pdt:dtselected)
		{
			boolean goahead = true;
