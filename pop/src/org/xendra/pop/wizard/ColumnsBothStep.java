...
public class ColumnsBothStep extends AbstractStep implements ActionListener {
	...
	public ColumnsBothStep(DataModel data) {
	...
	public void prepareRendering() {
		if (!initialize)
		{
			...
			String query =  " NOT EXISTS(SELECT 1 FROM C_POPLine pl WHERE pl.sourcemethodname = C_StorePOPLine.sourcemethodname and pl.type = 'B' and pl.c_pop_id = ?) ";
			List<X_C_StorePOPLine> dtavailable = new Query(Env.getCtx(), X_C_StorePOPLine.Table_Name, query, null)
			...
			List<X_C_POPLine> dtselected = new Query(Env.getCtx(), X_C_POPLine.Table_Name, "C_POP_ID = ? AND IsActive = 'Y' AND type ='B'", null)
			...
			for (X_C_POPLine pdt:dtselected)
			{
				...
				if (spl != null)
				{
					...
					//KeyNamePair pp = new KeyNamePair(spl.getC_StorePOPLine_ID(), spl.getName());
	...
	protected JComponent createComponent() {
		...
		//data.registerDataLookup(REF_TypePOPLine.Both,new DefaultDataLookup(yesList, method, null));
        return comp;
	}
