 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: AdempiereMonitor.java,v 1.1 2007/06/15 00:04:42 sergioaguayo Exp $
 */
public class XendraMonitor2 extends HttpServlet
{
	/**	Logger				*/
	private static CLogger	log = CLogger.getCLogger(XendraMonitor.class);
	/**	The Server			*/
	private XendraServerMgr	m_serverMgr = null;
	/** Message				*/
	private p				m_message = null;

	public XendraMonitor2() {
	...
	protected void doGet (HttpServletRequest request, HttpServletResponse response)
		...
		if (processCacheParameter (request, response))
			return;
		//
		if (processRunNowParameter (request, response))
			;
		else
	...
	private boolean processLogParameter (HttpServletRequest request, HttpServletResponse response)
		...
		WebDoc doc = WebDoc.create ("Xendra Server Monitor Log");
		//	Body
		body b = doc.getBody();
		//
		p para = new p();
		a link = new a ("xendraMonitor#" + serverID, "Return");
		...
		XendraProcessorLog[] logs = server.getLogs();
		for (int i = 0; i < logs.length; i++)
		{
			XendraProcessorLog pLog = logs[i];
			line = new tr();
	 ...
	 *	@throws IOException
	 */
	private boolean processRunNowParameter (HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException
			{		
	...
	private void createSummaryPage (HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException
			{
		WebDoc doc = WebDoc.create ("Xendra Server Monitor");
		...
		//
		tr line = new tr();
		line.addElement(new th().addElement(Xendra.getName()));
		line.addElement(new td().addElement(Xendra.getVersion()));
		table.addElement(line);
		line = new tr();
		line.addElement(new th().addElement(Xendra.getImplementationVendor()));
		line.addElement(new td().addElement(Xendra.getImplementationVersion()));
		table.addElement(line);
		line = new tr();
		...
		//
		p para = new p();
		a link = new a ("xendraMonitor?Action=Start_All", "Start All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("xendraMonitor?Action=Stop_All", "Stop All");
		para.addElement(link);
		para.addElement(" - ");
		link = new a ("xendraMonitor", "Refresh");
		para.addElement(link);
		bb.addElement(para);
		...
		bb.addElement(new hr());
		para = new p();
		XendraServer[] servers = m_serverMgr.getAll();		
		...
		for (int i = 0; i < servers.length; i++)
		{
			...
			if (server.isAlive())
			{
				...
				if (server.isInterrupted())
					msg += " (Interrupted)";
				link = new a ("xendraMonitor?Action=Stop_" + server.getServerID(), msg);
			...
			else
			{
				...
				link = new a ("xendraMonitor?Action=Start_" + server.getServerID(), msg);
				line.addElement(new td().addElement(link));
			}
			table.addElement(line);
			//
			...
			td.addElement(" - ");
			link = new a ("xendraMonitor?RunNow=" + server.getServerID(), "(Run Now)");
			td.addElement(link);
			line.addElement(td);
			table.addElement(line);
			//
			...
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("xendraMonitor?Log=" + server.getServerID(), "Log");
			bb.addElement(link);
			bb.addElement(" - ");
			link = new a ("xendraMonitor", "Refresh");
			bb.addElement(link);
	...
	private void createLogMgtPage (body bb)
	{
		...
		line.addElement(new td().addElement(Ini.getXendraHome()));
		table.addElement(line);
		//	OS + Name
		line = new tr();
		...
		XendraDatabase db = cc.getDatabase();
		...
		line.addElement(new td().addElement(new a ("xendraMonitor?CacheReset=Yes", "Reset Cache")));
		table.addElement(line);

		//	Trace Level
		line = new tr();
		line.addElement(new th().addElement(new label("TraceLevel").addElement("Trace Log Level")));
		form myForm = new form("xendraMonitor", form.METHOD_POST, form.ENC_DEFAULT);
		//	LogLevel Selection
		...
		line.addElement(new td().addElement(new a ("xendraMonitor?Trace=" + fileHandler.getFileName(), "Current")));
		table.addElement(line);
		//
		line = new tr();
		line.addElement(new td().addElement(new a ("xendraMonitor?Trace=ROTATE", "Rotate Trace Log")));
		line.addElement(new td().addElement(new a ("xendraMonitor?Trace=DELETE", "Delete all Trace Logs")));
		table.addElement(line);
		//
		bb.addElement(table);
		...
		if (logDir != null && logDir.isDirectory())
		{
			...
			for (int i = 0; i < logs.length; i++) 
			{
				if (i != 0)
					p.addElement(" - ");
				String fileName = logs[i].getAbsolutePath();
				a link = new a ("xendraMonitor?Trace=" + fileName, fileName);
				p.addElement(link);
		...
		for (int i = 0; i < clients.length; i++)
		{
			...
			if (i > 0)
				p.addElement(" - ");
			p.addElement(new a("xendraMonitor?EMail=" + client.getAD_Client_ID(), client.getName()));
		}
		if (clients.length == 0)
			p.addElement("&nbsp;");
		...
		//	
		line = new tr();
		MWebStore[] wstores = MWebStore.getActive();
