		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					PluginManager.getInstance().addExtensionHandlers("org/xendra/pfe/plugin/extensionhandler.xml");

					String path = "org/columba/core/plugin/plugin.xml";
					PluginManager.getInstance().addPlugin(path);

					path = "org/xendra/pfe/plugin/plugin.xml";
					PluginManager.getInstance().addPlugin(path);
					//Main.mainClassLoader = new MainClassLoader(Main.class.getClassLoader());
					ComponentManager.getInstance().init();
					FrameManager.getInstance().openView("PrinterFormatEditor");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
