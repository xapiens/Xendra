...
public class NewField extends AbstractStep {
	...
	public NewField(DataModel data, AbstractFolder node) {
		super(ResourceLoader.getString("dialog", "newfieldwizard", "identify"),
				ResourceLoader.getString("dialog", "newfieldwizard", "identify_description"));
		m_data = data;
		m_node = node;
		setCanGoNext(true);
	}
	...
	protected JComponent createComponent() {
		...
		component.add(new MultiLineLabel(ResourceLoader.getString("dialog","newfieldwizard", "identify_description")));
			...
			if (name.length() > 0) {
				...
				for (int i = 0; i < c; i++) {
					...
					if (ce.getName().equals("field")) {
						String fieldname = ce.getAttributeValue("name");
